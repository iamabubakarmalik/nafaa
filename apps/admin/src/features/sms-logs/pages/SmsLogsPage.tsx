...
import {
  MessageSquare, RefreshCw, CheckCircle2, XCircle, Clock, Send, Beaker,
} from 'lucide-react';
...
export default function SmsLogsPage() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: adminCommunicationsApi.smsStats,
  });

  const { data } = useQuery({
    queryKey: ['admin-sms-logs', statusFilter, page],
    queryFn: () =>
      adminCommunicationsApi.smsLogs({
        status: statusFilter || undefined, page, limit: 30,
      }),
  });

  const retryMutation = useMutation({
    mutationFn: adminCommunicationsApi.retrySms,
    onSuccess: () => {
  ...
  const testMutation = useMutation({
    mutationFn: () => adminCommunicationsApi.testSms(testPhone),
    onSuccess: () => {
      toast.success('Test SMS sent! Check phone.');
      setShowTest(false);
      setTestPhone('');
      queryClient.invalidateQueries({ queryKey: ['admin-sms-logs'] });
    },
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-medium">
              <MessageSquare className="h-3.5 w-3.5" /> SMS Delivery
            </div>
            <h2 className="mt-3 text-3xl font-bold">SMS Logs</h2>
            <p className="mt-2 text-sm text-white/80">
              Provider: <span className="font-bold">{stats?.provider || 'disabled'}</span>
            </p>
          </div>
          <Button onClick={() => setShowTest(true)} className="bg-white text-slate-900 hover:bg-slate-100">
            <Beaker className="h-4 w-4" /> Send Test SMS
          </Button>
        </div>
      </section>
      ...
      <section className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4">
          ...
          <div className="mt-1 text-2xl font-bold text-amber-900">{stats?.today ?? 0}</div>
        </div>
        <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-800 text-white p-4">
          <div className="text-xs text-white/80">Total Cost</div>
          <div className="mt-1 text-lg font-bold">{formatPKR(stats?.totalCost ?? 0)}</div>
        </div>
      </section>
        ...
        {['', 'QUEUED', 'SENT', 'FAILED'].map((s) => (
          <button key={s || 'ALL'} onClick={() => { setStatusFilter(s); setPage(1); }}
            className={`px-4 py-2 rounded-xl text-sm font-semibold ${
              statusFilter === s ? 'bg-emerald-600 text-white' : 'bg-white text-slate-700 border border-slate-200'
            }`}>
            {s || 'ALL'}
          </button>
        ))}
            ...
            <tbody className="divide-y divide-slate-100">
              {(data?.items ?? []).map((log) => (
                <tr key={log.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3 font-mono text-xs">{log.toPhone}</td>
                  <td className="px-6 py-3 text-sm max-w-md">
                  ...
                  <td className="px-6 py-3 text-right text-xs">{formatPKR(log.cost)}</td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold">Send Test SMS</h3>
                <p className="text-sm text-slate-500">Test your SMS provider</p>
              </div>
            </div>
            <Input label="Recipient Phone" value={testPhone}
              onChange={(e) => setTestPhone(e.target.value)} placeholder="+923001234567" />
            <div className="flex gap-2 mt-4">
              <Button variant="secondary" onClick={() => setShowTest(false)} className="flex-1">Cancel</Button>
              <Button onClick={() => {
                if (!testPhone.trim()) return toast.error('Phone likhein');
                testMutation.mutate();
