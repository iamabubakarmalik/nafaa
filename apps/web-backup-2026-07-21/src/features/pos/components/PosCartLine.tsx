  ...
  return (
    <div
      className={`rounded-2xl border-2 transition-all ${
        isEditing
          ? 'border-brand-500 bg-brand-50/40 shadow-lg'
          : notesOpen
    ...
    >
      {/* MAIN ROW — flex fixed heights for cross-browser */}
      <div className="p-3 flex items-start gap-2.5">
        <div className="h-12 w-12 rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center shrink-0 relative ring-2 ring-white shadow-sm">
          {item.variantImage ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="font-extrabold text-slate-900 text-sm leading-tight" style={{ wordBreak: 'break-word' }}>
            {item.name}
          </div>
          ...
          {item.variantName && (
            <div className="flex items-center gap-1 mt-0.5">
              {item.variantColorHex && (
                <span className="h-2.5 w-2.5 rounded-full border border-white shadow-sm shrink-0" style={{ backgroundColor: item.variantColorHex }} />
              )}
              <span className="text-xs font-extrabold text-violet-700 truncate">{item.variantName}</span>
            </div>
          )}

          {item.rollId && item.rollNumber && (
            <div className="mt-1 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-emerald-100 text-emerald-800 text-[10px] font-extrabold">
              <Layers className="h-2.5 w-2.5" />
              {item.rollNumber}
              {item.cutWidthFt !== undefined && item.cutLengthFt !== undefined && (
                <span className="font-bold opacity-80">
                  • {item.cutWidthFt}ft{(item.cutWidthInch || 0) > 0 ? ` ${item.cutWidthInch}in` : ''} × {item.cutLengthFt}ft{(item.cutLengthInch || 0) > 0 ? ` ${item.cutLengthInch}in` : ''}
                </span>
              )}
            </div>
          )}
          ...
          {item.cutPieceId && item.cutPieceCode && (
            <div className="mt-1 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-violet-100 text-violet-800 text-[10px] font-extrabold">
          ...
          {item.imeiNumber && (
            <div className="mt-1 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-blue-100 text-blue-800 text-[10px] font-extrabold font-mono">
              <Smartphone className="h-2.5 w-2.5" />
              {item.imeiNumber}
            </div>
          )}

          <div className="text-xs text-slate-500 mt-1 font-semibold tabular-nums">
          ...
          {hasUserNote && !notesOpen && (
            <div className="mt-1.5 flex items-start gap-1.5 rounded-md bg-amber-100 border border-amber-300 px-1.5 py-1">
              <MessageSquare className="h-3 w-3 text-amber-700 shrink-0 mt-0.5" />
              <div className="flex-1 text-xs font-bold text-amber-900 leading-snug line-clamp-2">
                {userNote}
              </div>
            </div>
          )}
          ...
          {hasInternalNote && !notesOpen && (
            <div className="mt-1 flex items-start gap-1.5 rounded-md bg-slate-100 border border-slate-300 px-1.5 py-1">
              <EyeOff className="h-3 w-3 text-slate-600 shrink-0 mt-0.5" />
              <div className="flex-1 text-xs font-bold text-slate-700 leading-snug line-clamp-2 italic">
                {item.internalNote}
              </div>
            </div>
          )}
          ...
          {hasDiscount && (
            <div className="mt-1 inline-flex items-center gap-1 text-[10px] text-rose-700 font-extrabold bg-rose-50 px-1.5 py-0.5 rounded">
        ...
        <div className="text-right shrink-0 flex flex-col items-end">
          ...
          <div className="flex gap-0.5 mt-1 justify-end">
            <button
              ...
              className={`h-7 w-7 rounded-md flex items-center justify-center transition relative ${
            ...
            <button
              onClick={onRemove}
              className="h-7 w-7 rounded-md bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center transition"
              title="Remove"
            >
        ...
        <div className="px-3 pb-3 -mt-1 space-y-2">
          <div className="rounded-lg bg-amber-50 border-2 border-amber-300 p-2 space-y-1">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-amber-800 flex items-center gap-1">
                <MessageSquare className="h-3 w-3" />
                Customer Note (prints on receipt)
              </label>
              {userNoteDraft.trim() && (
                <button onClick={() => saveUserNote('')} className="text-[10px] font-extrabold text-rose-600 hover:text-rose-800 inline-flex items-center gap-0.5">
            ...
            <textarea
              ...
              rows={2}
              placeholder='"1 piece damage tha, discount de dia"'
          ...
          <div className="rounded-lg bg-slate-100 border-2 border-slate-300 p-2 space-y-1">
            <div className="flex items-center justify-between">
              ...
              {internalNoteDraft.trim() && (
                <button onClick={() => saveInternalNote('')} className="text-[10px] font-extrabold text-rose-600 hover:text-rose-800 inline-flex items-center gap-0.5">
                  <X className="h-2.5 w-2.5" /> Clear
                </button>
              )}
            </div>
            <textarea
              ...
              rows={2}
              placeholder='"Regular customer, same rate next time"'
          ...
          <button
            ...
            className="w-full h-8 rounded-md bg-slate-800 hover:bg-slate-900 text-white text-xs font-extrabold transition"
          >
            Done
          </button>
        </div>
        ...
        <div className="px-3 pb-3 flex items-center justify-between gap-2">
          <div className="inline-flex items-center gap-1 bg-slate-100 rounded-lg p-0.5 shadow-inner">
            <button
              ...
              className="h-8 w-8 rounded-md bg-white border border-slate-200 hover:bg-slate-50 flex items-center justify-center transition shadow-sm"
            >
            ...
            <input
              ...
              className="w-16 text-center font-extrabold text-sm bg-transparent border-0 focus:outline-none"
            />
            <button
              onClick={() => onSetQuantity(item.quantity + 1)}
              disabled={item.quantity >= item.stock}
              className="h-8 w-8 rounded-md bg-brand-600 hover:bg-brand-700 disabled:bg-slate-300 text-white flex items-center justify-center transition shadow-sm"
            >
          ...
          {canUseLW && (
            <button
              onClick={onOpenLW}
              className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-brand-600 to-emerald-600 hover:from-brand-700 hover:to-emerald-700 text-white text-xs font-extrabold inline-flex items-center gap-1 shadow-md"
            >
      ...
      {/* EDIT PANEL */}
      {isEditing && (
        <div className="border-t-2 border-brand-200 bg-brand-50/40 p-3 space-y-2.5">
          {isLocked && (
            <div className="rounded-md bg-blue-50 border border-blue-200 px-2 py-1.5 flex items-start gap-1.5">
              <Lock className="h-3.5 w-3.5 text-blue-700 shrink-0 mt-0.5" />
              <div className="text-xs text-blue-900 font-bold leading-tight">
                {item.rollId && 'Fixed item (roll cut). Rate & discount editable.'}
                {item.cutPieceId && 'Fixed cut piece. Rate & discount editable.'}
                {item.imeiId && 'Fixed IMEI. Rate & discount editable.'}
              </div>
            </div>
          )}

          {item.wholesalePrice && !isLocked && (
            <label className="flex items-center justify-between gap-2 text-xs cursor-pointer bg-white rounded-lg px-3 py-2 border border-amber-200">
              <span className="font-extrabold text-amber-800 inline-flex items-center gap-1">
                <Tag className="h-3.5 w-3.5" />
                Wholesale ({formatPKR(item.wholesalePrice)})
              </span>
              <input
                type="checkbox"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="h-9 w-full rounded-md border-2 border-slate-200 px-2 text-sm font-extrabold focus:outline-none focus:border-brand-500"
              />
            </div>
            <div>
              <label className="block text-[10px] font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Line Discount</label>
              <input
                type="number"
                step="0.01"
                min="0"
          ...
          <button
            onClick={onToggleEdit}
            className="w-full h-9 rounded-md bg-brand-600 hover:bg-brand-700 text-white text-xs font-extrabold inline-flex items-center justify-center gap-1 shadow-sm transition"
          >
