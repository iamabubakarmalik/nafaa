...
import {
  Printer, ArrowLeft, MessageCircle, X as XIcon, StickyNote,
  ...
  Share2, Star, Sparkles, ImageDown,
} from 'lucide-react';
...
const parseCarpetNote = (note?: string | null): CarpetNoteInfo => {
  ...
  if (rollMatch) {
    return { type: 'roll', reference: rollMatch[1], dimensions: rollMatch[2], area: rollMatch[3], customRate: rollMatch[4] };
  }
...
export default function ReceiptPage() {
  ...
  // Auto-print if just opened after sale (owner preference)
  useEffect(() => {
    const autoPrint = localStorage.getItem('nafaa.pos.auto-print') === 'true';
    if (isAutoOpened && autoPrint && data && !isLoading) {
      setTimeout(() => window.print(), 500);
    }
  }, [isAutoOpened, data, isLoading]);
  ...
  const voidMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err?.response?.data?.message || 'Void failed'),
  });

  // ═══════════════════════════════════════════════
  // BILLION-DOLLAR WHATSAPP MESSAGE
  // ═══════════════════════════════════════════════
  const handleWhatsAppShare = () => {
    ...
    const shopEmail = data.tenant?.settings?.shopEmail;
    ...
    lines.push('');
    lines.push('Thank you for choosing us for your shopping needs. 🙏');
    lines.push('_Aap ki tafseel neeche di gayi hai:_');
    lines.push('');
    ...
    if (data.shop) lines.push(`🏪 ${data.shop.name}`);
    if (data.createdBy) lines.push(`👤 Served by: ${data.createdBy.fullName}`);
    lines.push('');

    // ═══ ITEMS WITH BEAUTIFUL FORMATTING ═══
    lines.push('┌─────────────────────────┐');
    ...
    data.items.forEach((it, idx) => {
      ...
      itemImeis.forEach((imei: any) => {
        ...
        if (imei.ptaStatus) {
          const emoji = imei.ptaStatus === 'APPROVED' ? '✅' : imei.ptaStatus === 'NON_PTA' ? '⚠️' : '🛡️';
          lines.push(`   ${emoji} ${PTA_LABELS[imei.ptaStatus] || imei.ptaStatus}`);
        }
        if (imei.warrantyMonths > 0) lines.push(`   ⏱️ Warranty: *${imei.warrantyMonths} months*`);
        if (imei.color) lines.push(`   🎨 Color: ${imei.color}`);
      });

      lines.push(`   ${formatQty(it.quantity)} ${it.product.unit} × ${formatPKR(it.price)}`);
    ...
    if (data.creditAmount > 0) {
      lines.push('');
      lines.push('┌─────────────────────────┐');
      lines.push('│  📒 *KHATA (UDHAAR)*  │');
      lines.push('└─────────────────────────┘');
      lines.push(`💰 Baqi Amount: *${formatPKR(data.creditAmount)}*`);
      lines.push('_Kindly clear at your earliest convenience._');
    }
    lines.push('');
    ...
    lines.push('└─────────────────────────┘');
    lines.push('');
    lines.push('🌟 We hope you loved shopping with us!');
    lines.push('');
    lines.push('_Aap ki har khareedari hamare liye important hai. Please share your feedback aur phir tashreef laiye ga!_ 😊');
    lines.push('');
    lines.push('🎁 *Refer a friend & get exclusive discounts!*');
    lines.push('');
    ...
    if (shopEmail) lines.push(`📧 ${shopEmail}`);
    ...
    lines.push('_Pakistan\'s #1 Business Solution_ 🇵🇰');

    const message = lines.join('\n');
    window.open(`https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}`, '_blank');
    toast.success('WhatsApp opened — beautiful receipt ready to send! 🎉');
  };
  ...
  const handleDownloadImage = async () => {
    toast.info('Screenshot feature coming soon — use Print → Save as PDF for now');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-6 px-4 print:bg-white print:py-0 print:px-0">
      <div className={`mx-auto space-y-4 ${format === 'a4' ? 'max-w-4xl' : 'max-w-md'}`}>
        {/* AUTO-OPENED BANNER */}
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
            <div className="h-9 w-9 rounded-xl bg-white/20 flex items-center justify-center">
              <CheckCircle2 className="h-5 w-5" />
            </div>
            <div className="flex-1">
              <div className="font-extrabold text-base">Sale Complete! 🎉</div>
              <div className="text-xs text-white/90 font-semibold">Receipt ready — print, WhatsApp, or share</div>
            </div>
            <Link to="/pos" className="text-xs font-extrabold underline hover:no-underline">
              → New Sale
            </Link>
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="inline-flex rounded-xl border-2 border-slate-200 bg-white shadow-sm overflow-hidden">
              <button onClick={() => setFormat('a4')} className={`px-3 py-2.5 text-xs font-bold transition ${format === 'a4' ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-slate-50'}`}>A4</button>
              <button onClick={() => setFormat('thermal80')} className={`px-3 py-2.5 text-xs font-bold transition border-l-2 border-slate-200 ${format === 'thermal80' ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-slate-50'}`}>80mm</button>
              <button onClick={() => setFormat('thermal58')} className={`px-3 py-2.5 text-xs font-bold transition border-l-2 border-slate-200 ${format === 'thermal58' ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-slate-50'}`}>58mm</button>
            </div>

            <button onClick={handleCopyLink} className="inline-flex items-center gap-2 rounded-xl bg-white border-2 border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 shadow-sm transition">
        ...
        {isVoided && (
          <div className="rounded-2xl border-2 border-rose-300 bg-gradient-to-br from-rose-50 to-rose-100 px-5 py-4 flex items-center gap-3 print:rounded-none print:border print:bg-white">
            ...
            <div>
              ...
              <div className="text-xs text-rose-700 mt-0.5">Stock aur customer credit reverse ho gaye hain</div>
            </div>
          </div>
        )}
        ...
        {(format === 'thermal58' || format === 'thermal80') && (
          <div
            className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`}
            style={{ fontFamily: 'Consolas, "Courier New", monospace' }}
          >
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              <div className="text-center mb-2">
                {showLogo && logoUrl && (
                  <img src={logoUrl} alt={shopName} className={`mx-auto mb-2 object-contain ${format === 'thermal58' ? 'h-12 w-12' : 'h-14 w-14'}`} />
                )}
                ...
                {taxNumber && <div className="text-[9px] mt-0.5 font-bold">{taxLabel} #: {taxNumber}</div>}
              </div>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                <div className="flex justify-between"><span className="font-bold">Receipt #</span><span className="font-bold">{data.saleNumber}</span></div>
                <div className="flex justify-between"><span>Date:</span><span>{formatDateShort(data.soldAt)}</span></div>
                {data.createdBy && <div className="flex justify-between"><span>Cashier:</span><span>{data.createdBy.fullName}</span></div>}
                {data.shop && <div className="flex justify-between"><span>Branch:</span><span>{data.shop.name}</span></div>}
              </div>

              {showCustomer && data.customer && (
                <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                  <div className="flex justify-between"><span className="font-bold">Customer:</span><span className="font-bold">{data.customer.name}</span></div>
                  ...
                  return (
                    <div key={item.id} className="mb-1.5">
                      ...
                      {carpet.type === 'cut-piece' && (
                        <div className="text-[9px] pl-2">Piece {carpet.reference}{carpet.dimensions && ` ${carpet.dimensions}`}</div>
                      )}
                        ...
                        <div className="pl-2 space-y-0.5">
                          {(item as any).imeis.map((imei: any) => (
                            <div key={imei.id} className="text-[9px] border-l-2 border-slate-400 pl-1">
                              ...
                              {imei.ptaStatus && <div className="font-bold">{PTA_LABELS[imei.ptaStatus] || imei.ptaStatus}</div>}
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                ...
                {data.discount > 0 && <div className="flex justify-between"><span>Discount:</span><span>-{formatPKR(data.discount)}</span></div>}
                {data.serviceChargesBreakdown && data.serviceChargesBreakdown.length > 0 && (
                  <>
                    <div className="pt-1 mt-1 border-t border-dotted border-slate-400 font-bold text-center">SERVICE CHARGES</div>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                <div className="flex justify-between"><span>Payment ({paymentLabels[data.paymentMethod]}):</span><span className="font-bold">{formatPKR(data.paidAmount)}</span></div>
                {data.changeAmount > 0 && <div className="flex justify-between"><span>Change:</span><span className="font-bold">{formatPKR(data.changeAmount)}</span></div>}
                {data.creditAmount > 0 && <div className="flex justify-between font-bold"><span>UDHAAR:</span><span>{formatPKR(data.creditAmount)}</span></div>}
              </div>
          ...
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border border-slate-200 overflow-hidden print:shadow-none print:border-none print:rounded-none">
            ...
            <div className="relative bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 px-8 py-7 text-white print:bg-white print:text-slate-900 print:border-b-4 print:border-double print:border-slate-700 overflow-hidden">
              ...
              <div className="relative flex items-start justify-between gap-6 flex-wrap">
                <div className="flex items-start gap-4 min-w-0 flex-1">
                  {showLogo && logoUrl && (
                    <img src={logoUrl} alt={shopName} className="h-20 w-20 rounded-2xl object-cover bg-white p-1.5 shrink-0 shadow-lg print:rounded-lg" />
                  )}
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 space-y-1 text-xs text-white/85 print:text-slate-600">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {shopPhone && <span className="inline-flex items-center gap-1"><Phone className="h-3 w-3" />{shopPhone}</span>}
                        {shopWhatsapp && shopWhatsapp !== shopPhone && <span className="inline-flex items-center gap-1"><MessageCircle className="h-3 w-3" />{shopWhatsapp}</span>}
                        {shopEmail && <span className="inline-flex items-center gap-1"><Mail className="h-3 w-3" />{shopEmail}</span>}
                        {shopWebsite && <span className="inline-flex items-center gap-1"><Globe className="h-3 w-3" />{shopWebsite}</span>}
                      </div>
                      {taxNumber && (
                        ...
                        <tr key={item.id} className="border-b border-slate-100 align-top hover:bg-slate-50/50 transition">
                          ...
                          <td className="py-4 px-2">
                            ...
                            {(item.product.sku || item.product.barcode) && (
                              <div className="text-[10px] text-slate-500 mt-0.5 font-mono">{item.product.sku || item.product.barcode}</div>
                            )}
                            ...
                            {(item as any).imeis && (item as any).imeis.length > 0 && (
                              <div className="mt-2 space-y-1">
                                {(item as any).imeis.map((imei: any) => (
                                  <div key={imei.id} className="rounded-lg border-2 border-blue-200 bg-blue-50 p-2 print:bg-white print:border-slate-400">
                                    <div className="flex items-center justify-between gap-2 flex-wrap">
                                      ...
                                      {imei.ptaStatus && (
                                        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full border text-[9px] font-extrabold uppercase ${PTA_BADGE_COLORS[imei.ptaStatus] || PTA_BADGE_COLORS.PENDING}`}>
                                    ...
                                    {imei.imei2 && (
                                      <div className="text-[10px] font-mono text-slate-600 mt-0.5">IMEI 2: <span className="font-bold">{imei.imei2}</span></div>
                                    )}
                                    {imei.serialNumber && (
                                      <div className="text-[10px] font-mono text-slate-600">S/N: <span className="font-bold">{imei.serialNumber}</span></div>
                                    )}
                                    <div className="flex flex-wrap gap-2 mt-1 text-[10px]">
                                      {imei.color && <span className="font-bold text-violet-700">🎨 {imei.color}</span>}
                                      {imei.warrantyMonths > 0 && <span className="font-bold text-teal-700">🛡️ {imei.warrantyMonths}m warranty</span>}
                                      {imei.warrantyExpiry && <span className="font-bold text-slate-700">Expires: {new Date(imei.warrantyExpiry).toLocaleDateString('en-PK')}</span>}
                                    </div>
                                  </div>
                                ))}
                              </div>
                          ...
                          <td className="py-4 px-2 text-center">
                            <div className="text-xl font-extrabold text-slate-900 tabular-nums">{formatQty(item.quantity)}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{item.product.unit}</div>
                          </td>
                          <td className="py-4 px-2 text-right">
                            <div className="text-slate-700 font-bold tabular-nums">{formatPKR(item.price)}</div>
                            <div className="text-[9px] text-slate-500 font-semibold">per {item.product.unit}</div>
                          </td>
                          <td className="py-4 px-2 text-right">
                            <div className="font-extrabold text-emerald-700 tabular-nums text-base">{formatPKR(item.total)}</div>
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="px-8 py-4 bg-amber-50 border-t-2 border-amber-300 flex items-center justify-between gap-3 print:hidden">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm">
                    ...
                    <span className="text-amber-700"> ka udhaar update</span>
                  </div>
                </div>
            ...
            <div className="px-8 py-6 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-emerald-50 to-green-50 print:bg-white">
              ...
              <div className="text-[10px] text-slate-400 mt-3 inline-flex items-center gap-1">
                ...
                Powered by Nafaa POS • {formatDate(new Date().toISOString())}
              </div>
            </div>
          </div>
        )}
