...
import {
  Printer, ArrowLeft, MessageCircle, X, Shirt, Palette, Ruler,
...
export default function GarmentReceiptPage() {
  const { id } = useParams();
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Boutique';
    ...
    sale.items.forEach((it: any, i: number) => {
      ...
      const name = variant ? `${it.product.name} (${variant.name})` : it.product.name;
      lines.push(`${i + 1}. ${name} × ${it.quantity} = ${formatPKR(it.total)}`);
    ...
    lines.push('');
    lines.push('_Please visit again! Alterations available._ ❤️');
  ...
  const shopName = settings?.shopName || sale.tenant?.name || 'Boutique';
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <Shirt className="h-3 w-3" /> Boutique Invoice
                    </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b-2 border-slate-300 bg-gradient-to-r from-pink-50 to-slate-50 print:bg-white">
                    ...
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase">Garment</th>
                        ...
                        <td className="py-3 px-2">
                          ...
                          {variant && (
                            <div className="mt-1 flex flex-wrap gap-1.5">
                              {variant.size && (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-pink-100 text-pink-800 text-[10px] font-extrabold">
                                  <Ruler className="h-2.5 w-2.5" /> Size: {variant.size}
                                </span>
                              )}
                              {variant.color && (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-fuchsia-100 text-fuchsia-800 text-[10px] font-extrabold">
                                  {variant.colorHex && <span className="h-2.5 w-2.5 rounded-full border border-white shadow" style={{ backgroundColor: variant.colorHex }} />}
                                  <Palette className="h-2.5 w-2.5" /> {variant.color}
                                </span>
                              )}
                              {variant.sku && (
                                <span className="text-[9px] font-mono text-slate-500">SKU: {variant.sku}</span>
                              )}
                            </div>
                          )}
                          {it.note && !variant && (
                            <div className="mt-1 text-[10px] italic text-amber-700">📝 {it.note}</div>
                          )}
                        </td>
            ...
            <div className="px-8 py-5 border-t-2 border-slate-100 bg-pink-50/50 print:bg-white">
              <div className="text-[10px] uppercase font-extrabold text-pink-700 mb-2">Terms</div>
              <ul className="text-[11px] text-slate-700 space-y-1 list-disc pl-4">
                <li>Exchange/return within 3 days with original tags & invoice (unworn)</li>
                <li>Alterations available — bring garment within 7 days with this receipt</li>
                <li>Sale items are non-returnable, non-exchangeable</li>
                <li>Custom tailored items are strictly non-returnable</li>
              </ul>
            </div>
          ...
          <div className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`} style={{ fontFamily: 'Consolas, "Courier New", monospace' }}>
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1 text-[9px] italic">
                ...
                <div>• Exchange within 3 days (unworn, with tags)</div>
                <div>• Alterations available within 7 days</div>
                <div>• Custom items non-returnable</div>
              </div>
