import { Check, Milk, Beaker, Truck } from 'lucide-react';
import type { WizardStep } from '../../hooks/useDairyWizard';
...
const STEPS = [
  { id: 1, label: 'Product Info', desc: 'Type, unit, pricing', icon: Milk },
  { id: 2, label: 'Composition', desc: 'Fat/SNF/quality', icon: Beaker },
  { id: 3, label: 'Availability', desc: 'Slots & delivery', icon: Truck },
] as const;

export function DairyWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-2">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-fuchsia-600 to-pink-700 text-white shadow-md shadow-fuchsia-500/40'
                    : isPast
                      ? 'bg-fuchsia-50 text-fuchsia-800 hover:bg-fuchsia-100 border-2 border-fuchsia-200'
              ...
              >
                <div className={[
                  ...
                  isActive ? 'bg-white/25 text-white'
                    : isPast && isComplete ? 'bg-fuchsia-600 text-white'
                ...
                <div className="text-left">
                  <div className={[
                    ...
                    isActive ? 'text-white/90' : isPast ? 'text-fuchsia-700' : 'text-slate-500',
                  ].join(' ')}>
                    Step {s.id}
                  </div>
              ...
              {idx < STEPS.length - 1 && (
                <div className={['h-0.5 w-8 rounded-full transition',
                  currentStep > s.id ? 'bg-fuchsia-500' : 'bg-slate-200'].join(' ')} />
              )}
            </div>
          );
        })}
