...
import {
  Printer, ArrowLeft, MessageCircle, X, Wrench, MapPin, Phone,
  Calendar, User, CheckCircle2, ShieldAlert, Tag, Shield, Clock,
} from 'lucide-react';
...
export default function ServicesBizReceiptPage() {
  const { id } = useParams();
  ...
  const { data: sale, isLoading } = useQuery({
    queryKey: ['sale-receipt', id], queryFn: () => salesApi.getOne(id!), enabled: !!id,
  });
  ...
  useEffect(() => {
    ...
    if (isAutoOpened && autoPrint && sale && !isLoading) setTimeout(() => window.print(), 500);
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Service Center';

    const lines: string[] = [
      `🛠️ *${shopName}*`, '',
      `Assalam-o-Alaikum ${sale.customer.name || 'Customer'}!`,
    ...
    sale.items.forEach((it: any, i: number) => {
      lines.push(`${i + 1}. 🛠️ ${it.product.name} × ${it.quantity} = ${formatPKR(it.total)}`);
    });
    ...
    lines.push('', '_Service warranty as per terms._ 🛡️');

    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(lines.join('\n'))}`, '_blank');
  };

  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><div className="h-10 w-10 rounded-full border-4 border-cyan-200 border-t-cyan-600 animate-spin" /></div>;
  if (!sale) return <div className="min-h-screen flex flex-col items-center justify-center"><p className="font-bold">Receipt not found</p><Link to="/sales" className="mt-4 text-cyan-600 hover:underline">← Back</Link></div>;

  const settings = sale.tenant?.settings;
  const shopName = settings?.shopName || sale.tenant?.name || 'Service Center';
        ...
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
            ...
            <div className="flex-1">
              <div className="font-extrabold">Service Sale Complete! 🛠️</div>
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <Wrench className="h-3 w-3" /> Service Invoice
                    </div>
                  ...
                  {sale.items.map((it: any, idx: number) => (
                    <tr key={it.id} className="border-b border-slate-100 align-top hover:bg-slate-50/50">
                      ...
                      <td className="py-3 px-2">
                        <div className="font-extrabold text-slate-900">🛠️ {it.product.name}</div>
                        {it.note && (
            ...
            <div className="px-8 py-5 border-t-2 border-slate-100 bg-cyan-50/50 print:bg-white">
              <div className="text-[10px] uppercase font-extrabold text-cyan-700 mb-2 flex items-center gap-1"><Shield className="h-3 w-3" /> Service Warranty & Terms</div>
              <ul className="text-[11px] text-slate-700 space-y-1 list-disc pl-4">
                <li>🛡️ Service warranty as per individual service terms (typically 30-90 days)</li>
                <li>🔧 Parts warranty separate from labor warranty</li>
                <li>📞 Contact us immediately for any post-service issues</li>
                <li>💳 Payment terms as agreed at time of service</li>
              </ul>
            </div>
            ...
            <div className="px-8 py-5 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-cyan-50 to-blue-50 print:bg-white">
              ...
              <div className="text-lg font-extrabold text-slate-900">🛠️ Shukriya! Quality service guaranteed 🛡️</div>
        ...
        {(format === 'thermal58' || format === 'thermal80') && (
          <div className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`} style={{ fontFamily: 'Consolas, "Courier New", monospace' }}>
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                <div className="font-bold text-center mb-1">🛠️ SERVICES</div>
                {sale.items.map((it: any, idx: number) => (
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1 text-[9px] italic">
                <div className="font-bold">🛡️ Warranty:</div>
                <div>As per service terms</div>
                <div>Contact for issues</div>
              </div>

              {receiptFooter && <div className="text-center text-[9px] italic border-t border-dashed border-slate-400 pt-1">{receiptFooter}</div>}
              <div className="text-center font-bold mt-2">🛠️ Shukriya! 🛡️</div>
