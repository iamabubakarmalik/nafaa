...
import {
  DollarSign, Hash, Barcode, Image as ImageIcon, Sparkles,
...
import type {
  CarpetWizardBasic, CarpetWizardVariant, CarpetStockType,
} from '../../hooks/useCarpetWizard';

interface Props {
  variant: CarpetWizardVariant;
  basic: CarpetWizardBasic;
  onChange: (patch: Partial<CarpetWizardVariant>) => void;
}

/**
 * VariantEditPanel — inline expandable body for a variant chip.
 * Shows per-variant overrides for name, design code, SKU, barcode,
 * price (cost/sale/wholesale), stock-type override, image + notes.
 *
 * Product-level values act as fallbacks: user only enters overrides
 * where this variant differs from the product default.
 */
export function VariantEditPanel({ variant, basic, onChange }: Props) {
  const [expanded, setExpanded] = useState(false);

  const effectiveCost = variant.costPriceOverride ?? Number(basic.costPricePerSqft || 0);
  const effectiveSale = variant.salePriceOverride ?? Number(basic.salePricePerSqft || 0);
  const effectiveWholesale =
    variant.wholesalePriceOverride ?? Number(basic.wholesalePricePerSqft || 0);
  const profit = effectiveSale - effectiveCost;
  const margin = effectiveSale > 0 ? (profit / effectiveSale) * 100 : 0;
      ...
      >
        <div className="flex-1 min-w-0 flex items-center gap-2">
          ...
          <div className="text-xs text-slate-600 font-bold ml-auto tabular-nums">
            {formatPKRFull(effectiveSale)} / {basic.unit}
            {profit > 0 && (
        ...
        <div className="p-4 border-t-2 border-slate-100 space-y-4">
          ...
          <div className="rounded-xl bg-slate-50 border-2 border-slate-200 p-3 space-y-2.5">
            ...
            <div className="grid sm:grid-cols-3 gap-2">
              <Input
                label="Design Code"
                value={variant.designCode ?? ''}
                onChange={(e) => onChange({ designCode: e.target.value })}
                placeholder={basic.designCode || 'e.g. SF-CR-17'}
                hint={basic.designCode ? `Product: ${basic.designCode}` : undefined}
              />
              <Input
                label="Variant SKU"
                value={variant.sku ?? ''}
                onChange={(e) => onChange({ sku: e.target.value })}
                placeholder="e.g. SF-CREAM"
              />
              <Input
          ...
          <div className="rounded-xl bg-emerald-50 border-2 border-emerald-200 p-3 space-y-2.5">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="text-[10px] uppercase tracking-wider font-extrabold text-emerald-700 flex items-center gap-1">
                <DollarSign className="h-3 w-3" /> Prices (per {basic.unit})
              </div>
              {(variant.costPriceOverride !== undefined ||
                variant.salePriceOverride !== undefined ||
                variant.wholesalePriceOverride !== undefined) && (
            ...
            {effectiveSale > 0 && effectiveCost > 0 && (
              <div className="rounded-lg bg-white border border-emerald-200 p-2 text-xs">
                <span className="font-bold text-slate-700">Profit / {basic.unit}: </span>
                <span
                  className={[
          ...
          {/* Mixed-mode: stock type override */}
          {basic.stockType === 'MIXED' && (
            <div className="rounded-xl bg-violet-50 border-2 border-violet-200 p-3 space-y-2.5">
              <div className="text-[10px] uppercase tracking-wider font-extrabold text-violet-700 flex items-center gap-1">
                <Layers className="h-3 w-3" /> Stock Type for this variant
              </div>
              <div className="grid grid-cols-3 gap-2">
                {(['ROLLS', 'PIECES', 'FT'] as CarpetStockType[]).map((t) => {
                  const Icon = t === 'ROLLS' ? Layers : t === 'PIECES' ? Package : Ruler;
                  const active = (variant.stockTypeOverride ?? 'ROLLS') === t;
                  return (
                    <button
                      key={t}
                      type="button"
                      onClick={() => onChange({ stockTypeOverride: t })}
                      className={[
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Premium quality, limited stock…"
                    className="w-full rounded-lg border-2 border-slate-200 px-2 py-1.5 text-xs font-semibold focus:outline-none focus:border-emerald-500"
                  />
                </div>
                <label className="inline-flex items-center gap-2 cursor-pointer p-2 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 transition">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 font-semibold">
                      Show this variant in POS & catalog
                    </div>
                  </div>
                </label>
              </div>
    ...
    <div>
      ...
      <input
        ...
        className={[
          ...
          accent
            ? 'border-emerald-200 bg-white focus:border-emerald-500 text-emerald-800'
