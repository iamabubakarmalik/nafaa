import {
  ...
  AlertTriangle, CheckCircle2, Ruler,
} from 'lucide-react';
...
export function CarpetWizardSummary({ draft, stats, allValid }: Props) {
  return (
    <aside className="flex flex-col gap-3 xl:sticky xl:top-4 xl:self-start">
      <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 text-white p-5 shadow-xl overflow-hidden relative">
        ...
        <div className="relative">
          ...
          <h3 className="mt-2 font-extrabold text-xl leading-tight line-clamp-2">
            {draft.basic.name || 'Untitled Carpet Product'}
          </h3>
          ...
          <div className="text-[10px] text-white/70 font-extrabold uppercase tracking-wider mt-1">
            Stock type: <span className="text-emerald-300">{draft.basic.stockType}</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-2 divide-x divide-slate-100 border-t border-slate-100">
          <StatCell
            ...
            tone="violet"
            hint={stats.pieceLineCount > 0 ? `${stats.pieceLineCount} lines` : undefined}
          />
          <StatCell
            icon={Ruler}
            label="Feet"
            value={stats.ftTotal.toFixed(1)}
            tone="blue"
            hint="running feet"
          />
        </div>
      ...
      {(stats.rollCount > 0 || stats.pieceCount > 0 || stats.ftTotal > 0) && (
      ...
      <div className="rounded-xl bg-slate-50 border border-slate-200 p-2.5 text-[10px] text-slate-500 font-semibold text-center">
        💾 Draft auto-saves as you type — you won't lose progress
      </div>
    </aside>
  );
}
