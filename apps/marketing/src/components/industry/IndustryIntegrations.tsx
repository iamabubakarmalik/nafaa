...
export function IndustryIntegrations({ industry, content }: { industry: Industry; content: IndustryContent }) {
  ...
  const relevantIntegrations = integrations.filter((it) => content.integrationSlugs.includes(it.slug));
    ...
    <Section variant="subtle" spacing="lg">
      <Container>
        ...
        >
          ...
          >
            {isUr
              ? `${industry.nameUr} کے لیے تجویز کردہ ٹولز`
              : `Recommended tools for your ${industry.nameEn.toLowerCase()}`}
          </motion.h2>
        </motion.div>
        ...
        >
          {relevantIntegrations.map((it) => (
            <motion.div key={it.slug} variants={fadeUp}>
              ...
              >
                <div className="text-4xl">{it.logo}</div>
