...
export default function RegisterPage() {
  ...
  const passwordRegMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(`Welcome to Nafaa Bazaar, ${data.customer.fullName}! 🎉`);
        ...
        <div className="w-full max-w-md">
          ...
          <Card className="p-6 md:p-8 shadow-soft-lg space-y-5">
            <div className="text-center">
              <div className="inline-flex items-center gap-1.5 rounded-full bg-brand-100 dark:bg-brand-900/40 border border-brand-200 dark:border-brand-800 px-3 py-1 text-2xs font-black text-brand-700 dark:text-brand-400 mb-3">
                <Sparkles className="h-3 w-3" />
                Join 500k+ shoppers
              </div>
              <h1 className="text-2xl md:text-3xl font-black text-content">Create your account</h1>
              <p className="text-sm text-content-muted mt-1">Pakistan's #1 marketplace</p>
            </div>
            ...
            >
              ...
              {method === 'password' && (
                <>
                  <Input
                    ...
                    inputSize="lg"
                    hint="For order updates and receipts"
                  />
                  <div>
                    <Input
                      label="Password"
                          ...
                          {[1, 2, 3, 4, 5].map((s) => (
                            <div
                              ...
                              className={cn(
                                ...
                                s <= strength ? strengthColor : 'bg-surface-muted',
                              )}
                            />
                          ))}
                        </div>
                        <div className="text-2xs text-content-muted mt-1 font-bold">
                          Strength: <span className="text-content">{strengthLabel}</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                <input
                  ...
                  className="h-4 w-4 mt-0.5 rounded accent-brand-600"
                />
                <span className="text-xs text-content-muted">
                  I agree to the{' '}
                  <Link to="/terms" className="underline text-brand-600 font-bold">Terms</Link> and{' '}
                  <Link to="/privacy" className="underline text-brand-600 font-bold">Privacy Policy</Link>
                </span>
              </label>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 text-center">
            {[
              ...
              { icon: Sparkles, label: '100 pts bonus' },
            ].map((t) => {
              const Icon = t.icon;
              return (
                <div key={t.label} className="p-3 rounded-2xl bg-surface/50 backdrop-blur-sm border border-border">
                  <Icon className="h-4 w-4 text-brand-600 mx-auto mb-1" />
                  <div className="text-2xs font-black text-content">{t.label}</div>
                </div>
              );
            })}
          </div>
