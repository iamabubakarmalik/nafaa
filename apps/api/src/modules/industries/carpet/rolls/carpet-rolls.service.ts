...
export class CarpetRollsService {
  ...
  private calcSqft(widthFt: number, widthInch: number, lengthFt: number): number {
    const totalWidth = Number(widthFt) + Number(widthInch || 0) / 12;
    return Number((totalWidth * Number(lengthFt)).toFixed(2));
  }
  ...
  async create(user: AuthenticatedUser, dto: CreateCarpetRollDto) {
    ...
    const originalSqft = this.calcSqft(dto.widthFt, dto.widthInch ?? 0, dto.originalLengthFt);
    const remainingSqft = this.calcSqft(dto.widthFt, dto.widthInch ?? 0, remainingLengthFt);
    ...
    const roll = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.carpetRollMovement.create({
        data: {
          ...
          tenantId: user.tenantId,
          type: 'OPENING',
          lengthFt: remainingLengthFt,
          sqft: remainingSqft,
          balanceLengthAfter: remainingLengthFt,
  ...
  async update(user: AuthenticatedUser, id: string, dto: UpdateCarpetRollDto) {
    ...
    const originalSqft = this.calcSqft(widthFt, widthInch, originalLengthFt);
    const remainingSqft = this.calcSqft(widthFt, widthInch, remainingLengthFt);
  ...
  async cut(user: AuthenticatedUser, id: string, dto: CutRollDto) {
    ...
    if (dto.lengthFt > roll.remainingLengthFt) {
      throw new BadRequestException(
        `Only ${roll.remainingLengthFt} ft remaining, cannot cut ${dto.lengthFt} ft`,
      );
    }
    ...
    const cutSqft = this.calcSqft(roll.widthFt, roll.widthInch, dto.lengthFt);
    const newRemaining = Number(roll.remainingLengthFt) - Number(dto.lengthFt);
    const newRemainingSqft = this.calcSqft(roll.widthFt, roll.widthInch, newRemaining);
