  ...
  return (
    <div className="space-y-5">
      <SectionCard title="Udhaar / Credit" desc="Customer credit management" icon={CreditCard} color="rose">
        <Toggle checked={s.allowCredit} onChange={(v: boolean) => set('allowCredit', v)} label="Allow credit sales" desc="Customers ko udhaar de sakte hain" />
        {s.allowCredit && (
      ...
      <SectionCard title="Loyalty Program" desc="Points-based rewards" icon={Award} color="rose">
        <Toggle checked={s.enableLoyalty} onChange={(v: boolean) => set('enableLoyalty', v)} label="Enable loyalty points" desc="Customers ko har sale par points dein" />
        {s.enableLoyalty && (
          <div className="grid sm:grid-cols-2 gap-4 mt-4">
      ...
      <SectionCard title="Auto-Create" desc="Customer creation rules" icon={Users} color="rose">
        <Toggle checked={s.autoCreateCustomer} onChange={(v: boolean) => set('autoCreateCustomer', v)} label="Auto-create customers" desc="Phone number se naya customer automatically banaayen" />
      </SectionCard>
    </div>
  );
}
