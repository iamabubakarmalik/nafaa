...
const paymentMethodConfig: Record<PaymentMethod, { label: string; icon: any; color: string; activeBg: string }> = {
  ...
  CARD:          { label: 'Card',      icon: CreditCard, color: '#2563eb', activeBg: 'bg-blue-50 border-blue-500 ring-2 ring-blue-200' },
  JAZZCASH:      { label: 'JazzCash',  icon: Smartphone, color: '#f97316', activeBg: 'bg-orange-50 border-orange-500 ring-2 ring-orange-200' },
  EASYPAISA:     { label: 'EasyPaisa', icon: Zap,        color: '#22c55e', activeBg: 'bg-green-50 border-green-500 ring-2 ring-green-200' },
  BANK_TRANSFER: { label: 'Bank',      icon: Building2,  color: '#7c3aed', activeBg: 'bg-violet-50 border-violet-500 ring-2 ring-violet-200' },
};
...
export function PosCheckoutPanel({
  ...
  const totalLineDiscount = useMemo(() => cart.reduce((sum, item) => sum + (item.lineDiscount || 0), 0), [cart]);
  ...
  return (
    <div className="shrink-0 border-t-2 border-slate-200 bg-slate-50 p-3 space-y-2.5">
      {showServiceCharges && (
        <ServiceChargesPanel charges={serviceCharges} onChange={setServiceCharges} />
      )}

      <div>
        <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 mb-1 flex items-center gap-1">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          <button
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition ${
              saleMode === 'FULL_PAYMENT' ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-200 shadow-sm' : 'bg-white border-slate-200 hover:border-emerald-300'
            }`}
          >
            <Banknote className={`h-4 w-4 ${saleMode === 'FULL_PAYMENT' ? 'text-emerald-600' : 'text-slate-400'}`} />
            <span className={`text-[10px] font-extrabold ${saleMode === 'FULL_PAYMENT' ? 'text-emerald-700' : 'text-slate-500'}`}>
              Full Cash
            </span>
          </button>
          <button
            ...
            disabled={!customerId}
            className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition ${
              saleMode === 'PARTIAL_CREDIT' ? 'border-amber-500 bg-amber-50 ring-2 ring-amber-200 shadow-sm'
              : !customerId ? 'bg-slate-50 border-slate-200 opacity-50 cursor-not-allowed'
              : 'bg-white border-slate-200 hover:border-amber-300'
            }`}
          >
            <HandCoins className={`h-4 w-4 ${saleMode === 'PARTIAL_CREDIT' ? 'text-amber-600' : 'text-slate-400'}`} />
            <span className={`text-[10px] font-extrabold ${saleMode === 'PARTIAL_CREDIT' ? 'text-amber-700' : 'text-slate-500'}`}>
              Partial
            </span>
          </button>
          <button
            ...
            disabled={!customerId}
            className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition ${
              saleMode === 'FULL_CREDIT' ? 'border-rose-500 bg-rose-50 ring-2 ring-rose-200 shadow-sm'
              : !customerId ? 'bg-slate-50 border-slate-200 opacity-50 cursor-not-allowed'
              : 'bg-white border-slate-200 hover:border-rose-300'
            }`}
          >
            <BookOpen className={`h-4 w-4 ${saleMode === 'FULL_CREDIT' ? 'text-rose-600' : 'text-slate-400'}`} />
            <span className={`text-[10px] font-extrabold ${saleMode === 'FULL_CREDIT' ? 'text-rose-700' : 'text-slate-500'}`}>
              Full Udhaar
            </span>
          </button>
        </div>
      ...
      {saleMode !== 'FULL_CREDIT' && (
        <div>
          <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 mb-1 block">Payment Method</label>
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-0.5 p-2 rounded-xl border-2 transition ${
                    active ? cfg.activeBg : 'bg-white border-slate-200 hover:border-slate-300'
                  }`}
                >
                  <Icon className="h-3.5 w-3.5" style={{ color: active ? cfg.color : '#94a3b8' }} />
                  <span className={`text-[10px] font-extrabold ${active ? 'text-slate-900' : 'text-slate-500'}`}>
                    {cfg.label}
                  </span>
                </button>
              );
      ...
      {saleMode === 'PARTIAL_CREDIT' && (
        <div>
          <label className="text-[10px] font-extrabold uppercase tracking-wider text-amber-700 mb-1 block">Paid Amount</label>
          <div className="relative">
            <Wallet className="h-4 w-4 text-amber-500 absolute left-2.5 top-1/2 -translate-y-1/2" />
            <input
              ...
              placeholder="0"
              max={total - 0.01}
              className="h-12 w-full rounded-xl border-2 border-amber-300 bg-amber-50 pl-9 pr-3 text-lg font-extrabold focus:outline-none focus:ring-2 focus:ring-amber-500/40 transition"
            />
          </div>
            ...
            <div className="grid grid-cols-4 gap-1 mt-1.5">
              {quickAmounts.map((amt, idx) => (
                <button
                  ...
                  className="py-1.5 rounded-md bg-amber-100 hover:bg-amber-200 text-xs font-extrabold text-amber-800 transition"
                >
                  {formatPKR(amt)}
                </button>
              ))}
      ...
      <div className={`rounded-2xl p-3.5 space-y-1.5 text-white shadow-xl ${
        saleMode === 'FULL_CREDIT' ? 'bg-gradient-to-br from-rose-700 to-rose-900'
        : saleMode === 'PARTIAL_CREDIT' ? 'bg-gradient-to-br from-amber-700 to-orange-900'
        ...
        {serviceChargesTotal > 0 && (
          <div className="flex items-center justify-between text-xs">
            <span className="text-orange-300 font-semibold">+ Service</span>
        ...
        {saleMode !== 'FULL_CREDIT' && (
          <div className="flex items-center justify-between text-xs pt-1.5 border-t border-white/10">
            <span className="text-emerald-300 font-semibold flex items-center gap-1">
              <ArrowDownCircle className="h-3 w-3" /> Paid
            </span>
        ...
        {credit > 0 && (
          <div className="flex items-center justify-between pt-2 border-t border-white/20 bg-white/10 -mx-3.5 -mb-3.5 px-3.5 py-2.5 rounded-b-2xl">
      ...
      <Button
        className={`w-full shadow-lg ${
          saleMode === 'FULL_CREDIT' ? 'bg-gradient-to-r from-rose-600 to-rose-700'
          : saleMode === 'PARTIAL_CREDIT' ? 'bg-gradient-to-r from-amber-600 to-orange-600'
      ...
      >
        ...
        {saleMode === 'FULL_CREDIT' ? `Add to Khata • ${formatPKR(total)}`
        : saleMode === 'PARTIAL_CREDIT' ? `Confirm • ${formatPKR(credit)} udhaar`
