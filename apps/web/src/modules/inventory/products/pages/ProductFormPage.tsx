...
import { productsApi, type CreateProductPayload } from '@modules/inventory/products/api/products.api';
...
import {
  ...
  type ProductVariant,
} from '@modules/inventory/products/api/product-variants.api';
import { Button } from '@core/ui/Button';
import { Input } from '@core/ui/Input';
import { UploadDropzone, ImageGallery } from '@core/components/uploads';
...
import {
  SafeIndustrySection as IndustrySection,
  SafeIndustryVariantExtra as IndustryVariantExtra,
  useActiveIndustryPlugin,
} from '@industries/_shared/components/SafeIndustrySection';
...
type Tab = 'basic' | 'pricing' | 'inventory' | 'images' | 'variants' | 'tags';
...
export default function ProductFormPage() {
  ...
  const { data: categories = [] } = useQuery({ queryKey: ['categories'], queryFn: categoriesApi.list, placeholderData: [] });
  const { data: brands = [] } = useQuery({ queryKey: ['brands'], queryFn: () => brandsApi.list(), placeholderData: [] });
  const { data: allTags = [] } = useQuery({ queryKey: ['tags'], queryFn: tagsApi.list, placeholderData: [] });
  ...
  const saveVariantMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['product', id] });
      queryClient.invalidateQueries({ queryKey: ['products'] });
      toast.success('Variant updated ✓', { duration: 1800 });
    },
  ...
  const removeVariantMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['product', id] });
      queryClient.invalidateQueries({ queryKey: ['products'] });
      toast.success('Variant deleted ✓', { duration: 1800 });
    },
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['products'] });
      queryClient.invalidateQueries({ queryKey: ['product-images', saved.id] });
      queryClient.invalidateQueries({ queryKey: ['product-variants', saved.id] });
      queryClient.invalidateQueries({ queryKey: ['carpet-product-summary'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard-overview'] });
      setIsDirty(false);
      toast.success(isEdit ? 'Product updated ✓' : 'Product created ✓', {
        description: isEdit ? 'Changes saved successfully' : 'Now add images and variants',
        duration: 2500,
      });
      if (!isEdit) {
          ...
          <div className="relative flex items-center justify-between gap-4 flex-wrap">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex items-center gap-2 flex-wrap text-sm text-white/80">
                ...
                {(() => {
                  const brand = form.brandId && Array.isArray(brands) ? brands.find((b) => b?.id === form.brandId) : null;
                ...
                {(() => {
                  const cat = form.categoryId && Array.isArray(categories) ? categories.find((c: any) => c?.id === form.categoryId) : null;
                  if (!cat) return null;
                  return (
                    <>
                      <span className="text-white/40">•</span>
            ...
            {tab === 'basic' && (
              <div className="space-y-5 max-w-4xl">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <UnitSelect value={form.unit ?? 'pcs'} onChange={(unit) => updateForm({ unit })} label="Unit" hint="kg for grocery, pcs for retail" />
                </div>
