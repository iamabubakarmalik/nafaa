...
import {
  ...
  Star, ChevronRight, Flower2, Receipt, ShoppingCart, Hash,
  Tag, Trash2, Calendar, AlertTriangle, History, Sparkles,
  BarChart3, Info, Plus, Heart, Leaf, Palette, Globe, Wand2,
  CheckCircle2, XCircle, Clock, Ribbon, Gift, Ruler,
} from 'lucide-react';
import {
...
export default function FloristProductDetailPage() {
  const { id } = useParams();
  ...
  const stats = useMemo(() => {
    ...
    const freshDays = profile?.freshUntil
      ? Math.ceil((new Date(profile.freshUntil).getTime() - Date.now()) / 86400000)
      : null;
    ...
    return {
      ...
      sold30, perDay, daysLeft, freshDays,
    };
  ...
  const fm = profile?.freshnessGrade ? FRESHNESS_META[profile.freshnessGrade] : null;

  const TABS: { id: Tab; label: string; count?: number; icon: any }[] = [
    { id: 'overview', label: 'Overview', icon: Info },
    { id: 'details', label: 'Flower Details', icon: Sparkles },
    { id: 'occasions', label: 'Occasions & Care', icon: Heart },
  ...
  return (
    <div className="space-y-5 pb-10">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <button onClick={() => navigate('/florist-products')} className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-pink-600 font-bold">
          <ArrowLeft className="h-4 w-4" /> All Products
        </button>
        <div className="flex items-center gap-2 flex-wrap">
          <Link to={`/florist-products/${id}/edit`} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-pink-50 border-2 border-pink-200 hover:bg-pink-100 text-pink-700 text-sm font-extrabold">
        ...
        <div className="relative grid lg:grid-cols-[280px_1fr] gap-6 p-6">
          <div className="space-y-2">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-white/10 backdrop-blur border-2 border-white/20">
              {gallery[imgIndex]?.url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-white/50"><Flower2 className="h-16 w-16" /></div>
              )}
              {profile?.isFeatured && (
              ...
              {profile?.isCustomizable && (
                <div className="absolute bottom-3 left-3 px-2 py-1 rounded-lg bg-violet-600 text-white text-[10px] font-extrabold shadow-lg inline-flex items-center gap-1">
                  <Wand2 className="h-3 w-3" /> CUSTOMIZABLE
                </div>
              )}
              {!product.isActive && (
          ...
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Flower2 className="h-3.5 w-3.5 text-amber-300" /> Florist
              {profile?.categoryType && (<><span className="text-white/40">•</span><span>{profile.categoryType.replace(/_/g, ' ')}</span></>)}
              {profile?.flowerType && (<><span className="text-white/40">•</span><span>{profile.flowerType}</span></>)}
            </div>
            ...
            <div className="mt-3 flex items-center gap-2 flex-wrap text-xs">
              ...
              {profile?.stemCount && <Chip icon={Ruler}>{profile.stemCount} stems</Chip>}
              {profile?.bouquetSize && <Chip icon={Package}>{profile.bouquetSize}</Chip>}
              {profile?.isImported && <Chip icon={Globe} tone="amber">Imported{profile.origin ? ` • ${profile.origin}` : ''}</Chip>}
            ...
            <div className="mt-5 flex items-end gap-5 flex-wrap">
              ...
              {profile?.weddingPrice ? (
                <div>
                  <div className="text-[10px] uppercase font-extrabold text-white/70 tracking-wider">Wedding Rate</div>
                  <div className="text-xl font-extrabold tabular-nums text-amber-300 leading-none mt-1">{formatPKRFull(profile.weddingPrice)}</div>
                </div>
              ) : null}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
              <HeroStat icon={Package} label="Stock" value={String(stats.stock)} sub={product.unit || 'pcs'}
              ...
              <HeroStat icon={Leaf} label="Freshness"
                value={stats.freshDays === null ? '—' : stats.freshDays <= 0 ? 'Withered' : `${stats.freshDays}d`}
                sub={fm?.l || 'not graded'}
                tone={stats.freshDays !== null && stats.freshDays <= 2 ? 'rose' : 'emerald'} />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ALERTS */}
      {stats.freshDays !== null && stats.freshDays <= 3 && (
        <section className={`rounded-3xl border-2 p-4 flex items-start gap-3 ${
          stats.freshDays <= 0 ? 'bg-rose-50 border-rose-300 text-rose-900' : 'bg-amber-50 border-amber-300 text-amber-900'}`}>
          <div className={`h-11 w-11 rounded-xl text-white flex items-center justify-center shadow-md ${stats.freshDays <= 0 ? 'bg-rose-600' : 'bg-amber-600'}`}>
            <Leaf className="h-5 w-5" />
          </div>
          <div className="flex-1">
            <h3 className="font-extrabold text-sm">
              {stats.freshDays <= 0
                ? `Withered ${Math.abs(stats.freshDays)} days ago — remove from display`
                : `Only ${stats.freshDays} days of freshness left`}
            </h3>
            <p className="text-xs font-semibold opacity-90 mt-0.5">
              {profile?.freshUntil && `Fresh until ${new Date(profile.freshUntil).toLocaleDateString('en-PK', { day: 'numeric', month: 'long' })}`}
              {stats.freshDays > 0 && ' — consider a clearance discount to move stock.'}
            </p>
          </div>
        </section>
      )}
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`px-4 py-2.5 rounded-xl text-sm font-extrabold inline-flex items-center gap-2 transition ${
                  active ? 'bg-gradient-to-br from-pink-500 to-rose-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>
      ...
      {tab === 'overview' && (
        <div className="space-y-5">
              ...
              <div className="h-[240px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    <defs>
                      <linearGradient id="floristDetailGrad" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#ec4899" strokeWidth={2.5} fill="url(#floristDetailGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
          ...
          <section className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <QuickLink to="/florist/orders" icon={Package} title="Orders" desc="All orders" tone="pink" />
            <QuickLink to="/florist/deliveries" icon={Clock} title="Deliveries" desc="Today" tone="amber" />
            <QuickLink to="/florist/weddings" icon={Heart} title="Weddings" desc="Contracts" tone="rose" />
            <QuickLink to="/florist/freshness" icon={Leaf} title="Freshness" desc="Alerts" tone="emerald" />
          </section>
        </div>
      )}
      ...
      {/* DETAILS */}
      {tab === 'details' && (
        <Panel icon={Sparkles} title="Flower Details" desc="Type, colour, freshness and presentation" tone="pink"
          empty={!profile} emptyText="No detail info yet"
          emptyAction={<Link to={`/florist-products/${id}/edit`}><Button className="bg-pink-600"><Edit3 className="h-4 w-4" /> Add Details</Button></Link>}>
          {profile && (
            <div className="p-5 space-y-5">
              {(profile.flowerType || profile.color || profile.stemLengthCm) && (
                <SpecSection icon={Flower2} title="Flower" tone="pink">
                  {profile.flowerType && <SpecRow label="Flower Type" value={profile.flowerType} />}
                  {profile.color && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-600 font-semibold">Colour</span>
                      <span className="font-extrabold text-slate-900 inline-flex items-center gap-1.5">
                        <span className="h-4 w-4 rounded-full border border-slate-300" style={{ backgroundColor: profile.colorHex || '#ec4899' }} />
                        {profile.color}
                      </span>
                    </div>
                  )}
                  {profile.stemLengthCm && <SpecRow label="Stem Length" value={`${profile.stemLengthCm} cm`} />}
                  {profile.stemCount && <SpecRow label="Stem Count" value={String(profile.stemCount)} />}
                  {profile.bouquetSize && <SpecRow label="Bouquet Size" value={profile.bouquetSize} />}
                </SpecSection>
              )}

              <SpecSection icon={Leaf} title="Freshness" tone="emerald">
                {fm && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-600 font-semibold">Grade</span>
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-extrabold ${fm.cls}`}>{fm.l}</span>
                  </div>
                )}
                {profile.arrivalDate && <SpecRow label="Arrival Date" value={new Date(profile.arrivalDate).toLocaleDateString('en-PK')} />}
                {profile.freshUntil && <SpecRow label="Fresh Until" value={new Date(profile.freshUntil).toLocaleDateString('en-PK')} />}
                {profile.daysToWither && <SpecRow label="Shelf Life" value={`${profile.daysToWither} days`} />}
              </SpecSection>

              {(profile.isImported || profile.origin || profile.season?.length > 0) && (
                <SpecSection icon={Globe} title="Origin & Season" tone="blue">
                  <SpecRow label="Imported" value={profile.isImported ? '✓ Yes' : '✗ No'} />
                  {profile.origin && <SpecRow label="Origin" value={profile.origin} />}
                  {profile.season?.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase font-extrabold text-slate-500 mb-1.5">Seasons</div>
                      <div className="flex flex-wrap gap-1.5">
                        {profile.season.map((s) => (
                          <span key={s} className="px-2.5 py-1 rounded-lg bg-blue-100 text-blue-800 text-xs font-extrabold">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {(profile.wrapType || profile.ribbonColor || profile.hasVase || profile.isPreArranged) && (
                <SpecSection icon={Ribbon} title="Presentation" tone="rose">
                  <SpecRow label="Pre-arranged" value={profile.isPreArranged ? '✓ Yes' : '✗ Custom made'} />
                  {profile.wrapType && <SpecRow label="Wrap Type" value={profile.wrapType} />}
                  {profile.ribbonColor && <SpecRow label="Ribbon" value={profile.ribbonColor} />}
                  <SpecRow label="Includes Vase" value={profile.hasVase ? '✓ Yes' : '✗ No'} />
                </SpecSection>
              )}
            </div>
          )}
      ...
      {/* OCCASIONS */}
      {tab === 'occasions' && (
        <Panel icon={Heart} title="Occasions & Care" desc="When to gift and how to keep fresh" tone="rose"
          empty={!profile} emptyText="No occasion info yet">
          {profile && (
            <div className="p-5 space-y-5">
              {profile.occasions?.length > 0 && (
                <SpecSection icon={Heart} title={`Occasions (${profile.occasions.length})`} tone="pink">
                  <div className="flex flex-wrap gap-2">
                    {profile.occasions.map((o) => (
                      <div key={o} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-pink-50 border-2 border-pink-200 text-sm font-extrabold text-pink-900">
              ...
              {profile.meaning && (
                <SpecSection icon={Sparkles} title="Symbolism & Meaning" tone="violet">
                  <p className="text-sm font-semibold text-slate-700 leading-relaxed">{profile.meaning}</p>
                </SpecSection>
              )}

              {profile.careInstructions && (
                <SpecSection icon={Leaf} title="Care Instructions" tone="emerald">
                  <p className="text-sm font-semibold text-slate-700 leading-relaxed whitespace-pre-line">{profile.careInstructions}</p>
                </SpecSection>
              )}

              {profile.isCustomizable && (
                <SpecSection icon={Wand2} title="Customization" tone="violet">
                  <SpecRow label="Customizable" value="✓ Yes" />
                  {profile.minLeadTimeHours && <SpecRow label="Min Lead Time" value={`${profile.minLeadTimeHours} hours`} />}
                  {profile.customizationOptions?.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase font-extrabold text-slate-500 mb-1.5">Options</div>
                      <div className="flex flex-wrap gap-1.5">
                        {profile.customizationOptions.map((c) => (
                          <span key={c} className="px-2.5 py-1 rounded-lg bg-violet-100 text-violet-800 text-xs font-extrabold">{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
      ...
      {/* VARIANTS */}
      {tab === 'variants' && (
        <Panel icon={Boxes} title="Variants" desc={`${stats.variantCount} variants • total ${stats.variantStock} units`} tone="fuchsia"
          empty={(variants as any[]).length === 0}
          emptyText="No variants — single SKU product"
          emptyAction={<Link to={`/florist-products/${id}/edit`}><Button variant="secondary"><Plus className="h-4 w-4" /> Add Variants</Button></Link>}>
...
function SpecSection({ icon: Icon, title, tone, children }: any) {
  const tones: Record<string, string> = {
    pink: 'from-pink-500 to-rose-600 bg-pink-50 border-pink-200',
...
function QuickLink({ to, icon: Icon, title, desc, tone }: any) {
  const tones: Record<string, string> = {
    pink: 'bg-pink-100 text-pink-700 group-hover:bg-pink-600',
  ...
  return (
    <Link to={to} className="rounded-2xl bg-white border-2 border-slate-200 hover:border-pink-400 hover:shadow-md p-4 flex items-center gap-3 transition group">
