...
import type {
  FloristWizardBasic, FloristWizardVariant, FloristWizardStock,
} from '../../hooks/useFloristWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<FloristWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<FloristWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<FloristWizardStock>) => void;
  errors: string[];
}

const QUICK_STOCK = [1, 5, 10, 20, 50, 100];

const VARIANT_PRESETS = [
  { group: 'Size', items: ['Mini', 'Small', 'Medium', 'Large', 'Grand', 'Deluxe'] },
  { group: 'Color', items: ['Red', 'Pink', 'White', 'Yellow', 'Purple', 'Mixed'] },
  { group: 'Stem Count', items: ['6 stems', '12 stems', '24 stems', '50 stems', '100 stems'] },
  { group: 'Wrap', items: ['Kraft', 'Cellophane', 'Silk', 'Basket', 'Vase'] },
];

export function FloristWizardStep4Stock({
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-white border-2 border-pink-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-pink-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> one quantity for the product.
            <strong> Variants:</strong> Small / Medium / Large or Red / Pink / White each with own stock.
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Variants (sizes, colors, stem counts)</div>
            <div className="text-xs text-slate-600 font-semibold">Small vs Large, 12 vs 24 stems, Red vs Pink</div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={stock.rackNumber} onChange={(e) => onUpdateStock({ rackNumber: e.target.value })}
                  placeholder="Cooler-A, Display-2"
        ...
        <section className="rounded-2xl border-2 border-fuchsia-300 bg-gradient-to-br from-fuchsia-50 to-white p-5 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-fuchsia-700 font-semibold">Sizes, colors, stem counts</p>
            </div>
          ...
          <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end">
            <Input label="Custom variant name" value={vName}
              ...
              placeholder="e.g. Large Red Rose Bouquet" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="text-3xl font-extrabold tabular-nums leading-none mt-1">
                {displayStock} <span className="text-sm text-white/60">{basic.unit}</span>
              </div>
            </div>
            <div>
