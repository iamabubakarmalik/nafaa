import { Check, Flower2, Sparkles, Heart, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/useFloristWizard';
...
const STEPS = [
  { id: 1, label: 'Basic Info', short: 'Info', desc: 'Name, category, price', icon: Flower2 },
  { id: 2, label: 'Flower Details', short: 'Details', desc: 'Color, freshness, wrap', icon: Sparkles },
  { id: 3, label: 'Occasions & Care', short: 'Care', desc: 'Occasions, meaning', icon: Heart },
  { id: 4, label: 'Stock & Variants', short: 'Stock', desc: 'Quantity, sizes', icon: Package },
] as const;

export function FloristWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-1.5 flex-1">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-pink-500 to-rose-600 text-white shadow-md shadow-pink-500/40 scale-[1.02]'
                    : isPast
