import { Check, Home, Sparkles, Shield, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/useApplianceWizard';
...
const STEPS = [
  { id: 1, label: 'Basic Info', shortLbl: 'Info', desc: 'Naam, brand, price', icon: Home },
  { id: 2, label: 'Specifications', shortLbl: 'Specs', desc: 'Capacity, features', icon: Sparkles },
  { id: 3, label: 'Warranty & Install', shortLbl: 'Warranty', desc: 'Warranty, installation', icon: Shield },
  { id: 4, label: 'Stock & Serials', shortLbl: 'Stock', desc: 'Variants, serials', icon: Package },
] as const;

export function ApplianceWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-1.5 flex-1">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-cyan-600 to-teal-700 text-white shadow-md shadow-cyan-500/40 scale-[1.02]'
                    : isPast
