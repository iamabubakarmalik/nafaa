...
import {
  ...
  Filter, SortAsc, Ruler, Calculator, Grid3x3, List,
} from 'lucide-react';
...
type UnitMode = 'sqft' | 'sqm' | 'sqyd' | 'ft' | 'meter';

const UNIT_CONFIG: Record<UnitMode, { label: string; short: string; conv: number; isArea: boolean }> = {
  sqft:  { label: 'Square Feet',   short: 'sqft', conv: 1,       isArea: true  },
  sqm:   { label: 'Square Meter',  short: 'sqm',  conv: 0.0929,  isArea: true  },
  sqyd:  { label: 'Square Yard',   short: 'sqyd', conv: 0.1111,  isArea: true  },
  ft:    { label: 'Running Feet',  short: 'ft',   conv: 1,       isArea: false },
  meter: { label: 'Meter',         short: 'm',    conv: 0.3048,  isArea: false },
};
...
export function CarpetRollPicker({ product, variant, preSelectedRoll, onConfirm, onClose }: Props) {
  ...
  const [unit, setUnit] = useState<UnitMode>('sqft');
    ...
    let result = q
      ? rolls.filter((r) => {
          const hay = [
            ...
            (r as any).quality, (r as any).pile,
  ...
  const calc = useMemo(() => {
    ...
    // Unit conversion
    const displaySqft = unit === 'sqft' ? cutSqft
      : unit === 'sqm' ? cutSqft * 0.0929
      : unit === 'sqyd' ? cutSqft * 0.1111
      : cutSqft;
    const displayUnitLabel = UNIT_CONFIG[unit].short;
    ...
    return {
      ...
      displaySqft, displayUnitLabel,
      isValid: !widthError && !lengthError && !rateError && customerWidth > 0 && cutLength > 0 && effectiveRate > 0,
    };
  }, [selectedRoll, customerWidthFt, customerWidthInch, lengthFt, lengthInch, customRate, useWholesale, unit]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="w-full sm:max-w-7xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[95dvh] sm:max-h-[92vh] flex flex-col min-h-0">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 text-white shrink-0">
          ...
          <div className="relative px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
              {showMobileConfig && (
                <button onClick={() => setShowMobileConfig(false)}
                  className="lg:hidden h-10 w-10 rounded-xl bg-white/15 backdrop-blur flex items-center justify-center shrink-0 active:scale-95 transition">
                  <ChevronLeft className="h-5 w-5 text-white" />
                </button>
              )}
              <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-white/15 backdrop-blur flex items-center justify-center shadow-lg ring-2 ring-white/20 shrink-0">
                <Layers className="h-6 w-6 sm:h-7 sm:w-7 text-white" />
              </div>
              <div className="min-w-0">
                <div className="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wider mb-1 border border-white/20">
                  <Scissors className="h-2.5 w-2.5 text-amber-300" />
                  Cut from Roll
                </div>
                <h3 className="font-extrabold text-lg sm:text-xl leading-tight truncate">
                  {product.name}
                  {variant && <span className="ml-2 text-amber-300 text-sm sm:text-base font-bold">— {variant.name}</span>}
                </h3>
                <p className="text-xs sm:text-sm text-white/85 font-semibold mt-0.5 flex items-center gap-2 flex-wrap">
                  <Package className="h-3 w-3" />
                  {rolls.length} rolls
                  <span className="text-white/40">•</span>
                  <span className="text-emerald-300 font-extrabold">{totalSqft.toFixed(0)} sqft</span>
                  <span className="text-white/40">•</span>
                  <span className="text-amber-300">{filtered.length} shown</span>
                </p>
              </div>
            </div>
            <button onClick={onClose}
              className="h-10 w-10 rounded-xl bg-white/15 hover:bg-white/25 backdrop-blur border border-white/20 flex items-center justify-center transition shrink-0 active:scale-95">
        ...
        <div className="flex-1 overflow-hidden min-h-0 flex flex-col lg:flex-row">
          <div className="contents">
            ...
            <div className={`flex flex-col min-h-0 border-r-2 border-slate-200 lg:flex-1 lg:min-w-0 ${showMobileConfig ? 'hidden lg:flex' : 'flex flex-1'}`}>
              <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-b-2 border-slate-100 bg-white space-y-2 shrink-0">
                <div className="relative">
                  <Search className="h-5 w-5 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                  <input autoFocus value={search} onChange={(e) => setSearch(e.target.value)}
                    placeholder="Search: roll #, design, rack, quality, size..."
                  ...
                  {search && (
                    <button onClick={() => setSearch('')}
                      className="absolute right-2 top-1/2 -translate-y-1/2 h-7 w-7 rounded-md hover:bg-slate-100 flex items-center justify-center active:scale-95">
                    ...
                    {[
                      ...
                      return (
                        <button key={opt.v} onClick={() => setSortBy(opt.v)}
                          className={`shrink-0 px-2.5 py-1.5 rounded-lg text-[11px] font-extrabold inline-flex items-center gap-1 transition active:scale-95 ${
              ...
              <div ref={listRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-2 sm:p-3 bg-gradient-to-b from-slate-50/30 to-white min-h-0">
                {isLoading ? (
                    ...
                    {visible.map((roll) => {
                      ...
                      return (
                        <button key={roll.id}
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <div className="font-mono font-extrabold text-sm text-slate-900">{roll.rollNumber}</div>
                                {isSelected && (
                                ...
                                {isLowRoll && (
                                  <span className="px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-700 text-[9px] font-extrabold uppercase animate-pulse">Low</span>
                                )}
                              </div>
                              {(roll as any).variant && (
                                <div className="text-xs font-bold text-violet-700 mt-0.5 inline-flex items-center gap-1">
                                  {(roll as any).variant.colorHex && (
                                    <span className="h-2.5 w-2.5 rounded-full border border-white shadow-sm" style={{ backgroundColor: (roll as any).variant.colorHex }} />
                                  )}
                                  {(roll as any).variant.name}
                                </div>
                              )}
                              {(roll as any).designCode && (
                                <div className="text-[10px] font-mono text-slate-500 mt-0.5">{(roll as any).designCode}</div>
                              )}
                            </div>
                          ...
                          <div className="mt-2 flex items-center gap-1.5 text-[11px] font-bold text-slate-600 flex-wrap">
                          ...
                          <div className="mt-1.5 flex items-center justify-between gap-2">
                            ...
                            <span className="text-emerald-700 font-extrabold tabular-nums text-xs shrink-0">
                  ...
                  <div className="divide-y divide-slate-200 rounded-xl border-2 border-slate-200 bg-white overflow-hidden">
                    {visible.map((roll) => {
                      const isSelected = selectedRoll?.id === roll.id;
                      return (
                        <button key={roll.id}
                          ...
                          <div className="font-mono font-extrabold text-sm text-slate-900 min-w-[90px]">{roll.rollNumber}</div>
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-bold text-slate-700 truncate">
                              {(roll as any).variant?.name || product.name}
                              {(roll as any).designCode && <span className="text-slate-500 ml-1">({(roll as any).designCode})</span>}
                            </div>
                            <div className="text-[10px] text-slate-500 font-bold">
                              {Number(roll.widthFt)}ft × {Number(roll.remainingLengthFt)}ft
                              {(roll as any).rackNumber && ` • ${(roll as any).rackNumber}`}
                            </div>
                          </div>
                          <div className="text-right shrink-0">
                            <div className="text-base font-extrabold text-emerald-700 tabular-nums">{Number(roll.remainingSqft).toFixed(0)} sqft</div>
                            <div className="text-[10px] text-emerald-700 font-bold">{formatPKR(Number((roll as any).salePricePerSqft || 0))}/sqft</div>
                          </div>
                ...
                {visibleCount < filtered.length && (
                  <div className="mt-3 text-center">
                    <button onClick={() => setVisibleCount((c) => c + 80)}
                      ...
                      Show {Math.min(80, filtered.length - visibleCount)} more ({visibleCount}/{filtered.length})
                    </button>
                  </div>
                )}
              </div>
            ...
            {/* ═══ RIGHT — CUT CONFIG ═══ */}
            <div className={`flex flex-col min-h-0 bg-slate-50/30 lg:w-[480px] lg:shrink-0 h-full lg:h-auto ${!showMobileConfig ? 'hidden lg:flex' : 'flex flex-1'}`}>
              {!selectedRoll ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-6 py-12">
                  ...
                  <div className="text-sm text-slate-500 mt-1">Left se ek roll chuno cut karne ke liye</div>
                </div>
              ) : (
                <div className="flex-1 overflow-y-auto overscroll-contain p-3 sm:p-4 space-y-3 min-h-0" style={{ WebkitOverflowScrolling: "touch" }}>

                  {/* Selected Roll Card */}
                  <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-700 text-white p-3 sm:p-4 shadow-lg">
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      <div className="text-xs uppercase tracking-wider font-extrabold opacity-90">Selected Roll</div>
                      <Award className="h-4 w-4 text-amber-300" />
                    </div>
                    <div className="font-mono font-extrabold text-xl sm:text-2xl leading-none">{selectedRoll.rollNumber}</div>
                    <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <div className="text-[10px] uppercase font-extrabold opacity-75">Roll Width</div>
                      ...
                      <div>
                        <div className="text-[10px] uppercase font-extrabold opacity-75">Available</div>
                        <div className="font-extrabold tabular-nums text-base sm:text-lg">
                  ...
                  {/* Unit Switcher */}
                  <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-2.5">
                    <div className="text-[10px] uppercase font-extrabold text-blue-700 mb-1.5 flex items-center gap-1">
                      <Calculator className="h-3 w-3" />
                      Display Unit
                    </div>
                    <div className="grid grid-cols-3 gap-1.5">
                      {(['sqft', 'sqm', 'sqyd'] as UnitMode[]).map((u) => (
                        <button key={u} onClick={() => setUnit(u)}
                          className={`h-9 rounded-lg text-xs font-extrabold transition active:scale-95 ${
                            unit === u ? 'bg-blue-600 text-white shadow' : 'bg-white border border-blue-200 text-blue-700'
                          }`}>
                          {u.toUpperCase()}
                        </button>
                      ))}
                  ...
                  <div className="rounded-2xl bg-white border-2 border-emerald-200 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => {
                        setCustomerWidthFt(String(Number(selectedRoll.widthFt)));
                        setCustomerWidthInch(String(Number((selectedRoll as any).widthInch || 0)));
                      }} className="text-[10px] font-extrabold text-emerald-700 inline-flex items-center gap-0.5">
                        <RotateCcw className="h-2.5 w-2.5" /> Full roll
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[10px] font-extrabold text-slate-700 mb-1 uppercase">Feet</label>
                        <input ref={widthInputRef} type="number" inputMode="decimal" step="1" min="0"
                          ...
                          className={`h-13 sm:h-14 w-full rounded-xl border-2 px-3 text-xl sm:text-2xl font-extrabold focus:outline-none focus:ring-2 transition text-center ${
                      ...
                      <div>
                        <label className="block text-[10px] font-extrabold text-slate-700 mb-1 uppercase">Inches</label>
                        <input type="number" inputMode="numeric" step="1" min="0" max="11"
                          ...
                          onFocus={(e) => e.target.select()}
                          placeholder="0"
                          className="h-13 sm:h-14 w-full rounded-xl border-2 border-emerald-200 bg-white px-3 text-xl sm:text-2xl font-extrabold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition text-center" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl bg-white border-2 border-emerald-200 p-3 space-y-2">
                    ...
                    {(calc?.widthError || calc?.lengthError) && (
                      <div className="rounded-lg bg-rose-50 border-2 border-rose-200 p-2 flex items-start gap-1.5">
                        <AlertCircle className="h-4 w-4 text-rose-600 shrink-0 mt-0.5" />
                        <p className="text-xs text-rose-900 font-bold">{calc?.widthError || calc?.lengthError}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="rounded-2xl bg-amber-50 border-2 border-amber-300 p-3 space-y-2">
                    ...
                    <div className="relative">
                      <input type="number" step="0.01"
                        ...
                        className="h-13 sm:h-14 w-full rounded-xl border-2 border-amber-300 bg-white pl-3 pr-20 sm:pr-24 text-xl sm:text-2xl font-extrabold text-amber-900 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition text-center" />
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] sm:text-xs font-extrabold text-amber-700">Rs/sqft</div>
                    </div>

                    {calc && calc.rateSource !== 'default' && (
                      <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      {calc?.wholesaleRate ? (
                        <button onClick={() => { setUseWholesale(true); setCustomRate(''); }}
                          className={`h-9 rounded-lg border-2 text-xs font-extrabold transition active:scale-95 ${
                      ...
                      {[5, 10, 15].map((p) => (
                        <button key={p} onClick={() => applyDiscount(p)}
                          className="h-9 rounded-lg border-2 border-amber-200 bg-white text-amber-700 hover:border-amber-400 text-xs font-extrabold inline-flex items-center justify-center gap-0.5 transition active:scale-95">
                          <Percent className="h-2.5 w-2.5" />{p}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {calc && calc.cutSqft > 0 && !calc.widthError && !calc.lengthError && (
                    <div className={`rounded-2xl border-2 p-3 space-y-2 ${
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <div className="text-[10px] uppercase font-extrabold text-slate-600">Cut Area</div>
                          <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 tabular-nums">{calc.displaySqft.toFixed(2)}</div>
                          <div className="text-[10px] font-extrabold text-slate-600">{calc.displayUnitLabel}</div>
                          {unit !== 'sqft' && (
                            <div className="text-[10px] text-slate-500 font-bold">= {calc.cutSqft.toFixed(2)} sqft</div>
                          )}
                        </div>
                        <div className="text-right">
                          <div className="text-[10px] uppercase font-extrabold text-slate-600">Total Price</div>
                          <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 tabular-nums">{formatPKRFull(calc.totalPrice)}</div>
                          <div className="text-[10px] font-extrabold text-slate-600">@ {formatPKR(calc.effectiveRate)}/sqft</div>
                        </div>
                      </div>
                        ...
                        <div className="pt-2 border-t-2 border-slate-300">
                          <label className="flex items-start gap-2 cursor-pointer">
                            ...
                            <div className="flex-1">
                              <div className="text-xs font-extrabold text-amber-900">Create leftover cut piece</div>
        ...
        {/* ═══ FOOTER ═══ */}
        <div className="border-t-2 border-slate-200 bg-emerald-50 px-3 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between gap-2 shrink-0">
          <button onClick={onClose}
            className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-extrabold text-slate-600 hover:bg-slate-100 transition active:scale-95">
            Cancel
          </button>

          {selectedRoll && !showMobileConfig && (
            <button onClick={() => setShowMobileConfig(true)}
              className="lg:hidden flex-1 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-extrabold inline-flex items-center justify-center gap-1.5 active:scale-95">
              Configure Cut <ArrowRight className="h-4 w-4" />
            </button>
          )}

          <Button onClick={handleConfirm} disabled={!calc?.isValid}
            className="bg-gradient-to-r from-emerald-700 to-emerald-600 shadow-lg disabled:from-slate-300 disabled:to-slate-400 disabled:shadow-none px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base">
            ...
            <span className="sm:hidden">
              {calc?.isValid ? formatPKR(calc.totalPrice) : 'Add'}
            </span>
