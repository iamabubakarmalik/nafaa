...
const UNIT_FORMULAS: Record<string, { label: string; calc: (l: number, w: number) => number; pieceLabel: string; needsWidth: boolean }> = {
  ...
  meter: { label: 'Meter',         calc: (l)    => l,     pieceLabel: 'm',    needsWidth: false },
  ft:    { label: 'Foot',          calc: (l)    => l,     pieceLabel: 'ft',   needsWidth: false },
  yard:  { label: 'Yard',          calc: (l)    => l,     pieceLabel: 'yard', needsWidth: false },
  gaj:   { label: 'Gaj',           calc: (l)    => l,     pieceLabel: 'gaj',  needsWidth: false },
};

const newId = () => Math.random().toString(36).slice(2, 8);

export function LengthWidthCalculator({ productName, unit, onApply, onClose }: Props) {
  const [pieces, setPieces] = useState<Piece[]>([{ id: newId(), length: '', width: '', quantity: '1' }]);
  ...
  const updatePiece = (id: string, field: keyof Piece, value: string) => {
    setPieces((p) => p.map((piece) => (piece.id === id ? { ...piece, [field]: value } : piece)));
  };
  ...
  const grandTotal = useMemo(() => calculations.reduce((sum, c) => sum + c.total, 0), [calculations]);
  const totalPieces = useMemo(() => calculations.reduce((sum, c) => sum + c.q, 0), [calculations]);
  ...
  const handleApply = () => {
    if (grandTotal <= 0) { toast.error('Dimensions enter karein'); return; }
      ...
      <div className="w-full sm:max-w-2xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur px-2.5 py-1 text-xs font-extrabold uppercase tracking-wider mb-1 border border-white/20">
                  <Ruler className="h-3 w-3 text-amber-300" />
                  {formula.label}
                </div>
                <h3 className="font-extrabold text-xl leading-tight truncate">{productName}</h3>
                <p className="text-sm text-white/85 font-semibold mt-1">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 bg-slate-50/30 min-h-0">
          <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-3 flex items-start gap-2">
            ...
            <div className="text-sm text-blue-900 font-semibold leading-snug">
              {isAreaBased
                ? <><strong>Tip:</strong> Length × Width per piece, phir count. e.g. <strong>12 × 12 ft × 3 pieces = 432 sqft</strong></>
                : <><strong>Tip:</strong> Length per piece, phir quantity.</>}
            </div>
          </div>
            ...
            return (
              <div key={piece.id}
                className={`rounded-2xl border-2 p-4 transition ${
                  hasValue ? 'border-emerald-300 bg-emerald-50/60 shadow-sm' : 'border-slate-200 bg-white hover:border-emerald-200'
                }`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="inline-flex items-center gap-2">
                    <div className={`h-9 w-9 rounded-lg flex items-center justify-center font-extrabold text-base ${
                      hasValue ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600'
                    }`}>
                      {i + 1}
                    </div>
                    ...
                    {hasValue && <CheckCircle2 className="h-5 w-5 text-emerald-600" />}
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => duplicatePiece(piece.id)}
                      className="h-8 w-8 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 flex items-center justify-center transition"
                      title="Duplicate">
                ...
                <div className={`grid gap-2 ${isAreaBased ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2'}`}>
                  <div>
                    <label className="block text-xs font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Length</label>
                    <div className="relative">
                      <input data-piece-input={`length-${i}`} type="number" inputMode="decimal" step="0.01"
                        value={piece.length}
                        onChange={(e) => updatePiece(piece.id, 'length', e.target.value)}
                        placeholder="0"
                        className="h-13 w-full rounded-xl border-2 border-slate-200 bg-white px-3 pr-10 text-2xl font-extrabold text-slate-900 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition"
                        style={{ height: '52px' }} />
                      <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs font-extrabold text-slate-400">
                        {formula.pieceLabel}
                      </div>
                    </div>
                  </div>
                  ...
                  {isAreaBased && (
                    <div>
                      <label className="block text-xs font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Width</label>
                      <div className="relative">
                  ...
                  <div>
                    <label className="block text-xs font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Pieces</label>
                    <input type="number" inputMode="numeric" min="1" step="1"
                      value={piece.quantity}
                      onChange={(e) => updatePiece(piece.id, 'quantity', e.target.value)}
                      placeholder="1"
                      className="h-13 w-full rounded-xl border-2 border-slate-200 bg-white px-3 text-2xl font-extrabold text-slate-900 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition"
                      style={{ height: '52px' }} />
                  </div>

                  <div>
                    <label className="block text-xs font-extrabold text-emerald-700 mb-1 uppercase tracking-wider">Sub Total</label>
                    <div className="rounded-xl bg-emerald-50 border-2 border-emerald-300 px-3 flex items-center justify-end shadow-sm"
                      style={{ height: '52px' }}>
                      <div className="text-2xl font-extrabold text-emerald-700 tabular-nums">
                        {calc.total.toFixed(2)}
                      </div>
                      <div className="text-xs font-extrabold text-emerald-600 ml-1">{unit}</div>
                    </div>
                  </div>
                </div>

                {calc.total > 0 && (
                  <div className="mt-3 pt-2.5 border-t border-emerald-200/50 text-xs font-mono text-emerald-800 font-bold">
                    {isAreaBased
          ...
          <button onClick={addPiece}
            className="w-full h-13 rounded-xl border-2 border-dashed border-emerald-300 hover:border-emerald-500 text-emerald-700 hover:bg-emerald-50 font-extrabold text-base transition inline-flex items-center justify-center gap-2 group"
            style={{ height: '52px' }}>
        ...
        <div className="border-t-2 border-emerald-200 bg-emerald-50 px-5 py-4 space-y-3 shrink-0">
          <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-700 text-white p-4 shadow-lg">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-xs uppercase tracking-wider font-extrabold opacity-90">Grand Total</div>
              ...
              <div className="text-right shrink-0">
                ...
                <div className="text-sm font-extrabold opacity-90 mt-0.5">{unit}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1 bg-gradient-to-r from-emerald-600 to-emerald-700 shadow-lg"
              onClick={handleApply} disabled={grandTotal <= 0}>
