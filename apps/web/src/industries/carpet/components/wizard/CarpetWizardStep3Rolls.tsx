...
export function CarpetWizardStep3Rolls({
  ...
  const piecesByBucket = useMemo(() => {
    const map = new Map<string | null, CarpetWizardPieceLine[]>();
    for (const b of buckets) map.set(b.tempId, []);
    for (const p of pieceLines) {
      const key = p.variantTempId;
      if (!map.has(key)) map.set(key, []);
      map.get(key)!.push(p);
    }
    return map;
  ...
  const ftByBucket = useMemo(() => {
    const map = new Map<string | null, CarpetWizardFtStock | undefined>();
    for (const b of buckets) map.set(b.tempId, undefined);
    for (const f of ftStock) map.set(f.variantTempId, f);
    return map;
  }, [ftStock, buckets]);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-white border-2 border-emerald-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-extrabold text-emerald-900 text-sm">Add Stock</h3>
          <p className="text-xs text-emerald-800 font-semibold mt-0.5 leading-relaxed">
            Har variant apni stock type ke hisaab se — rolls, pieces ya running feet. Prices
            <strong> auto-fill</strong> hain. Existing rolls <strong>🔒 locked</strong> hain (edit from /carpet-rolls).
            Naye rolls <strong>Add Roll</strong> button se add karein.
          </p>
        </div>
      </div>
          ...
          >
            ...
            >
              ...
              {b.stockType === 'ROLLS' && (
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); onAddRoll(b.tempId); setCollapsed(c => ({ ...c, [b.tempId ?? '__none__']: false })); }}
              ...
              {b.stockType === 'PIECES' && (
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); onAddPieceLine(b.tempId); setCollapsed(c => ({ ...c, [b.tempId ?? '__none__']: false })); }}
                  className="px-3 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-extrabold inline-flex items-center gap-1 shadow-sm"
                >
                  <Plus className="h-3.5 w-3.5" /> Add Piece Line
                </button>
              )}
            ...
            {!collapsedNow && (
              <>
                {b.stockType === 'ROLLS' && (
                  <RollsTable
                    lines={rollsByBucket.get(b.tempId) ?? []}
                ...
                {b.stockType === 'PIECES' && (
                  <CarpetPiecesTable
                    ...
                    lines={piecesByBucket.get(b.tempId) ?? []}
                ...
                {b.stockType === 'FT' && (
                  <CarpetFtStockCard
                    ...
                    bucketName={b.name}
                    variantTempId={b.tempId}
                    ftStock={ftByBucket.get(b.tempId)}
      ...
      {totalRolls === 0 && totalPieces === 0 && ftStock.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center">
          ...
          <div className="text-xs text-slate-500 font-semibold mt-1">
            Sirf product create karna hai to <strong>Save</strong> kar dein — stock baad mein add ho sakti hai
          </div>
        </div>
      )}
    </div>
...
function RollsTable({
  ...
  // Existing DB rolls (loaded in edit mode) have tempId "edit-roll-*"
  // and must stay read-only — user edits them from /carpet-rolls page.
    ...
    <>
          ...
          <tbody className="divide-y divide-slate-100">
            {lines.map((r) => {
              ...
              return (
                <tr
                  key={r.tempId}
                  className={locked ? 'bg-slate-50/70' : 'hover:bg-slate-50/50'}
                >
                  ...
                  <td className="px-2 py-1.5">
                    <div className="flex items-center gap-1">
                      {locked ? (
                        <span
                          className="text-[9px] font-extrabold text-slate-500 bg-slate-200 px-2 py-1 rounded-md"
                          title="Existing roll — edit from /carpet-rolls page"
                        >
                          SAVED
                        </span>
                      ) : (
                        <>
                          <button type="button" onClick={() => onDuplicate(r.tempId)} className="h-7 w-7 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 flex items-center justify-center">
                            <Copy className="h-3 w-3" />
                          </button>
                          <button type="button" onClick={() => onRemove(r.tempId)} className="h-7 w-7 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center">
      ...
      <div className="border-t border-slate-100 bg-slate-50/70 p-2 flex items-center justify-between gap-2 flex-wrap">
        <div className="text-[10px] font-extrabold text-slate-600 flex items-center gap-1 pl-1">
          ...
          Prices auto-filled from variant / product default — you can change per roll
        </div>
        <button
          type="button"
          onClick={onAdd}
