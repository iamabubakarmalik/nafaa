...
import {
  ...
  ChevronDown, Eye, EyeOff, ArrowRight, Printer, PawPrint,
  Star, Pause, Play, Percent, Wifi, WifiOff, Heart, Pill,
  AlertTriangle, Clock,
} from 'lucide-react';
...
const HIDE_PRICES_KEY = 'nafaa.petshop-pos.hide-prices';
const VIEW_MODE_KEY = 'nafaa.petshop-pos.view-mode';

type ViewMode = 'products' | 'animals';
...
export default function PetshopPosPage() {
  ...
  }, [debouncedSearch, speciesFilter, viewMode]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'F8') { e.preventDefault(); setViewMode('animals'); }
      if (e.key === 'Escape') {
  ...
  const { data: productsData, isLoading: loadingProducts } = useQuery({
    queryKey: ['products-for-petshop-pos'],
  ...
  const { data: animals = [], isLoading: loadingAnimals } = useQuery({
    queryKey: ['pos-live-animals'],
    queryFn: () => liveAnimalsApi.list({ status: 'AVAILABLE' }),
    staleTime: 30_000,
  });
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (speciesFilter) {
      list = list.filter((p) => {
        ...
        return pr?.species === speciesFilter || pr?.species === 'ALL';
      });
    }
  ...
  }, [products, debouncedSearch, speciesFilter, profileByProduct]);
  ...
  const filteredAnimals = useMemo(() => {
    ...
    return list;
  }, [animals, debouncedSearch, speciesFilter]);
  ...
  const openProduct = useCallback((product: Product) => {
    ...
    if (profile?.expiryDate) {
      const days = Math.ceil((new Date(profile.expiryDate).getTime() - Date.now()) / 86400000);
      if (days < 0) {
        toast.error(`${product.name} — EXPIRED, cannot sell`);
        return;
      }
    }
  ...
  const addProductLine = (product: Product, qty: number, profile?: any) => {
    ...
    const expiryDays = profile?.expiryDate
      ? Math.ceil((new Date(profile.expiryDate).getTime() - Date.now()) / 86400000)
      : null;
    setCart((prev) => [...prev, {
      id: lineId(),
      type: 'product',
      ...
      isPrescription: profile?.isPrescriptionOnly,
      isExpiring: expiryDays !== null && expiryDays >= 0 && expiryDays <= 90,
      note: profile?.isPrescriptionOnly ? '⚠️ Rx required' : undefined,
    }]);
    toast.success(`${product.name} added`, { duration: 900 });
  };

  const addAnimalLine = (animal: any) => {
    const existing = cart.find((l) => l.animalId === animal.id);
    if (existing) { toast.error('Animal already in cart'); return; }
    setCart((prev) => [...prev, {
      id: lineId(),
      type: 'animal',
      animalId: animal.id,
      name: animal.name || `${animal.species} #${animal.animalNumber}`,
      image: animal.imageUrls?.[0],
      species: animal.species,
      unitPrice: animal.askingPrice,
      quantity: 1,
      baseStock: 1,
      lineTotal: animal.askingPrice,
      note: `${animal.breed || animal.species} • ${animal.gender || ''} • ${animal.ageMonths ? animal.ageMonths + 'mo' : ''}`,
    }]);
    toast.success(`${animal.name || 'Animal'} added`, { duration: 1200 });
  };

  const changeQty = (id: string, delta: number) => {
    setCart((prev) => prev.flatMap((l) => {
      if (l.id !== id) return [l];
      if (l.type === 'animal' && delta > 0) { toast.error('Only 1 animal per line'); return [l]; }
  ...
  const setQtyDirect = (id: string, qty: number) => {
    setCart((prev) => prev.flatMap((l) => {
      if (l.id !== id) return [l];
      if (qty <= 0) return [];
      if (l.type === 'animal' && qty > 1) { toast.error('Only 1 animal per line'); return [l]; }
  ...
  const checkoutMutation = useMutation({
    mutationFn: async (data: { paymentMethod: PaymentMethod; paidAmount: number }) => {
      ...
      const animalLines = cart.filter((l) => l.type === 'animal' && l.animalId);

      // Sell animals first
      for (const a of animalLines) {
        try {
          await liveAnimalsApi.sell(a.animalId!, {
            soldPrice: a.unitPrice,
            soldToCustomerId: customerId || undefined,
            soldToCustomerName: selectedCustomer?.name,
          });
        } catch (e: any) {
          console.error('Animal sell failed', e);
        }
      }
    ...
    onSuccess: (sale, vars) => {
      ...
      setLastSale({
        id: sale?.id || 'animal-only',
        number: sale?.saleNumber || 'ANIMAL-SALE',
        change, total,
      });
      setShowCheckout(false);
      setShowMobileCart(false);
      clearCart();
      queryClient.invalidateQueries({ queryKey: ['products-for-petshop-pos'] });
      queryClient.invalidateQueries({ queryKey: ['pos-live-animals'] });
  ...
  const isLoading = viewMode === 'products' ? loadingProducts : loadingAnimals;
          ...
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 grid grid-cols-2 gap-2">
              ...
              <button onClick={() => setLastSale(null)}
                className={`${lastSale.id !== 'animal-only' ? '' : 'col-span-2'} h-14 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-700 font-extrabold text-white text-lg shadow-lg inline-flex items-center justify-center gap-2`}>
          ...
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              <button onClick={() => {
                ...
                className="w-full h-16 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-700 font-extrabold text-white text-xl shadow-lg disabled:opacity-50">
                Add Customer
              </button>
            </div>
          </div>
            ...
            <div className="relative px-3 sm:px-4 py-2.5 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h2 className="text-base sm:text-lg font-extrabold leading-none">🐾 Pet Shop POS</h2>
                    {isOnline ? (
                  ...
                  <p className="text-[11px] sm:text-xs text-white/80 font-semibold mt-0.5 flex items-center gap-1 truncate">
                    ...
                    <span className="truncate">{tenant?.name || 'Pet Shop'}</span>
                  </p>
                </div>
              </div>
          ...
          <div className="shrink-0 px-3 sm:px-4 pt-3 bg-slate-50 border-b-2 border-slate-100">
            <div className="flex gap-1.5 bg-white rounded-2xl border-2 border-slate-200 p-1">
              <ViewTab active={viewMode === 'products'} onClick={() => setViewMode('products')}
                icon={Package} label="Products" count={products.length} color="amber" shortcut="F7" />
              <ViewTab active={viewMode === 'animals'} onClick={() => setViewMode('animals')}
                icon={Heart} label="Live Animals" count={animals.length} color="rose" shortcut="F8" highlight={animals.length > 0} />
            </div>
          </div>
          ...
          <div className="shrink-0 px-3 sm:px-4 py-2.5 bg-slate-50 border-b-2 border-slate-100 space-y-2">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  className="h-14 sm:h-16 w-full rounded-2xl border-4 border-slate-200 bg-white pl-11 sm:pl-14 pr-10 sm:pr-12 text-lg sm:text-xl font-bold focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-200"
                  placeholder={viewMode === 'products' ? 'Product / brand / SKU...' : 'Animal name / breed / ID...'}
                  value={search}
              ...
              <button onClick={() => setScannerOpen(true)}
                className="h-14 sm:h-16 w-16 sm:w-20 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 hover:from-slate-800 hover:to-slate-800 text-white flex flex-col items-center justify-center gap-0.5 shadow-lg shrink-0">
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (barcodeInput.trim()) { handleBarcode(barcodeInput); setBarcodeInput(''); } }} className="relative">
              <ScanLine className="h-4 w-4 sm:h-5 sm:w-5 text-amber-600 absolute left-3 top-1/2 -translate-y-1/2" />
              <input ref={barcodeRef} value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)}
                placeholder="Barcode..."
                className="h-10 sm:h-12 w-full rounded-2xl border-2 border-amber-300 bg-amber-50 pl-10 sm:pl-11 pr-3 text-sm sm:text-base font-mono font-extrabold focus:outline-none focus:border-amber-600 focus:ring-2 focus:ring-amber-200" />
            </form>

            <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
              <FilterChip active={!speciesFilter} onClick={() => setSpeciesFilter('')} label="All" emoji="🌐" />
              {[
                { v: 'DOG', l: 'Dog', e: '🐕' },
                { v: 'CAT', l: 'Cat', e: '🐈' },
                { v: 'BIRD', l: 'Bird', e: '🦜' },
                ...
                <FilterChip key={s.v} active={speciesFilter === s.v}
                  onClick={() => setSpeciesFilter(speciesFilter === s.v ? '' : s.v)}
                  label={s.l} emoji={s.e} />
              ))}
            </div>
          </div>
          ...
          <div ref={scrollRef} onScroll={handleScroll} className="lg:flex-1 lg:overflow-y-auto p-2 sm:p-3 bg-slate-50/50 lg:min-h-0">
            {isLoading ? (
                ...
                <>
                  ...
                  {hasMore && (
                    <button onClick={() => setVisibleCount((c) => c + 60)}
                      className="mt-3 w-full h-12 rounded-2xl bg-white border-4 border-slate-200 hover:border-amber-400 text-slate-700 text-sm font-extrabold inline-flex items-center justify-center gap-2">
              ...
              filteredAnimals.length === 0 ? (
                <EmptyState icon={Heart} title="No animals available" hint={animals.length === 0 ? 'Add live animals to inventory' : 'Change filter'} />
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3">
                  {filteredAnimals.map((a) => (
                    <AnimalTile key={a.id} animal={a} cart={cart} hidePrices={hidePrices} onClick={() => addAnimalLine(a)} />
                  ))}
                </div>
              )
            )}
...
function ViewTab({ active, onClick, icon: Icon, label, count, color, shortcut, highlight }: any) {
  ...
  return (
    <button onClick={onClick}
      ...
      {highlight && !active && <span className="absolute top-1 right-1 h-2 w-2 rounded-full bg-rose-500 animate-pulse" />}
      <Icon className="h-4 w-4" />
      <span>{label}</span>
      {count > 0 && (
        <span className={`px-1.5 rounded-md text-[10px] ${active ? 'bg-white/25' : 'bg-slate-200 text-slate-700'}`}>{count}</span>
      )}
      <span className={`hidden lg:inline text-[9px] font-mono px-1 rounded ${active ? 'bg-white/20' : 'bg-slate-200 text-slate-500'}`}>{shortcut}</span>
    </button>
  );
}
...
function ProductTile({ product: p, profile, cart, hidePrices, onClick }: any) {
  ...
  const daysToExpiry = profile?.expiryDate
    ? Math.ceil((new Date(profile.expiryDate).getTime() - Date.now()) / 86400000)
    : null;
  ...
  return (
    <button onClick={onClick} disabled={out || isExpired}
      className={`group relative text-left rounded-2xl border-4 overflow-hidden transition-all active:scale-95 ${
        out || isExpired ? 'border-slate-200 bg-slate-100 opacity-50 cursor-not-allowed'
      ...
      <div className="aspect-square bg-slate-100 overflow-hidden relative">
        ...
        {isExpired && !out && <div className="absolute inset-0 bg-rose-900/70 flex items-center justify-center"><span className="px-3 py-1 rounded-xl bg-rose-600 text-white text-xs font-extrabold shadow-lg">EXPIRED</span></div>}
        {low && !out && !isExpired && <div className="absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded-md bg-amber-500 text-white text-[10px] font-extrabold shadow-lg animate-pulse">LOW</div>}
        {profile?.isPrescriptionOnly && !out && <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded-md bg-rose-500 text-white text-[9px] font-extrabold shadow-lg inline-flex items-center gap-0.5"><Pill className="h-2 w-2" /> Rx</div>}
        {isExpiring && !isExpired && (
          <div className="absolute bottom-1.5 left-1.5 h-6 px-1.5 rounded-md bg-amber-600 flex items-center gap-1 text-white text-[9px] font-extrabold shadow-lg">
            <Clock className="h-2.5 w-2.5" /> {daysToExpiry}d
          </div>
        )}
        {p.isFeatured && !out && !isExpired && !profile?.isPrescriptionOnly && (
      ...
      <div className="p-2 sm:p-3">
        ...
        {profile?.brand && (
          <div className="text-[9px] font-extrabold uppercase text-amber-700 mt-0.5 truncate">{profile.brand}</div>
        )}
...
function AnimalTile({ animal: a, cart, hidePrices, onClick }: any) {
  const inCart = cart.some((l: CartLine) => l.animalId === a.id);
  const img = a.imageUrls?.[0];

  return (
    <button onClick={onClick} disabled={inCart}
      className={`group relative text-left rounded-2xl border-4 overflow-hidden transition-all active:scale-95 ${
        inCart ? 'border-emerald-500 bg-emerald-50 shadow-xl ring-4 ring-emerald-200 cursor-not-allowed'
          : 'border-rose-200 bg-gradient-to-br from-white via-rose-50 to-white hover:border-rose-400 hover:shadow-xl hover:-translate-y-1'}`}>
      {inCart && (
        <div className="absolute -top-2 -right-2 h-8 w-8 rounded-full bg-emerald-600 text-white flex items-center justify-center shadow-xl ring-4 ring-white z-10">
          <CheckCircle2 className="h-4 w-4" />
        </div>
      )}
      <div className="aspect-square bg-gradient-to-br from-rose-100 to-pink-100 overflow-hidden relative">
        {img ? (
          <img src={img} alt={a.name} loading="lazy" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <Heart className="h-16 w-16 text-rose-400" />
          </div>
        )}
        <div className="absolute top-1.5 left-1.5 px-2 py-0.5 rounded-md bg-rose-600 text-white text-[9px] font-extrabold uppercase tracking-wider shadow-lg">
          Live
        </div>
        {a.isVaccinated && (
          <div className="absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded-md bg-emerald-500 text-white text-[9px] font-extrabold shadow-lg">
            💉 VAC
          </div>
        )}
        {a.isFeatured && (
          <div className="absolute bottom-1.5 left-1.5 h-6 w-6 rounded-full bg-amber-500 flex items-center justify-center shadow-lg">
      ...
      <div className="p-2 sm:p-3">
        ...
        <div className="mt-1.5 flex items-end justify-between gap-1">
          <div className="text-lg sm:text-2xl font-extrabold text-rose-700 leading-none tabular-nums">
            {hidePrices ? '•••' : formatPKR(a.askingPrice)}
          </div>
        </div>
      </div>
    </button>
...
function EmptyState({ icon: Icon, title, hint, onClear }: any) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6">
      ...
      {onClear && (
        <button onClick={onClear} className="mt-4 h-12 px-5 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white font-extrabold inline-flex items-center gap-2">
          Clear
        </button>
      )}
    </div>
  ...
  return (
    <aside className={containerClass}>
            ...
            {cart.length > 0 && (
              <>
                <button onClick={onHold}
                  className="h-10 sm:h-12 px-2.5 rounded-2xl bg-white/15 hover:bg-amber-500/50 text-white text-xs sm:text-sm font-extrabold border-2 border-white/20 inline-flex items-center gap-1">
      ...
      <div className="shrink-0 px-3 py-2.5 border-b-2 border-slate-100 bg-slate-50">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <User className="h-4 w-4 sm:h-5 sm:w-5 text-amber-600 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <select value={customerId} onChange={(e) => setCustomerId(e.target.value)}
              className="h-12 sm:h-14 w-full rounded-2xl border-4 border-slate-200 bg-white pl-10 sm:pl-11 pr-9 text-sm sm:text-base font-bold focus:outline-none focus:border-amber-500 appearance-none">
          ...
          <button onClick={onAddCustomer}
            className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white flex items-center justify-center shadow-md shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-2.5 sm:p-3 space-y-2 bg-slate-50/50 min-h-0">
        {cart.length === 0 ? (
          <div className="rounded-3xl bg-white border-4 border-dashed border-slate-200 p-8 sm:p-10 text-center">
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-semibold mt-1">Tap product or animal</p>
          </div>
        ) : (
          cart.map((l: CartLine) => (
            <div key={l.id} className={`rounded-2xl bg-white border-4 p-2.5 sm:p-3 shadow-sm ${
              l.type === 'animal' ? 'border-rose-300' :
              l.isPrescription ? 'border-rose-300' :
              l.isExpiring ? 'border-amber-300' : 'border-slate-200'}`}>
              <div className="flex items-start gap-2.5 sm:gap-3">
                <div className={`h-12 w-12 sm:h-14 sm:w-14 rounded-xl overflow-hidden shrink-0 flex items-center justify-center relative ${
                  l.type === 'animal' ? 'bg-gradient-to-br from-rose-100 to-pink-100' : 'bg-slate-100'}`}>
                  {l.image ? <img src={l.image} alt="" className="w-full h-full object-cover" /> :
                    l.type === 'animal' ? <Heart className="h-6 w-6 text-rose-400" /> : <PawPrint className="h-6 w-6 text-slate-400" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <div className="font-extrabold text-sm sm:text-base text-slate-900 leading-tight line-clamp-2">{l.name}</div>
                    {l.type === 'animal' && (
                      <span className="px-1.5 py-0.5 rounded bg-rose-600 text-white text-[9px] font-extrabold uppercase tracking-wider shrink-0">Live</span>
                    )}
                    {l.isPrescription && (
                      <span className="px-1.5 py-0.5 rounded bg-rose-500 text-white text-[9px] font-extrabold uppercase tracking-wider shrink-0">Rx</span>
                    )}
                  </div>
                  {l.brand && <div className="text-[10px] font-bold text-slate-500 mt-0.5">{l.brand}</div>}
                  <div className="text-xs sm:text-sm font-bold text-amber-700 mt-0.5">{formatPKR(l.unitPrice)}</div>
                  {l.note && <div className="text-[10px] sm:text-[11px] font-semibold text-slate-600 mt-1 bg-slate-50 px-2 py-1 rounded-lg">{l.note}</div>}
                  {l.isExpiring && (
                    <div className="text-[10px] font-extrabold text-amber-700 mt-1 inline-flex items-center gap-0.5">
              ...
              <div className="mt-2.5 sm:mt-3 flex items-center justify-between gap-2">
                <div className="inline-flex items-center bg-slate-100 rounded-2xl overflow-hidden border-2 border-slate-200">
                  ...
                  <button onClick={() => onChangeQty(l.id, 1)}
                    disabled={l.type === 'animal'}
                    className={`h-12 sm:h-14 w-12 sm:w-14 text-white flex items-center justify-center ${
                      l.type === 'animal' ? 'bg-rose-600 hover:bg-rose-700' : 'bg-amber-600 hover:bg-amber-700'}`}>
      ...
      {cart.length > 0 && (
        <div className="shrink-0 p-2.5 sm:p-3 border-t-4 border-slate-100 bg-white space-y-2">
          ...
          <button onClick={onCheckout} disabled={!canCheckout}
            className="w-full h-[76px] sm:h-[88px] rounded-3xl font-extrabold text-white shadow-2xl active:scale-[0.98] bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-between px-5 sm:px-6">
            <div className="text-left">
