...
import type {
  PetshopWizardBasic, PetshopWizardVariant, PetshopWizardStock,
} from '../../hooks/usePetshopWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<PetshopWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<PetshopWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<PetshopWizardStock>) => void;
  errors: string[];
}
...
const VARIANT_PRESETS = [
  { group: 'Pack Size', items: ['500g', '1kg', '2kg', '4kg', '7.5kg', '10kg', '15kg'] },
  { group: 'Flavor', items: ['Chicken', 'Beef', 'Fish', 'Lamb', 'Mixed'] },
  { group: 'Size', items: ['XS', 'S', 'M', 'L', 'XL'] },
  { group: 'Color', items: ['Red', 'Blue', 'Green', 'Pink', 'Black'] },
  { group: 'Age', items: ['Puppy', 'Junior', 'Adult', 'Senior'] },
];

export function PetshopWizardStep4Stock({
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-amber-50 to-white border-2 border-amber-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-amber-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> one quantity (single-pack products).
            <strong> Variants:</strong> different pack sizes / flavors / colors — each with its own stock and price.
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Variants (pack sizes / flavors)</div>
            <div className="text-xs text-slate-600 font-semibold">1kg vs 4kg, Chicken vs Beef, Red vs Blue</div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={stock.rackNumber} onChange={(e) => onUpdateStock({ rackNumber: e.target.value })}
                  ...
                  className="h-12 w-full rounded-xl border-2 border-slate-200 pl-10 pr-3 text-sm font-bold focus:outline-none focus:border-amber-500" />
              </div>
            </div>
          </div>
        </section>
      ...
      {displayStock > 0 && sale > 0 && (
        <section className="rounded-2xl bg-gradient-to-br from-slate-950 to-amber-900 text-white p-5 shadow-xl">
