import {
  PawPrint, Boxes, TrendingUp, DollarSign, Pill, Heart,
...
export function PetshopWizardSummary({ draft, stats, allValid }: Props) {
  const hasName = !!draft.basic.name.trim();
  const hasPrice = Number(draft.basic.retailPrice || 0) > 0;

  const expiryDate = draft.medicine.expiryDate ? new Date(draft.medicine.expiryDate) : null;
  const daysToExpiry = expiryDate ? Math.ceil((expiryDate.getTime() - Date.now()) / 86400000) : null;
  const isExpiring = daysToExpiry !== null && daysToExpiry <= 90 && daysToExpiry >= 0;
    ...
    <aside className="flex flex-col gap-3 xl:sticky xl:top-4 xl:self-start">
      <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-amber-900 to-orange-700 text-white p-5 shadow-xl overflow-hidden relative">
        ...
        <div className="relative">
          ...
          {draft.basic.brand && (
            <div className="mt-1 text-[11px] font-bold text-white/80">by {draft.basic.brand}</div>
          )}
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {draft.basic.species && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <Heart className="h-3 w-3" /> {draft.basic.species.replace(/_/g, ' ')}
              </span>
            )}
            {draft.basic.categoryType && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <PawPrint className="h-3 w-3" /> {draft.basic.categoryType.replace(/_/g, ' ')}
              </span>
            )}
            {draft.basic.lifeStage && draft.basic.lifeStage !== 'ALL_STAGES' && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-amber-500/30 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider border border-amber-300/40">
                {draft.basic.lifeStage.replace(/_/g, ' ')}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm overflow-hidden">
        ...
        {draft.medicine.isPrescriptionOnly && (
          <div className="border-t border-slate-100 p-4 bg-gradient-to-br from-rose-50 to-white">
            <div className="flex items-center gap-1.5 mb-1">
              <Pill className="h-3 w-3 text-rose-700" />
              <div className="text-[10px] uppercase tracking-wider font-extrabold text-rose-700">Prescription Only</div>
            </div>
            <div className="text-sm font-extrabold text-rose-900">Rx required to sell</div>
          </div>
        )}

        {expiryDate && (
          <div className={['border-t border-slate-100 p-4',
            isExpired ? 'bg-gradient-to-br from-rose-50 to-white' :
            isExpiring ? 'bg-gradient-to-br from-amber-50 to-white' :
            'bg-gradient-to-br from-emerald-50 to-white'].join(' ')}>
            <div className="flex items-center gap-1.5 mb-1">
              <Calendar className={['h-3 w-3', isExpired ? 'text-rose-700' : isExpiring ? 'text-amber-700' : 'text-emerald-700'].join(' ')} />
              <div className={['text-[10px] uppercase tracking-wider font-extrabold',
                isExpired ? 'text-rose-700' : isExpiring ? 'text-amber-700' : 'text-emerald-700'].join(' ')}>
                {isExpired ? 'Expired' : 'Expires'}
              </div>
            </div>
            <div className={['text-sm font-extrabold',
              isExpired ? 'text-rose-900' : isExpiring ? 'text-amber-900' : 'text-emerald-900'].join(' ')}>
              {expiryDate.toLocaleDateString('en-PK', { day: 'numeric', month: 'short', year: 'numeric' })}
            </div>
            {daysToExpiry !== null && !isExpired && (
              <div className={['text-[10px] font-bold mt-0.5', isExpiring ? 'text-amber-700' : 'text-emerald-700'].join(' ')}>
      ...
      {(draft.basic.isFeatured || draft.basic.isBestSeller || draft.basic.isNewArrival || draft.basic.isOnSale) && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {draft.basic.isOnSale && <Flag icon="🔥" label="On Sale" tone="rose" />}
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-slate-50 border-2 border-slate-200 p-3 space-y-1.5">
        ...
        <Chk done={!!draft.basic.species} label="Species" />
