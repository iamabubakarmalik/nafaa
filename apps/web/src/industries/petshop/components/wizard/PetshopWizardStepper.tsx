import { Check, PawPrint, Sparkles, Pill, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/usePetshopWizard';
...
const STEPS = [
  { id: 1, label: 'Basic Info', short: 'Info', desc: 'Name, species, price', icon: PawPrint },
  { id: 2, label: 'Product Details', short: 'Details', desc: 'Food specs, dimensions', icon: Sparkles },
  { id: 3, label: 'Medicine & Expiry', short: 'Medicine', desc: 'Rx, batch, expiry', icon: Pill },
  { id: 4, label: 'Stock & Variants', short: 'Stock', desc: 'Quantity, pack sizes', icon: Package },
] as const;

export function PetshopWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-1.5 flex-1">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-amber-500 to-orange-700 text-white shadow-md shadow-amber-500/40 scale-[1.02]'
                    : isPast
