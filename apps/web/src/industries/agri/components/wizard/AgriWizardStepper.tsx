import { Check, Wheat, FlaskConical, ShieldCheck } from 'lucide-react';
import type { WizardStep } from '../../hooks/useAgriWizard';
...
const STEPS = [
  { id: 1, label: 'Basic & Category', desc: 'Product info & pricing', icon: Wheat },
  { id: 2, label: 'Agri Profile', desc: 'Specs, crops, certs', icon: FlaskConical },
  { id: 3, label: 'Safety & Stock', desc: 'Hazard, storage, bulk', icon: ShieldCheck },
] as const;

export function AgriWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-2">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-lime-600 to-green-700 text-white shadow-md shadow-lime-500/40'
                    : isPast
                      ? 'bg-lime-50 text-lime-800 hover:bg-lime-100 border-2 border-lime-200'
              ...
              >
                <div className={[
                  ...
                  isActive ? 'bg-white/25 text-white'
                    : isPast && isComplete ? 'bg-lime-600 text-white'
                ...
                <div className="text-left">
                  <div className={[
                    ...
                    isActive ? 'text-white/90' : isPast ? 'text-lime-700' : 'text-slate-500',
                  ].join(' ')}>Step {s.id}</div>
              ...
              {idx < STEPS.length - 1 && (
                <div className={['h-0.5 w-8 rounded-full transition',
                  currentStep > s.id ? 'bg-lime-500' : 'bg-slate-200'].join(' ')} />
              )}
            </div>
          );
        })}
