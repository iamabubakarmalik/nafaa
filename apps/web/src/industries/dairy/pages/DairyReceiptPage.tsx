...
import {
  Printer, ArrowLeft, MessageCircle, X, Milk, Sunrise, Sunset,
  MapPin, Phone, Calendar, User, CheckCircle2, ShieldAlert, Truck,
} from 'lucide-react';
...
export default function DairyReceiptPage() {
  const { id } = useParams();
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Dairy';

    const lines: string[] = [];
    lines.push(`🥛 *${shopName}*`);
    lines.push('');
    lines.push(`Assalam-o-Alaikum ${sale.customer.name}!`);
    lines.push('Thanks for your dairy purchase 🙏');
    lines.push('');
    lines.push(`*Receipt:* ${sale.saleNumber}`);
    lines.push(`*Date:* ${formatDate(sale.soldAt)}`);
    if ((sale as any).note) lines.push(`*Note:* ${(sale as any).note}`);
    lines.push('');
    ...
    if (sale.creditAmount > 0) lines.push(`Khata baqi: ${formatPKR(sale.creditAmount)}`);
    lines.push('');
    lines.push('_Fresh milk daily!_ 🥛');

    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(lines.join('\n'))}`, '_blank');
  };

  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><div className="h-10 w-10 rounded-full border-4 border-fuchsia-200 border-t-fuchsia-600 animate-spin" /></div>;
  if (!sale) return <div className="min-h-screen flex flex-col items-center justify-center"><p className="font-bold">Receipt not found</p><Link to="/sales" className="mt-4 text-fuchsia-600 hover:underline">← Back</Link></div>;

  const settings = sale.tenant?.settings;
  const shopName = settings?.shopName || sale.tenant?.name || 'Dairy Shop';
  ...
  const note = (sale as any).note || '';
  const isMorning = note.includes('MORNING');
  const isEvening = note.includes('EVENING');

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-fuchsia-50 py-6 px-4 print:bg-white print:py-0 print:px-0">
      <div className={`mx-auto space-y-4 ${format === 'a4' ? 'max-w-4xl' : 'max-w-md'}`}>
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-fuchsia-500 to-pink-600 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="inline-flex rounded-xl border-2 border-slate-200 bg-white shadow-sm overflow-hidden">
              {(['a4', 'thermal80', 'thermal58'] as Format[]).map((f, i) => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`px-3 py-2.5 text-xs font-bold transition ${format === f ? 'bg-fuchsia-600 text-white' : 'text-slate-700 hover:bg-slate-50'} ${i > 0 ? 'border-l-2 border-slate-200' : ''}`}>
            ...
            <button onClick={() => window.print()}
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-600 to-pink-700 hover:from-fuchsia-700 hover:to-pink-800 px-4 py-2.5 text-sm font-bold text-white shadow-md">
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
            <div className="relative bg-gradient-to-br from-slate-950 via-fuchsia-900 to-pink-700 text-white px-8 py-7 print:bg-white print:text-slate-900 print:border-b-4 print:border-double print:border-slate-700 overflow-hidden">
              <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full bg-fuchsia-400/20 blur-2xl print:hidden" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <Milk className="h-3 w-3" /> Dairy Receipt
                    </div>
            ...
            {(isMorning || isEvening) && (
              <div className={`px-8 py-4 border-b-2 print:bg-white ${isMorning ? 'bg-amber-50 border-amber-200' : 'bg-indigo-50 border-indigo-200'}`}>
                <div className="flex items-center justify-center gap-3">
                  {isMorning && <Sunrise className="h-6 w-6 text-amber-700" />}
                  {isEvening && <Sunset className="h-6 w-6 text-indigo-700" />}
                  <div className={`text-2xl font-extrabold ${isMorning ? 'text-amber-900' : 'text-indigo-900'}`}>
                    {isMorning ? '🌅 Morning Delivery' : '🌆 Evening Delivery'}
                  </div>
                </div>
                {note && <div className="mt-2 text-center text-xs text-slate-600 font-semibold italic">{note}</div>}
              </div>
            )}
            ...
            <div className="px-8 py-6">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b-2 border-slate-300 bg-gradient-to-r from-fuchsia-50 to-slate-50 print:bg-white">
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase">Item</th>
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase text-center w-20">Qty</th>
                  ...
                  {sale.items.map((it: any) => (
                    <tr key={it.id} className="border-b border-slate-100">
                      <td className="py-3 px-2">
                        ...
                        {it.note && <div className="text-[10px] italic text-fuchsia-700 mt-0.5">{it.note}</div>}
                      </td>
                      <td className="py-3 px-2 text-center">
                        <div className="font-extrabold tabular-nums">{it.quantity.toFixed(1)}</div>
            ...
            <div className="px-8 py-6 border-t-2 border-dashed border-slate-300 bg-gradient-to-br from-slate-50 to-white print:bg-white">
              <div className="ml-auto max-w-md space-y-1.5">
                ...
                <div className="flex items-center justify-between text-lg pt-3 border-t-2 border-slate-300">
                  ...
                  <span className="font-extrabold text-fuchsia-700 text-3xl tabular-nums">{formatPKR(sale.total)}</span>
                </div>
                <div className="pt-3 border-t border-slate-200 space-y-1">
                  ...
                  {sale.creditAmount > 0 && (
                    <div className="flex justify-between text-sm rounded-lg bg-amber-50 border-2 border-amber-300 px-3 py-2 mt-2 print:bg-white"><span className="text-amber-800 font-bold">Khata / Udhaar</span><span className="font-extrabold text-amber-700 text-base tabular-nums">{formatPKR(sale.creditAmount)}</span></div>
                  )}
                </div>
              </div>
            </div>

            <div className="px-8 py-5 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-fuchsia-50 to-pink-50 print:bg-white">
              {receiptFooter && <div className="text-sm italic text-slate-700 mb-2">{receiptFooter}</div>}
              <div className="text-lg font-extrabold text-slate-900">🥛 Fresh Milk Daily! 🙏</div>
        ...
        {(format === 'thermal58' || format === 'thermal80') && (
          <div className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`}
            ...
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              {(isMorning || isEvening) && (
                <div className="border-t border-b border-dashed border-slate-400 py-1 mb-1 text-center">
                  <div className="font-extrabold text-base">
                    {isMorning ? '🌅 MORNING SLOT' : '🌆 EVENING SLOT'}
                  </div>
                </div>
              )}
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                {sale.items.map((item: any, idx: number) => (
                  <div key={item.id} className="mb-0.5">
                    <div className="font-bold text-[10px]">{idx + 1}. {item.product.name}</div>
                    <div className="flex justify-between pl-2">
                      <span>{item.quantity.toFixed(1)} {item.product.unit} × {formatPKR(item.price)}</span>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                ...
                {sale.creditAmount > 0 && <div className="flex justify-between font-bold"><span>KHATA:</span><span>{formatPKR(sale.creditAmount)}</span></div>}
              </div>
