    ...
    <aside className="flex flex-col gap-3 xl:sticky xl:top-4 xl:self-start">
      ...
      <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="border-t-2 border-slate-100 p-4 bg-gradient-to-br from-sky-50 to-white">
          ...
          <div className="text-3xl font-extrabold text-sky-900 tabular-nums">{stats.totalStock}</div>
        </div>
      </div>
          ...
          <div className="pt-2 border-t border-slate-100 space-y-1">
            <div className="flex items-center justify-between">
              <div className="text-xs font-extrabold text-slate-700 flex items-center gap-1">
                <TrendingUp className="h-3 w-3 text-emerald-600" /> Sambhava faida
              </div>
...
function Cell({ icon: Icon, label, value, tone, hint }: {
  ...
  const tones: Record<string, string> = {
    sky: 'text-sky-700', violet: 'text-violet-700', amber: 'text-amber-700', emerald: 'text-emerald-700',
  };
  return (
    <div className="p-4">
      <div className="flex items-center gap-1.5 mb-1">
