import { Check, Pill, Beaker, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/usePharmacyWizard';
...
const STEPS = [
  { id: 1, label: 'Medicine Info', desc: 'Basic + pricing', icon: Pill },
  { id: 2, label: 'Salts & Clinical', desc: 'Composition + storage', icon: Beaker },
  { id: 3, label: 'Batches & Stock', desc: 'Expiry tracking', icon: Package },
] as const;

export function PharmacyWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-2">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-teal-600 to-cyan-700 text-white shadow-md shadow-teal-500/40'
                    : isPast
                      ? 'bg-teal-50 text-teal-800 hover:bg-teal-100 border-2 border-teal-200'
              ...
              >
                <div className={[
                  ...
                  isActive ? 'bg-white/25 text-white'
                    : isPast && isComplete ? 'bg-teal-600 text-white'
                ...
                <div className="text-left">
                  <div className={[
                    ...
                    isActive ? 'text-white/90' : isPast ? 'text-teal-700' : 'text-slate-500',
                  ].join(' ')}>
                    Step {s.id}
                  </div>
              ...
              {idx < STEPS.length - 1 && (
                <div className={['h-0.5 w-8 rounded-full transition',
                  currentStep > s.id ? 'bg-teal-500' : 'bg-slate-200'].join(' ')} />
              )}
            </div>
          );
        })}
