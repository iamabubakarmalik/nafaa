...
import {
  ...
  Star, ChevronRight, Glasses, Eye, Receipt, ShoppingCart, Hash,
  Tag, Trash2, AlertTriangle, History, Sparkles, BarChart3, Info,
  Plus, Shield, Users, Palette, Award, CheckCircle2, XCircle, Video,
} from 'lucide-react';
import {
...
export default function OpticalProductDetailPage() {
  const { id } = useParams();
  ...
  const TABS: { id: Tab; label: string; count?: number; icon: any }[] = [
    ...
    { id: 'prescription', label: 'Rx Support', icon: Eye },
  ...
  return (
    <div className="space-y-5 pb-10">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <button onClick={() => navigate('/optical-products')} className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-cyan-600 font-bold">
          <ArrowLeft className="h-4 w-4" /> All Products
        </button>
        <div className="flex items-center gap-2 flex-wrap">
          <Link to={`/optical-products/${id}/edit`} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-cyan-50 border-2 border-cyan-200 hover:bg-cyan-100 text-cyan-700 text-sm font-extrabold">
      ...
      {/* HERO */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-cyan-900 to-sky-700 text-white shadow-2xl">
        ...
        <div className="relative grid lg:grid-cols-[280px_1fr] gap-6 p-6">
          <div className="space-y-2">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-white/10 backdrop-blur border-2 border-white/20">
              {gallery[imgIndex]?.url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-white/50"><Glasses className="h-16 w-16" /></div>
              )}
              {profile?.isDesigner && (
                <div className="absolute top-3 right-3 px-2 py-1 rounded-lg bg-fuchsia-500 text-white text-[10px] font-extrabold shadow-lg">
                  ✨ DESIGNER
                </div>
              )}
              {profile?.isFeatured && !profile?.isDesigner && (
            ...
            {profile?.tryOnUrl && (
              <a href={profile.tryOnUrl} target="_blank" rel="noreferrer"
                className="w-full h-11 rounded-xl bg-violet-500 hover:bg-violet-600 text-white font-extrabold text-xs inline-flex items-center justify-center gap-1.5 transition">
          ...
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Glasses className="h-3.5 w-3.5 text-amber-300" /> Optical
            ...
            <div className="mt-3 flex items-center gap-2 flex-wrap text-xs">
              ...
              {profile?.collectionName && <Chip icon={Sparkles}>{profile.collectionName}</Chip>}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
              ...
              <HeroStat icon={Boxes} label="Variants" value={String(stats.variantCount)} sub={stats.variantStock > 0 ? `${stats.variantStock} units` : 'single SKU'} tone="fuchsia" />
            </div>
          </div>
        </div>
      </section>
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`px-4 py-2.5 rounded-xl text-sm font-extrabold inline-flex items-center gap-2 transition ${
                  active ? 'bg-gradient-to-br from-cyan-600 to-sky-700 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>
        ...
        <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-semibold">{stats.sold30} units sold • avg {stats.perDay.toFixed(1)}/day</p>
            </div>
          </div>
            ...
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  <defs>
                    <linearGradient id="opticalDetailGrad" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#06b6d4" strokeWidth={2.5} fill="url(#opticalDetailGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ) : (
      ...
      {tab === 'specs' && (
        <Panel icon={Sparkles} title="Product Specifications" desc="Frame, lens or contact lens details" tone="cyan"
          empty={!profile} emptyText="No specs added"
          emptyAction={<Link to={`/optical-products/${id}/edit`}><Button className="bg-cyan-600"><Edit3 className="h-4 w-4" /> Add Specs</Button></Link>}>
          {profile && (
            <div className="p-5 space-y-5">
              {(profile.frameShape || profile.frameMaterial || profile.frameSizeMm) && (
                <SpecSection icon={Glasses} title="Frame" tone="cyan">
                  {profile.frameShape && <SpecRow label="Shape" value={profile.frameShape.replace(/_/g, ' ')} />}
                  {profile.frameMaterial && <SpecRow label="Material" value={profile.frameMaterial.replace(/_/g, ' ')} />}
                  {profile.frameSizeMm && profile.bridgeSizeMm && profile.templeLengthMm && (
                    <SpecRow label="Measurements" value={`${profile.frameSizeMm}▫${profile.bridgeSizeMm}-${profile.templeLengthMm} mm`} mono />
                  )}
                  {profile.frameWeightG && <SpecRow label="Weight" value={`${profile.frameWeightG} g`} />}
                </SpecSection>
              )}

              {profile.frameColorOptions?.length > 0 && (
                <SpecSection icon={Palette} title="Available Colors" tone="pink">
                  <div className="flex flex-wrap gap-2">
                    {profile.frameColorOptions.map((c: string) => (
                      <span key={c} className="px-3 py-1.5 rounded-lg bg-pink-100 text-pink-800 text-xs font-extrabold">{c}</span>
                    ))}
                  </div>
                </SpecSection>
              )}

              {(profile.lensType || profile.lensMaterial || (profile.lensCoatings ?? []).length > 0) && (
                <SpecSection icon={Eye} title="Lens Features" tone="blue">
                  {profile.lensType && <SpecRow label="Type" value={profile.lensType} />}
                  {profile.lensMaterial && <SpecRow label="Material" value={profile.lensMaterial} />}
                  {profile.lensIndex && <SpecRow label="Refractive Index" value={profile.lensIndex} />}
                  {profile.lensCoatings?.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase font-extrabold text-slate-500 mb-1.5">Coatings</div>
                      <div className="flex flex-wrap gap-1.5">
                        {profile.lensCoatings.map((c: string) => (
                          <span key={c} className="px-2 py-0.5 rounded-md bg-blue-100 text-blue-800 text-[10px] font-extrabold">{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {(profile.hasBlueCut || profile.hasAntiGlare || profile.hasUvProtection || profile.isPolarized || profile.isPhotochromic) && (
                <SpecSection icon={Shield} title="Special Features" tone="emerald">
                  <div className="grid grid-cols-2 gap-2">
                    {profile.hasBlueCut && <FeatureBadge label="Blue Cut" />}
                    {profile.hasAntiGlare && <FeatureBadge label="Anti-Glare" />}
                    {profile.hasUvProtection && <FeatureBadge label="UV Protection" />}
                    {profile.isPolarized && <FeatureBadge label="Polarized" />}
                    {profile.isPhotochromic && <FeatureBadge label="Photochromic" />}
                    {profile.supportsProgressive && <FeatureBadge label="Progressive Support" />}
                  </div>
                </SpecSection>
              )}

              {profile.isContactLens && (
                <SpecSection icon={Eye} title="Contact Lens Details" tone="violet">
                  {profile.clDuration && <SpecRow label="Duration" value={profile.clDuration} />}
                  {profile.clWaterContent && <SpecRow label="Water Content" value={profile.clWaterContent} />}
                  {profile.clBaseCurve && <SpecRow label="Base Curve (BC)" value={profile.clBaseCurve} />}
                  {profile.clDiameter && <SpecRow label="Diameter (DIA)" value={profile.clDiameter} />}
                  {profile.clUvProtection && <SpecRow label="UV Protection" value="✓ Yes" />}
                  {profile.clForAstigmatism && <SpecRow label="Toric (Astigmatism)" value="✓ Yes" />}
                </SpecSection>
              )}

              {(profile.warrantyMonths || profile.warrantyType) && (
                <SpecSection icon={Shield} title="Warranty" tone="amber">
                  {profile.warrantyMonths && <SpecRow label="Period" value={`${profile.warrantyMonths} months`} />}
                  {profile.warrantyType && <SpecRow label="Type" value={profile.warrantyType} />}
                </SpecSection>
              )}
            </div>
          )}
      ...
      {tab === 'prescription' && (
        <Panel icon={Eye} title="Prescription Support" desc="Which powers this product can be made in" tone="cyan"
          empty={!profile} emptyText="No prescription range set">
          {profile && (
            <div className="p-5 space-y-4">
              {(profile.supportsMinSph != null || profile.supportsMaxSph != null) ? (
                <div className="rounded-2xl bg-gradient-to-br from-cyan-50 to-sky-50 border-2 border-cyan-200 p-5">
                  <div className="text-[10px] uppercase font-extrabold text-cyan-700 mb-1">SPH Range</div>
                  <div className="text-3xl font-extrabold text-cyan-900 tabular-nums">
                    {profile.supportsMinSph != null ? (profile.supportsMinSph >= 0 ? '+' : '') + profile.supportsMinSph : '?'}
                    <span className="text-xl text-slate-500 mx-2">to</span>
                    {profile.supportsMaxSph != null ? (profile.supportsMaxSph >= 0 ? '+' : '') + profile.supportsMaxSph : '?'}
                  </div>
                </div>
              ) : (
                <div className="rounded-2xl bg-slate-50 border-2 border-slate-200 p-5 text-center text-sm font-extrabold text-slate-600">
                  No SPH restriction — plano or universal
                </div>
              )}

              {(profile.supportsMinCyl != null || profile.supportsMaxCyl != null) && (
                <div className="rounded-2xl bg-gradient-to-br from-violet-50 to-fuchsia-50 border-2 border-violet-200 p-5">
                  <div className="text-[10px] uppercase font-extrabold text-violet-700 mb-1">CYL Range (Astigmatism)</div>
                  <div className="text-2xl font-extrabold text-violet-900 tabular-nums">
                    Up to {Math.abs(profile.supportsMaxCyl ?? 0)}
                  </div>
                </div>
              )}

              <div className={`rounded-2xl border-2 p-4 ${profile.supportsProgressive ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center gap-2">
                  {profile.supportsProgressive ? <CheckCircle2 className="h-5 w-5 text-emerald-600" /> : <XCircle className="h-5 w-5 text-slate-400" />}
                  <div className={`font-extrabold ${profile.supportsProgressive ? 'text-emerald-900' : 'text-slate-600'}`}>
                    Progressive Lens Support: {profile.supportsProgressive ? 'Yes' : 'No'}
                  </div>
                </div>
              </div>
            </div>
      ...
      {tab === 'variants' && (
        <Panel icon={Boxes} title="Variants" desc={`${stats.variantCount} variants • total ${stats.variantStock} units`} tone="fuchsia"
          ...
          emptyAction={<Link to={`/optical-products/${id}/edit`}><Button variant="secondary"><Plus className="h-4 w-4" /> Add Variants</Button></Link>}>
              ...
              return (
                <Link key={s.id} to={`/sales/${s.id}/receipt`} className="block px-5 py-3 hover:bg-cyan-50/40 transition">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="font-extrabold text-emerald-700 tabular-nums shrink-0">{formatPKRFull(rev)}</div>
...
function HeroStat({ icon: Icon, label, value, sub, tone }: any) {
  const tones: Record<string, string> = {
    ...
    fuchsia: 'from-fuchsia-400/30 to-fuchsia-600/20 border-fuchsia-300/40',
...
function SpecSection({ icon: Icon, title, tone, children }: any) {
  const tones: Record<string, string> = {
    cyan: 'from-cyan-500 to-sky-700 bg-cyan-50 border-cyan-200',
...
function SpecRow({ label, value, mono }: any) {
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <span className={['font-extrabold text-slate-900', mono ? 'font-mono' : ''].join(' ')}>{value}</span>
    </div>
  );
}
...
function FeatureBadge({ label }: { label: string }) {
  return (
    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-100 border-2 border-emerald-300 text-xs font-extrabold text-emerald-800">
