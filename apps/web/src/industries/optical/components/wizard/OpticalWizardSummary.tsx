import {
  Glasses, Boxes, TrendingUp, DollarSign, Eye, Shield,
  AlertTriangle, CheckCircle2, Sparkles, Users, Palette,
} from 'lucide-react';
import { formatPKRFull } from '@core/lib/format';
import type { OpticalWizardDraft } from '../../hooks/useOpticalWizard';

interface Props {
  draft: OpticalWizardDraft;
  stats: {
    variantCount: number;
    totalStock: number;
    stockValue: number;
...
export function OpticalWizardSummary({ draft, stats, allValid }: Props) {
  ...
  return (
    <aside className="flex flex-col gap-3 xl:sticky xl:top-4 xl:self-start">
      <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-cyan-900 to-sky-700 text-white p-5 shadow-xl overflow-hidden relative">
        ...
        <div className="relative">
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap text-[11px] font-bold text-white/80">
            ...
            {draft.basic.modelNumber && <span className="font-mono">• {draft.basic.modelNumber}</span>}
          </div>
          {hasPrice && (
            <div className="mt-3">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {draft.basic.categoryType && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <Glasses className="h-3 w-3" /> {draft.basic.categoryType.replace(/_/g, ' ')}
              </span>
            )}
            {draft.basic.gender && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <Users className="h-3 w-3" /> {draft.basic.gender}
              </span>
            )}
          </div>
        </div>
      ...
      {/* Frame preview */}
      {(draft.frame.frameShape || draft.frame.frameMaterial || draft.frame.colorName) && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-4">
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-slate-600 mb-2 flex items-center gap-1">
            <Sparkles className="h-3 w-3" /> Frame specs
          </div>
          <div className="space-y-1.5 text-xs">
            {draft.frame.frameShape && <Row label="Shape" value={draft.frame.frameShape.replace(/_/g, ' ')} />}
            {draft.frame.frameMaterial && <Row label="Material" value={draft.frame.frameMaterial.replace(/_/g, ' ')} />}
            {draft.frame.colorName && (
              <Row label="Color" value={
                <span className="inline-flex items-center gap-1.5">
                  {draft.frame.colorHex && <span className="h-3 w-3 rounded-full border border-slate-300" style={{ backgroundColor: draft.frame.colorHex }} />}
                  {draft.frame.colorName}
                </span>
              } />
            )}
            {draft.frame.frameSizeMm && draft.frame.bridgeSizeMm && draft.frame.templeLengthMm && (
              <Row label="Size" value={`${draft.frame.frameSizeMm}▫${draft.frame.bridgeSizeMm}-${draft.frame.templeLengthMm}`} mono />
            )}
          </div>
        </div>
      )}
      ...
      {/* Lens preview */}
      {(draft.lens.lensType || draft.lens.hasBlueCut || draft.lens.isPolarized || (draft.lens.lensCoatings ?? []).length > 0) && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-4">
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-slate-600 mb-2 flex items-center gap-1">
            <Eye className="h-3 w-3" /> Lens features
          </div>
          <div className="flex flex-wrap gap-1">
            {draft.lens.lensType && <Chip label={draft.lens.lensType} tone="blue" />}
            {draft.lens.lensIndex && <Chip label={`Index ${draft.lens.lensIndex}`} tone="slate" />}
            {draft.lens.hasBlueCut && <Chip label="Blue Cut" tone="blue" />}
            {draft.lens.hasAntiGlare && <Chip label="Anti-Glare" tone="violet" />}
            {draft.lens.hasUvProtection && <Chip label="UV" tone="amber" />}
            {draft.lens.isPolarized && <Chip label="Polarized" tone="emerald" />}
            {draft.lens.isPhotochromic && <Chip label="Photochromic" tone="pink" />}
            {draft.lens.supportsProgressive && <Chip label="Progressive" tone="fuchsia" />}
          </div>
          {draft.lens.supportsMinSph !== '' && draft.lens.supportsMaxSph !== '' && (
            <div className="mt-2 pt-2 border-t border-slate-100 text-xs">
              <span className="font-bold text-slate-500">Rx range: </span>
              <span className="font-extrabold text-slate-900 tabular-nums">
                {Number(draft.lens.supportsMinSph) >= 0 ? '+' : ''}{Number(draft.lens.supportsMinSph)} to {Number(draft.lens.supportsMaxSph) >= 0 ? '+' : ''}{Number(draft.lens.supportsMaxSph)}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm overflow-hidden">
        <div className="grid grid-cols-2 divide-x divide-slate-100">
          <Cell icon={Boxes} label="Variants" value={stats.variantCount} hint={draft.hasVariants ? 'variants on' : 'single SKU'} tone="fuchsia" />
        ...
        {draft.warranty.warrantyMonths ? (
          <div className="border-t border-slate-100 p-4 bg-gradient-to-br from-blue-50 to-white">
            ...
            <div className="text-[10px] text-blue-700 font-bold">{draft.warranty.warrantyType}</div>
          </div>
        ) : null}
      </div>
      ...
      {stats.totalStock > 0 && hasPrice && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-2.5">
          ...
          <Row2 label="Cost value" value={formatPKRFull(stats.stockCost)} tone="slate" />
          <Row2 label="Retail value" value={formatPKRFull(stats.stockValue)} tone="emerald" />
      ...
      {(draft.basic.isFeatured || draft.basic.isBestSeller || draft.basic.isNewArrival || draft.basic.isDesigner) && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {draft.basic.isDesigner && <Flag icon="✨" label="Designer" tone="fuchsia" />}
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl bg-cyan-50 border-2 border-cyan-200 p-2.5 text-[10px] text-cyan-800 font-extrabold text-center">
        💾 Draft auto-saved — safe to close and come back later
      </div>
    </aside>
  );
}
...
function Row2({ label, value, tone }: any) {
...
function Chip({ label, tone }: any) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md border text-[10px] font-extrabold ${tones[tone]}`}>
      {label}
    </span>
  );
}
