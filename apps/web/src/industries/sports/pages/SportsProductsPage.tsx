...
import {
  Dumbbell, Plus, Search, X, RefreshCw, Download, Grid3x3, List,
  Package, AlertTriangle, DollarSign, Eye, Edit3, Trash2,
  CheckCircle2, Star, Users, Trophy, PackageX, Award,
} from 'lucide-react';
...
const SPORTS = ['Cricket', 'Football', 'Basketball', 'Tennis', 'Badminton', 'Gym / Fitness', 'Swimming', 'Boxing'];
...
export default function SportsProductsPage() {
  ...
  const { data: brands = [] } = useQuery({
    queryKey: ['sports-brands'],
    queryFn: () => sportsBrandsApi.list({ active: true }),
  });
  ...
  const stats = useMemo(() => ({
    ...
    teamOrderable: (profiles as any[]).filter((p) => p.isTeamOrderable).length,
  }), [products, profiles]);
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((p) => {
      ...
      return (p.name || '').toLowerCase().includes(q) || (p.sku || '').toLowerCase().includes(q) ||
        (p.barcode || '').toLowerCase().includes(q) || (pr?.sport || '').toLowerCase().includes(q);
    });
    if (sportFilter !== 'all') list = list.filter((p) => pmap.get(p.id)?.sport === sportFilter);
    if (brandFilter !== 'all') list = list.filter((p) => pmap.get(p.id)?.brandId === brandFilter);
    if (teamOnly) list = list.filter((p) => pmap.get(p.id)?.isTeamOrderable);
    if (professionalOnly) list = list.filter((p) => pmap.get(p.id)?.isProfessional);
    ...
    return list;
  }, [products, search, sportFilter, brandFilter, teamOnly, professionalOnly, stockFilter, sortKey, pmap]);
  ...
  const exportCSV = () => {
    ...
    const head = ['Name', 'SKU', 'Sport', 'Category', 'Brand', 'Cost', 'Retail', 'Stock', 'Team Orderable'];
    const rows = filtered.map((p) => {
      const pr = pmap.get(p.id);
      const br = (brands as any[]).find((b) => b.id === pr?.brandId);
      return [p.name, p.sku || '', pr?.sport || '', pr?.categoryType?.replace(/_/g, ' ') || '',
        br?.name || '', Number(p.costPrice || 0).toFixed(2), Number(p.price || 0).toFixed(2),
        Number(p.stock || 0), pr?.isTeamOrderable ? 'Yes' : 'No'];
    });
    const csv = [head, ...rows].map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8' }));
    const a = document.createElement('a');
    a.href = url; a.download = `sports-products-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="space-y-5 pb-6">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Dumbbell className="h-3.5 w-3.5 text-amber-300" /> Sports Catalogue
            </div>
            <h1 className="mt-3 text-3xl sm:text-4xl font-extrabold">Products</h1>
            <p className="mt-2 text-sm text-white/80">
              {stats.total} products • {stats.totalStock} pcs • Value{' '}
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Kpi icon={Package} label="Products" value={stats.total} sub={`${stats.totalStock} pcs`} tone="emerald" />
        <Kpi icon={DollarSign} label="Stock Value" value={formatPKR(stats.stockValue)} sub="Retail" tone="blue" />
        <Kpi icon={Users} label="Team Orderable" value={stats.teamOrderable} sub="Bulk-ready" tone="violet" onClick={() => setTeamOnly((v) => !v)} />
      ...
      <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input autoFocus value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Name, SKU, sport, barcode..."
          ...
          <div className="inline-flex rounded-2xl border-2 border-slate-200 bg-white overflow-hidden">
            <button onClick={() => setView('grid')} className={`px-4 h-12 ${view === 'grid' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>
              <Grid3x3 className="h-4 w-4" />
            </button>
            <button onClick={() => setView('table')} className={`px-4 h-12 border-l-2 border-slate-200 ${view === 'table' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>
              <List className="h-4 w-4" />
            </button>
          </div>
          <button onClick={exportCSV}
            className="h-12 px-4 rounded-2xl border-2 border-slate-200 hover:border-emerald-300 text-sm font-bold text-slate-700 inline-flex items-center gap-1.5">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <select value={sportFilter} onChange={(e) => setSportFilter(e.target.value)}
            className="h-10 rounded-xl border-2 border-slate-200 bg-white px-3 text-xs font-bold focus:outline-none focus:border-emerald-500">
          ...
          <button onClick={() => setTeamOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${teamOnly ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-700'}`}>
            <Users className="h-3.5 w-3.5" /> Team Orderable
          </button>
          <button onClick={() => setProfessionalOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${professionalOnly ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-700'}`}>
            <Trophy className="h-3.5 w-3.5" /> Pro Grade
          </button>
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
            {(['all', 'in', 'low', 'out'] as StockFilter[]).map((v) => (
              <button key={v} onClick={() => setStockFilter(v)}
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold ${stockFilter === v ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>
      ...
      {isLoading ? (
        ...
        <section className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          <div className="mx-auto h-20 w-20 rounded-3xl bg-gradient-to-br from-emerald-100 to-teal-200 flex items-center justify-center">
            <Dumbbell className="h-9 w-9 text-emerald-600" />
          </div>
          <h3 className="mt-5 text-xl font-extrabold text-slate-900">No products found</h3>
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add your first bat, jersey or gym item</p>
          <Link to="/sports-products/new">
            <Button className="mt-5 bg-gradient-to-r from-emerald-600 to-teal-700"><Plus className="h-4 w-4" /> Add Product</Button>
          </Link>
        </section>
      ) : view === 'grid' ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b-2 border-slate-200">
              <tr>
                ...
                <Th>Product</Th><Th>Sport</Th><Th>Brand</Th>
                <Th className="text-right">Retail</Th><Th className="text-right">Stock</Th>
                <Th className="text-center">Team</Th><Th className="text-right">Actions</Th>
              </tr>
            </thead>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-emerald-50/40">
                    ...
                    <td className="px-3 py-2.5">
                      <Link to={`/sports-products/${p.id}`} className="flex items-center gap-2.5 group">
                        ...
                        <div className="min-w-0">
                          <div className="font-extrabold text-slate-900 text-sm truncate group-hover:text-emerald-700">{p.name}</div>
                          <div className="text-[10px] font-mono text-slate-500 truncate">{p.sku || '—'}</div>
                        </div>
                      </Link>
                    </td>
                    <td className="px-3 py-2.5 text-xs font-bold text-slate-700">{pr?.sport || '—'}</td>
                    <td className="px-3 py-2.5 text-xs font-bold text-slate-700">{br?.name || '—'}</td>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      {pr?.isTeamOrderable ? (
                        <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-extrabold">✓ Yes</span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/sports-products/${p.id}`} className="h-8 w-8 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 flex items-center justify-center">
                          <Eye className="h-3.5 w-3.5" />
                        </Link>
                        <Link to={`/sports-products/${p.id}/edit`} className="h-8 w-8 rounded-lg bg-violet-50 hover:bg-violet-100 text-violet-700 flex items-center justify-center">
...
function Card({ p, pr, sel, onToggle }: any) {
  ...
  return (
    <div className={`group relative rounded-2xl bg-white border-2 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 ${
      sel ? 'border-emerald-500 ring-2 ring-emerald-200' : isOut ? 'border-rose-200' : isLow ? 'border-amber-200' : 'border-slate-200'}`}>
      <button onClick={onToggle}
        className={`absolute top-2 left-2 z-10 h-6 w-6 rounded-lg border-2 flex items-center justify-center transition ${
          sel ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white/90 border-slate-300 opacity-0 group-hover:opacity-100'}`}>
        {sel && <CheckCircle2 className="h-3.5 w-3.5" />}
      </button>
      <Link to={`/sports-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 relative overflow-hidden">
          {p.images?.[0]?.url
            ...
            : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-50 to-teal-50"><Dumbbell className="h-10 w-10 text-emerald-300" /></div>}
          {pr?.isProfessional && <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-md bg-blue-600 text-white text-[9px] font-extrabold">PRO</div>}
          {pr?.isFeatured && !pr?.isProfessional && (
        ...
        <div className="p-2.5">
          ...
          {pr?.sport && <div className="mt-1 text-[9px] font-extrabold uppercase text-emerald-700">{pr.sport}</div>}
...
function Kpi({ icon: Icon, label, value, sub, tone, onClick }: any) {
  const tones: Record<string, string> = {
    emerald: 'from-emerald-500 to-teal-700', blue: 'from-blue-500 to-cyan-700',
