...
import {
  ...
  Star, Pause, Play, Percent, Dumbbell, Users, Trophy,
...
const AUTO_CLOSE_KEY = 'nafaa.sports-pos.auto-close-success';
...
export default function SportsPosPage() {
  ...
  useEffect(() => {
    const refocus = () => {
      if (scannerOpen || showCheckout || showCustomerAdd || showHeldCarts || lastSale) return;
    ...
    return () => clearTimeout(t);
  }, [scannerOpen, showCheckout, showCustomerAdd, showHeldCarts, lastSale]);
  ...
  const { data: productsData, isLoading: loadingProducts } = useQuery({
    queryKey: ['products-for-sports-pos'],
  ...
  const { data: profiles = [] } = useQuery({
    queryKey: ['sports-profiles-all'],
    queryFn: () => sportsProductsApi.list(),
    staleTime: 60_000,
  });
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (sportFilter) {
      list = list.filter((p) => profileByProduct.get(p.id)?.sport === sportFilter);
    }
  ...
  }, [products, debouncedSearch, sportFilter, profileByProduct]);
  ...
  const addProductLine = (product: Product, qty: number, profile?: any) => {
    ...
    if (existing) {
      ...
      setCart((prev) => prev.map((l) => l.id === existing.id
        ? { ...l, quantity: newQty, lineTotal: newQty * (l.useTeamPrice && l.teamPrice ? l.teamPrice : l.unitPrice) } : l));
    ...
    setCart((prev) => [...prev, {
      ...
      teamPrice: profile?.teamPrice,
      useTeamPrice: false,
      quantity: qty,
      baseStock: product.stock,
      lineTotal: qty * product.price,
      isTeamOrderable: profile?.isTeamOrderable || false,
      minTeamOrder: profile?.minTeamOrder,
    }]);
  ...
  const changeQty = (id: string, delta: number) => {
    setCart((prev) => prev.flatMap((l) => {
      ...
      const price = l.useTeamPrice && l.teamPrice ? l.teamPrice : l.unitPrice;
      return [{ ...l, quantity: nextQty, lineTotal: nextQty * price }];
    }));
  };
  ...
  const setQtyDirect = (id: string, qty: number) => {
    setCart((prev) => prev.flatMap((l) => {
      ...
      const price = l.useTeamPrice && l.teamPrice ? l.teamPrice : l.unitPrice;
      return [{ ...l, quantity: qty, lineTotal: qty * price }];
    }));
  };

  const toggleTeamPrice = (id: string) => {
    setCart((prev) => prev.map((l) => {
      if (l.id !== id || !l.teamPrice || !l.isTeamOrderable) return l;
      const use = !l.useTeamPrice;
      if (use && l.minTeamOrder && l.quantity < l.minTeamOrder) {
        toast.error(`Minimum team order: ${l.minTeamOrder} pieces`);
        return l;
      }
      const price = use && l.teamPrice ? l.teamPrice : l.unitPrice;
      return { ...l, useTeamPrice: use, lineTotal: l.quantity * price };
    }));
  };
  ...
  const checkoutMutation = useMutation({
    mutationFn: async (data: { paymentMethod: PaymentMethod; paidAmount: number }) => {
      ...
      const items = cart.map((l) => ({
        ...
        quantity: l.quantity,
        priceOverride: l.useTeamPrice && l.teamPrice ? l.teamPrice : l.unitPrice,
        note: l.useTeamPrice ? 'Team price applied' : undefined,
      }));
    ...
    onSuccess: (sale, vars) => {
      ...
      clearCart();
      queryClient.invalidateQueries({ queryKey: ['products-for-sports-pos'] });
    ...
    <>
      ...
      {lastSale && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeSuccessModal}>
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="relative px-6 py-8 bg-gradient-to-br from-emerald-500 to-teal-600 text-white text-center">
            ...
            <div className="p-4 grid grid-cols-2 gap-2">
              ...
              <button onClick={closeSuccessModal} className="h-14 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 font-extrabold text-white text-lg shadow-lg transition inline-flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
            <div className="px-5 py-4 bg-gradient-to-br from-emerald-600 to-teal-700 text-white flex items-center justify-between">
              ...
              <button onClick={() => setShowCustomerAdd(false)} className="h-11 w-11 rounded-2xl bg-white/15 hover:bg-white/25 flex items-center justify-center"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-5 space-y-3">
              <input autoFocus value={newCustomer.name} onChange={(e) => setNewCustomer({ ...newCustomer, name: e.target.value })}
                placeholder="Customer name"
                className="h-16 w-full rounded-2xl border-4 border-slate-200 px-4 text-xl font-bold focus:outline-none focus:border-emerald-500" />
              ...
              <button onClick={() => {
                ...
                className="w-full h-16 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-700 font-extrabold text-white text-xl shadow-lg transition disabled:opacity-50">
                Add Customer
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
            <div className="px-5 py-4 bg-gradient-to-br from-amber-600 to-orange-700 text-white flex items-center justify-between">
              ...
              <button onClick={() => setShowHeldCarts(false)} className="h-11 w-11 rounded-2xl bg-white/15 hover:bg-white/25 flex items-center justify-center"><X className="h-5 w-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {heldCarts.length === 0 ? (
                ...
                <div key={h.id} className="rounded-2xl border-2 border-slate-200 p-3 flex items-center gap-3">
                  <div className="h-12 w-12 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center shrink-0"><ShoppingCart className="h-6 w-6" /></div>
      ...
      <div className="min-h-[calc(100dvh-5rem)] lg:h-[calc(100dvh-7rem)] flex flex-col lg:grid lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_440px] gap-2 lg:gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h2 className="text-base sm:text-lg font-extrabold leading-none">🏏 Sports POS</h2>
                    {isOnline ? (
                      <div className="h-6 w-6 rounded-full bg-emerald-500/30 flex items-center justify-center"><Wifi className="h-3 w-3 text-emerald-200" /></div>
                    ) : (
                      <div className="h-6 w-6 rounded-full bg-amber-500/30 flex items-center justify-center"><WifiOff className="h-3 w-3 text-amber-200" /></div>
                    )}
                  </div>
                  <p className="text-[11px] sm:text-xs text-white/80 font-semibold mt-0.5 flex items-center gap-1 truncate">
                    <Store className="h-3 w-3 shrink-0" /><span className="truncate">{tenant?.name || 'Sports Shop'}</span>
                  </p>
                </div>
              </div>
          ...
          <div className="shrink-0 px-3 sm:px-4 py-2.5 bg-slate-50 border-b-2 border-slate-100 space-y-2">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  className="h-14 sm:h-16 w-full rounded-2xl border-4 border-slate-200 bg-white pl-11 sm:pl-14 pr-10 sm:pr-12 text-lg sm:text-xl font-bold focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-200 transition"
                  placeholder="Product name / SKU / sport..."
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
              <FilterChip active={!sportFilter} onClick={() => setSportFilter('')} label="All" emoji="🌐" />
              {[
                { v: 'Cricket', e: '🏏' },
                { v: 'Football', e: '⚽' },
                { v: 'Basketball', e: '🏀' },
                ...
                <FilterChip key={p.v} active={sportFilter === p.v}
                  onClick={() => setSportFilter(sportFilter === p.v ? '' : p.v)}
                  label={p.v} emoji={p.e} />
              ))}
            </div>
          </div>
              ...
              <>
                ...
                {hasMore && (
                  <button onClick={() => setVisibleCount((c) => c + 60)}
                    className="mt-3 w-full h-12 rounded-2xl bg-white border-4 border-slate-200 hover:border-emerald-400 text-slate-700 text-sm font-extrabold inline-flex items-center justify-center gap-2 transition">
        ...
        <CartPanel
          ...
          subtotal={subtotal} total={total} discountPct={discountPct} setDiscountPct={setDiscountPct}
          discountAmount={discountAmount} hidePrices={hidePrices}
          ...
          onToggleTeamPrice={toggleTeamPrice}
      ...
      {cart.length > 0 && !showMobileCart && (
        <div className="lg:hidden fixed bottom-4 inset-x-4 z-30">
          <button onClick={() => setShowMobileCart(true)}
            className="w-full h-16 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-2xl active:scale-[0.98] flex items-center justify-between px-5 transition">
  ...
  return (
    <button onClick={onClick}
      className={`shrink-0 h-9 sm:h-10 px-3 rounded-xl text-xs sm:text-sm font-extrabold inline-flex items-center gap-1.5 border-2 transition ${
        active ? 'bg-emerald-600 text-white border-emerald-600 shadow-md' : 'bg-white text-slate-700 border-slate-200 hover:border-emerald-300'}`}>
  ...
  return (
    <button onClick={onClick} disabled={out}
      ...
      <div className="aspect-square bg-slate-100 overflow-hidden relative">
        {img ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-50 to-teal-50">
            <Dumbbell className="h-12 w-12 text-emerald-300" />
          </div>
        )}
        ...
        {profile?.isProfessional && !out && <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded-md bg-blue-600 text-white text-[9px] font-extrabold shadow-lg inline-flex items-center gap-0.5"><Trophy className="h-2 w-2" /> PRO</div>}
        {p.isFeatured && !profile?.isProfessional && !out && <div className="absolute top-1.5 left-1.5 h-7 w-7 rounded-full bg-amber-500 flex items-center justify-center shadow-lg"><Star className="h-3.5 w-3.5 fill-white text-white" /></div>}
        {profile?.isTeamOrderable && !out && (
          <div className="absolute bottom-1.5 left-1.5 h-6 px-1.5 rounded-md bg-emerald-600 flex items-center gap-1 text-white text-[9px] font-extrabold shadow-lg">
      ...
      <div className="p-2 sm:p-3">
        ...
        {profile?.sport && (
          <div className="text-[9px] font-extrabold uppercase text-emerald-700 mt-0.5">{profile.sport}</div>
        )}
        <div className="mt-1.5 sm:mt-2 flex items-end justify-between gap-1">
          <div>
            ...
            {profile?.teamPrice && (
              <div className="text-[9px] font-extrabold text-violet-700 mt-0.5">Team: {formatPKR(profile.teamPrice)}</div>
            )}
          </div>
...
function CartPanel({
  ...
  onSetQty, onRemove, onToggleTeamPrice, onCheckout, canCheckout,
}: any) {
  ...
  return (
    <aside className={containerClass}>
      <div className="shrink-0 relative overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 text-white px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex gap-1.5 shrink-0">
            {cart.length > 0 && (
              <>
                <button onClick={onHold} className="h-10 sm:h-12 px-2.5 rounded-2xl bg-white/15 hover:bg-amber-500/50 text-white text-xs sm:text-sm font-extrabold border-2 border-white/20 transition inline-flex items-center gap-1">
                  <Pause className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Hold
                </button>
                <button onClick={onClear} className="h-10 sm:h-12 px-2.5 rounded-2xl bg-white/15 hover:bg-rose-500/50 text-white text-xs sm:text-sm font-extrabold border-2 border-white/20 transition">Clear</button>
              </>
            )}
            {isMobile && (
              <button onClick={onCloseMobile} className="h-10 w-10 sm:h-12 sm:w-12 rounded-2xl bg-white/15 hover:bg-white/25 flex items-center justify-center border-2 border-white/20 transition"><X className="h-5 w-5" /></button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 px-3 py-2.5 border-b-2 border-slate-100 bg-slate-50">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <User className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-600 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <select value={customerId} onChange={(e) => setCustomerId(e.target.value)}
              className="h-12 sm:h-14 w-full rounded-2xl border-4 border-slate-200 bg-white pl-10 sm:pl-11 pr-9 text-sm sm:text-base font-bold focus:outline-none focus:border-emerald-500 appearance-none">
              <option value="">Walk-in Customer</option>
              {customers.map((c: any) => (
                <option key={c.id} value={c.id}>{c.name}{c.balance > 0 ? ` • Balance ${formatPKR(c.balance)}` : ''}</option>
              ))}
            </select>
          ...
          <button onClick={onAddCustomer}
            className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white flex items-center justify-center shadow-md shrink-0 transition">
      ...
      <div className="flex-1 overflow-y-auto p-2.5 sm:p-3 space-y-2 bg-slate-50/50 min-h-0">
        {cart.length === 0 ? (
          <div className="rounded-3xl bg-white border-4 border-dashed border-slate-200 p-8 sm:p-10 text-center">
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-semibold mt-1">Scan or tap product</p>
          </div>
        ) : (
          cart.map((l: CartLine) => (
            <div key={l.id} className={`rounded-2xl bg-white border-4 p-2.5 sm:p-3 shadow-sm ${l.useTeamPrice ? 'border-violet-300' : 'border-slate-200'}`}>
              <div className="flex items-start gap-2.5 sm:gap-3">
                <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-xl overflow-hidden shrink-0 bg-slate-100 flex items-center justify-center">
                  {l.image ? <img src={l.image} alt="" className="w-full h-full object-cover" /> : <Dumbbell className="h-6 w-6 text-slate-400" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-extrabold text-sm sm:text-base text-slate-900 leading-tight line-clamp-2">{l.name}</div>
                  {l.sport && <div className="text-[10px] font-bold text-emerald-700 mt-0.5 uppercase">{l.sport}</div>}
                  <div className="text-xs sm:text-sm font-bold text-emerald-700 mt-0.5">
                    {formatPKR(l.useTeamPrice && l.teamPrice ? l.teamPrice : l.unitPrice)}
                    {l.useTeamPrice && (
                      <span className="ml-2 px-1.5 py-0.5 rounded bg-violet-100 text-violet-800 text-[9px] font-extrabold uppercase">Team price</span>
                    )}
                  </div>
                  {l.isTeamOrderable && l.teamPrice && (
                    <button onClick={() => onToggleTeamPrice(l.id)}
                      className={`mt-1 text-[10px] px-2 py-1 rounded-lg font-extrabold inline-flex items-center gap-1 transition ${
                        l.useTeamPrice ? 'bg-violet-600 text-white' : 'bg-violet-100 text-violet-800 hover:bg-violet-200'}`}>
                      <Users className="h-2.5 w-2.5" />
                      {l.useTeamPrice ? 'Use retail price' : `Use team price (min ${l.minTeamOrder || 2})`}
                    </button>
                  )}
                </div>
              ...
              <div className="mt-2.5 sm:mt-3 flex items-center justify-between gap-2">
                <div className="inline-flex items-center bg-slate-100 rounded-2xl overflow-hidden border-2 border-slate-200">
                  <button onClick={() => onChangeQty(l.id, -1)} className="h-12 sm:h-14 w-12 sm:w-14 hover:bg-slate-200 flex items-center justify-center transition"><Minus className="h-5 w-5 sm:h-6 sm:w-6 text-slate-700" /></button>
                  <input type="number" value={l.quantity} onChange={(e) => onSetQty(l.id, Number(e.target.value))} onFocus={(e) => e.target.select()}
                  ...
                  <button onClick={() => onChangeQty(l.id, 1)}
                    className="h-12 sm:h-14 w-12 sm:w-14 bg-emerald-600 hover:bg-emerald-700 text-white flex items-center justify-center transition">
      ...
      {cart.length > 0 && (
        <div className="shrink-0 p-2.5 sm:p-3 border-t-4 border-slate-100 bg-white space-y-2">
          ...
          <button onClick={onCheckout} disabled={!canCheckout}
            className="w-full h-[76px] sm:h-[88px] rounded-3xl font-extrabold text-white shadow-2xl transition-all active:scale-[0.98] bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-between px-5 sm:px-6">
            <div className="text-left">
              <div className="text-[10px] sm:text-xs uppercase font-extrabold text-white/80 tracking-wider">Checkout <span className="hidden sm:inline">(F9)</span></div>
