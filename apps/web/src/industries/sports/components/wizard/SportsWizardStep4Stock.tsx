...
import type { SportsWizardBasic, SportsWizardVariant, SportsWizardStock } from '../../hooks/useSportsWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<SportsWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<SportsWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<SportsWizardStock>) => void;
  errors: string[];
}
...
const VARIANT_PRESETS = [
  { group: 'Size (Apparel)', items: ['XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL'] },
  { group: 'Size (Shoes UK)', items: ['UK 4', 'UK 5', 'UK 6', 'UK 7', 'UK 8', 'UK 9', 'UK 10', 'UK 11'] },
  { group: 'Bat Size', items: ['Full Size', 'Size 6', 'Size 5', 'Size 4', 'Junior'] },
  { group: 'Color', items: ['Red', 'Blue', 'Green', 'Black', 'White', 'Yellow'] },
  { group: 'Weight', items: ['5kg', '10kg', '15kg', '20kg', '25kg', '30kg'] },
];

export function SportsWizardStep4Stock({
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-white border-2 border-emerald-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-extrabold text-emerald-900">Stock entry — two ways</h3>
          <p className="text-xs text-emerald-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> one quantity (bats, balls, single-size items).
            <strong> Variants:</strong> different sizes/colors each with own stock (jerseys, shoes).
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        className={['w-full rounded-2xl border-2 p-4 text-left transition',
          hasVariants ? 'border-violet-500 bg-violet-50 shadow-md' : 'border-slate-200 bg-white hover:border-violet-300'].join(' ')}>
        <div className="flex items-center gap-3">
          <div className={['h-12 w-12 rounded-xl flex items-center justify-center shrink-0',
            hasVariants ? 'bg-violet-500 text-white' : 'bg-violet-100 text-violet-700'].join(' ')}>
            <Boxes className="h-6 w-6" />
          </div>
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Size / Color Variants</div>
            <div className="text-xs text-slate-600 font-semibold">S/M/L/XL, Red/Blue/Green, 5kg/10kg/15kg</div>
          </div>
          {hasVariants ? <ToggleRight className="h-7 w-7 text-violet-600" /> : <ToggleLeft className="h-7 w-7 text-slate-400" />}
        </div>
      </button>
      ...
      {!hasVariants && (
        <section className="rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 to-white p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-600 mb-1.5">
                Rack / Shelf <span className="text-slate-400 normal-case font-bold">(optional)</span>
              </label>
              <div className="relative">
                ...
                <input value={stock.rackNumber} onChange={(e) => onUpdateStock({ rackNumber: e.target.value })}
                  ...
                  className="h-12 w-full rounded-xl border-2 border-slate-200 pl-10 pr-3 text-sm font-bold focus:outline-none focus:border-emerald-500" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border-2 border-violet-300 bg-gradient-to-br from-violet-50 to-white p-5 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-violet-700 font-semibold">Sizes, colors, weights — each with own stock</p>
            </div>
          ...
          <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end">
            <Input label="Custom variant name" value={vName}
              ...
              placeholder="e.g. XL Red, 25kg Rubber Coated" />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div>
                      ...
                      <input type="number" step="1" value={v.stock}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-sm font-extrabold tabular-nums focus:outline-none focus:border-emerald-500" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={v.sku ?? ''} onChange={(e) => onUpdateVariant(v.tempId, { sku: e.target.value })}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-xs font-mono font-bold focus:outline-none focus:border-emerald-500" />
                    </div>
                    <div>
