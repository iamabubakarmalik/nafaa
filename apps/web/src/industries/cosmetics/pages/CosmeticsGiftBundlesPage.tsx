...
import {
  Gift, Plus, Search, X, Edit3, Trash2, Star, RefreshCw,
  Sparkles, Package, Percent, TrendingUp, Copy, Award,
} from 'lucide-react';
...
const OCCASIONS = ['All', 'Eid', 'Christmas', 'Birthday', 'Anniversary', 'Valentine', 'Mother\'s Day', 'Wedding', 'Corporate'];

export default function CosmeticsGiftBundlesPage() {
  ...
  const { data: bundles = [], isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['cosmetics-bundles-list', occasion, featuredOnly, activeOnly],
    queryFn: () => cosmeticsGiftBundlesApi.list({
  ...
  const stats = useMemo(() => ({
    ...
    totalSold: bundles.reduce((s, b) => s + Number(b.totalSold || 0), 0),
    totalSavings: bundles.reduce((s, b) => s + Number(b.savings || 0), 0),
    avgSavings: bundles.length > 0 ?
      (bundles.reduce((s, b) => s + Number(b.savings || 0), 0) / bundles.length) : 0,
  }), [bundles]);

  const remove = useMutation({
    mutationFn: (id: string) => cosmeticsGiftBundlesApi.remove(id),
    onSuccess: () => {
  ...
  const duplicate = useMutation({
    mutationFn: async (bundle: any) =>
      cosmeticsGiftBundlesApi.create({
        ...
        hasGiftWrap: bundle.hasGiftWrap,
        giftWrapCost: bundle.giftWrapCost,
        includesGreetingCard: bundle.includesGreetingCard,
        isActive: false,
        isFeatured: false,
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Gift className="h-3.5 w-3.5 text-amber-300" /> Gift Bundles
            </div>
            <h1 className="mt-3 text-3xl sm:text-4xl font-extrabold leading-tight">🎁 Gift Bundles</h1>
            <p className="mt-2 text-sm text-white/80">
              {stats.total} bundles • {stats.totalSold} sold • Avg savings{' '}
              <strong className="text-emerald-300">{formatPKR(stats.avgSavings)}</strong>
            </p>
          </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi icon={Gift} label="Total Bundles" value={stats.total} sub={`${stats.featured} featured`} tone="fuchsia" />
        <Kpi icon={TrendingUp} label="Sold" value={stats.totalSold} sub="Total units" tone="emerald" />
        <Kpi icon={Percent} label="Customer Savings" value={formatPKR(stats.totalSavings)} sub="All bundles" tone="amber" />
        <Kpi icon={Award} label="Avg Savings" value={formatPKR(stats.avgSavings)} sub="Per bundle" tone="pink" />
      </section>
      ...
      <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-3">
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="h-12 w-full rounded-2xl border-2 border-slate-200 bg-white pl-11 pr-10 text-sm font-semibold focus:outline-none focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-200" />
          {search && (
          ...
          {OCCASIONS.map((o) => (
            <button key={o} onClick={() => setOccasion(o)}
              className={`shrink-0 h-9 px-3 rounded-xl text-xs font-extrabold border-2 transition ${
                occasion === o ? 'bg-fuchsia-600 text-white border-fuchsia-600 shadow-md' : 'bg-white text-slate-700 border-slate-200 hover:border-fuchsia-300'}`}>
              {o}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <button onClick={() => setFeaturedOnly(!featuredOnly)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${
              featuredOnly ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-700'}`}>
            <Star className={`h-3.5 w-3.5 ${featuredOnly ? 'fill-current' : ''}`} /> Featured Only
          </button>
          <button onClick={() => setActiveOnly(!activeOnly)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold ${
              activeOnly ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-700'}`}>
            Active Only
          </button>
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          ...
          <h3 className="text-lg font-extrabold text-slate-900">No bundles yet</h3>
          <p className="text-sm text-slate-500 font-semibold mt-1">
            Create bundles for Eid, Birthdays, Valentine's Day
          </p>
          <Link to="/cosmetics/bundles/new">
            <Button className="mt-4 bg-gradient-to-r from-fuchsia-600 to-pink-700">
  ...
  return (
    <div className={`group relative rounded-2xl bg-white border-2 shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition overflow-hidden ${
      ...
      <div className="relative aspect-video bg-gradient-to-br from-fuchsia-500 via-pink-600 to-rose-600 overflow-hidden">
        {b.imageUrl ? (
          <img src={b.imageUrl} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
        ) : (
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="rounded-xl bg-slate-50 border border-slate-100 p-2">
          <div className="text-[10px] uppercase font-extrabold text-slate-500 mb-1">
            {b.items?.length || 0} items
          </div>
          <div className="space-y-0.5">
            {(b.items ?? []).slice(0, 2).map((item: any, i: number) => (
              <div key={i} className="flex items-center gap-1.5 text-[11px]">
                <Sparkles className="h-2.5 w-2.5 text-slate-400 shrink-0" />
            ...
            {(b.items?.length || 0) > 2 && (
              <div className="text-[10px] font-extrabold text-fuchsia-700">+ {b.items.length - 2} more</div>
            )}
          </div>
        </div>

        {(b.hasGiftWrap || b.includesGreetingCard) && (
          <div className="flex flex-wrap gap-1">
            {b.hasGiftWrap && <span className="px-1.5 py-0.5 rounded bg-pink-100 text-pink-700 text-[9px] font-extrabold">🎀 Gift Wrap</span>}
            {b.includesGreetingCard && <span className="px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 text-[9px] font-extrabold">💌 Card</span>}
          </div>
        )}
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          <div className="text-right">
            ...
            <div className="text-lg font-extrabold text-slate-900 tabular-nums">{b.totalSold || 0}</div>
          </div>
        </div>

        <div className="flex gap-1.5 pt-2 border-t border-slate-100">
          <Link to={`/cosmetics/bundles/${b.id}/edit`}
            className="flex-1 h-9 rounded-lg bg-fuchsia-50 hover:bg-fuchsia-100 text-fuchsia-700 text-xs font-extrabold inline-flex items-center justify-center gap-1">
