...
import {
  Sparkles, Plus, Search, X, RefreshCw, Download, Grid3x3, List,
  Package, AlertTriangle, DollarSign, Eye, Edit3, Trash2,
  CheckCircle2, Star, Award, Crown, PackageX, Palette,
} from 'lucide-react';
...
const CATEGORIES = [
  'FOUNDATION', 'CONCEALER', 'LIPSTICK', 'MASCARA', 'EYESHADOW',
  'FACE_WASH', 'MOISTURIZER', 'SERUM', 'SUNSCREEN',
  'PERFUME', 'EAU_DE_TOILETTE', 'BODY_MIST', 'ATTAR',
...
export default function CosmeticsProductsPage() {
  ...
  const { data: brands = [] } = useQuery({
    queryKey: ['cosmetics-brands'],
    queryFn: () => cosmeticsBrandsApi.list({ active: true }),
  });
  ...
  const stats = useMemo(() => ({
    ...
    halalCount: (profiles as any[]).filter((p) => p.isHalalCertified).length,
  }), [products, profiles]);
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((p) => {
      ...
      return (p.name || '').toLowerCase().includes(q) ||
        ...
        (pr?.shadeCode || '').toLowerCase().includes(q);
    });
    ...
    if (halalOnly) list = list.filter((p) => pmap.get(p.id)?.isHalalCertified);
    if (crueltyFreeOnly) list = list.filter((p) => pmap.get(p.id)?.isCrueltyFree);
    if (veganOnly) list = list.filter((p) => pmap.get(p.id)?.isVegan);
    ...
    return list;
  }, [products, search, brandId, categoryType, halalOnly, crueltyFreeOnly, veganOnly, stockFilter, sortKey, pmap]);
  ...
  const exportCSV = () => {
    ...
    const head = ['Name', 'SKU', 'Brand', 'Category', 'Shade', 'Cost', 'Retail', 'Stock', 'Halal', 'CF', 'Vegan'];
    const rows = filtered.map((p) => {
      const pr = pmap.get(p.id);
      const brand = (brands as any[]).find((b) => b.id === pr?.brandId);
      return [p.name, p.sku || '', brand?.name || '', pr?.categoryType || '',
        pr?.shadeName || '', Number(p.costPrice || 0).toFixed(2), Number(p.price || 0).toFixed(2),
        Number(p.stock || 0), pr?.isHalalCertified ? 'Yes' : '', pr?.isCrueltyFree ? 'Yes' : '',
        pr?.isVegan ? 'Yes' : ''];
    });
    const csv = [head, ...rows].map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8' }));
    const a = document.createElement('a');
    a.href = url; a.download = `cosmetics-products-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="space-y-5 pb-6">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Sparkles className="h-3.5 w-3.5 text-amber-300" /> Cosmetics Catalogue
            </div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <PrivacyToggle />
            <Link to="/cosmetics-products/new"
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <Kpi icon={Award} label="Halal Certified" value={stats.halalCount} sub="products" tone="teal"
      ...
      <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-3">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <button onClick={() => setHalalOnly((v) => !v)}
          ...
          <button onClick={() => setCrueltyFreeOnly((v) => !v)}
          ...
          <button onClick={() => setVeganOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${veganOnly ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-200 text-slate-700'}`}>
            🌱 Vegan
          </button>
...
Ye irreversible hai. Continue?`);
      ...
      {isLoading ? (
        ...
        <section className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          ...
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add your first cosmetics product</p>
          <Link to="/cosmetics-products/new">
            <Button className="mt-5 bg-gradient-to-r from-pink-600 to-rose-700"><Plus className="h-4 w-4" /> Add Product</Button>
          </Link>
        </section>
      ) : view === 'grid' ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b-2 border-slate-200">
              <tr>
                ...
                <Th>Product</Th><Th>Brand</Th><Th>Shade</Th>
                <Th className="text-right">Retail</Th><Th className="text-right">Stock</Th>
                <Th className="text-center">Certs</Th><Th className="text-right">Actions</Th>
              </tr>
            </thead>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-pink-50/40">
                    ...
                    <td className="px-3 py-2.5">
                      <Link to={`/cosmetics-products/${p.id}`} className="flex items-center gap-2.5 group">
                    ...
                    <td className="px-3 py-2.5">
                      {pr?.shadeName ? (
                        <div className="flex items-center gap-1.5">
                          {pr.shadeHex && <span className="h-4 w-4 rounded-full border border-slate-200" style={{ backgroundColor: pr.shadeHex }} />}
                          <span className="text-xs font-bold text-slate-700">{pr.shadeName}</span>
                        </div>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/cosmetics-products/${p.id}`} className="h-8 w-8 rounded-lg bg-pink-50 hover:bg-pink-100 text-pink-700 flex items-center justify-center">
                          <Eye className="h-3.5 w-3.5" />
                        </Link>
                        <Link to={`/cosmetics-products/${p.id}/edit`} className="h-8 w-8 rounded-lg bg-violet-50 hover:bg-violet-100 text-violet-700 flex items-center justify-center">
...
function Card({ p, pr, brands, sel, onToggle }: any) {
  ...
  return (
    <div className={`group relative rounded-2xl bg-white border-2 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 ${
      ...
      <Link to={`/cosmetics-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 relative overflow-hidden">
          {p.images?.[0]?.url
            ...
            : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-pink-50 to-rose-50"><Sparkles className="h-10 w-10 text-pink-300" /></div>}
          {pr?.isLimitedEdition && <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-md bg-purple-600 text-white text-[9px] font-extrabold inline-flex items-center gap-0.5"><Crown className="h-2.5 w-2.5" /> LIMITED</div>}
          {pr?.isViral && !pr?.isLimitedEdition && <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-md bg-rose-600 text-white text-[9px] font-extrabold">🔥 VIRAL</div>}
          {pr?.isFeatured && !pr?.isLimitedEdition && !pr?.isViral && (
          ...
          <div className="absolute bottom-1.5 left-1.5 flex gap-0.5">
            {pr?.isHalalCertified && <span className="h-6 w-6 rounded-full bg-emerald-500 flex items-center justify-center text-white text-xs shadow">🕌</span>}
            {pr?.isCrueltyFree && <span className="h-6 w-6 rounded-full bg-pink-500 flex items-center justify-center text-white text-xs shadow">🐰</span>}
          </div>
          {(isOut || isLow) && (
        ...
        <div className="p-2.5">
          ...
          {brand && <div className="mt-1 text-[9px] font-extrabold uppercase text-pink-700 truncate">{brand.name}</div>}
          {pr?.shadeName && (
            <div className="mt-1 flex items-center gap-1">
              {pr.shadeHex && <span className="h-3 w-3 rounded-full border border-slate-200 shrink-0" style={{ backgroundColor: pr.shadeHex }} />}
              <span className="text-[10px] font-bold text-slate-600 truncate">{pr.shadeCode ? `${pr.shadeCode} · ` : ''}{pr.shadeName}</span>
            </div>
          )}
...
function Kpi({ icon: Icon, label, value, sub, tone, onClick }: any) {
  const tones: Record<string, string> = {
    pink: 'from-pink-500 to-rose-700', emerald: 'from-emerald-500 to-emerald-700',
    teal: 'from-teal-500 to-emerald-700', amber: 'from-amber-500 to-orange-600', rose: 'from-rose-500 to-rose-700',
  };
