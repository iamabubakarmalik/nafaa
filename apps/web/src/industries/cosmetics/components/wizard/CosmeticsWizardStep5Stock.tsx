...
import {
  ...
  Boxes, MapPin, Sparkles, CheckCircle2, Palette,
} from 'lucide-react';
import { Input } from '@core/ui/Input';
import { formatPKRFull } from '@core/lib/format';
import type {
  CosmeticsWizardBasic, CosmeticsWizardVariant, CosmeticsWizardStock,
} from '../../hooks/useCosmeticsWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<CosmeticsWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<CosmeticsWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<CosmeticsWizardStock>) => void;
  errors: string[];
}
...
const VARIANT_PRESETS = [
  { group: 'Foundation Shades', items: ['110 Porcelain', '120 Classic Ivory', '128 Warm Nude', '220 Natural Beige', '228 Soft Tan', '310 Sun Beige', '332 Golden Caramel', '355 Coconut', '365 Espresso'] },
  { group: 'Lipstick Shades', items: ['Ruby Woo', 'Velvet Teddy', 'Rebel', 'Diva', 'Russian Red', 'Nude Rose', 'Berry Bold', 'Coral Pop', 'Pink Nouveau'] },
  { group: 'Size', items: ['30ml', '50ml', '75ml', '100ml', '150ml', '200ml', '250ml', '500ml'] },
  { group: 'Volume (Perfume)', items: ['30ml EDT', '50ml EDT', '100ml EDT', '30ml EDP', '50ml EDP', '100ml EDP', '5ml Sample'] },
];

const SHADE_COLORS: Record<string, string> = {
  'porcelain': '#f5e6d3', 'ivory': '#f0d6b0', 'warm nude': '#e8c9a0',
  'natural beige': '#dfa0a0', 'soft tan': '#b07b4a', 'sun beige': '#c99872',
  'golden caramel': '#a67440', 'coconut': '#8b5a3c', 'espresso': '#5c3a2e',
  'ruby': '#c81d3f', 'velvet teddy': '#b47d6a', 'rebel': '#843150', 'diva': '#650b20',
  'russian red': '#a11a2b', 'nude rose': '#dfa0a0', 'berry': '#8b3a5f',
  'coral': '#f88379', 'pink': '#e91e63',
};

const guessShadeColor = (name: string): string | undefined => {
  const lower = name.toLowerCase();
  for (const [key, hex] of Object.entries(SHADE_COLORS)) {
    if (lower.includes(key)) return hex;
  }
  return undefined;
};

export function CosmeticsWizardStep5Stock({
  ...
  const addVariant = (name: string) => {
    ...
    const shadeHex = guessShadeColor(n);
    onAddVariant({ name: n, stock: 0, lowStockAlert: 3, shadeHex });
    setVName('');
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-white border-2 border-pink-200 p-4 flex items-start gap-3">
        <div className="h-11 w-11 rounded-xl bg-pink-600 text-white flex items-center justify-center shadow-md shrink-0">
          <Package className="h-5 w-5" />
        </div>
        <div className="flex-1">
          <h3 className="font-extrabold text-pink-900">Stock entry — two ways</h3>
          <p className="text-xs text-pink-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> one quantity (single shade / one size).
            <strong> Variants:</strong> multiple shades (Foundation shade 110, 120, 220…) or sizes (30ml, 50ml, 100ml) — each with its own stock and price.
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        className={['w-full rounded-2xl border-2 p-4 text-left transition',
          hasVariants ? 'border-pink-500 bg-pink-50 shadow-md' : 'border-slate-200 bg-white hover:border-pink-300'].join(' ')}>
        <div className="flex items-center gap-3">
          <div className={['h-12 w-12 rounded-xl flex items-center justify-center shrink-0',
            hasVariants ? 'bg-pink-500 text-white' : 'bg-pink-100 text-pink-700'].join(' ')}>
            <Palette className="h-6 w-6" />
          </div>
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Shades / Sizes</div>
            <div className="text-xs text-slate-600 font-semibold">Foundation shades, lipstick colors, perfume sizes</div>
          </div>
          {hasVariants ? <ToggleRight className="h-7 w-7 text-pink-600" /> : <ToggleLeft className="h-7 w-7 text-slate-400" />}
        </div>
      </button>
      ...
      {!hasVariants && (
        <section className="rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 to-white p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-600 mb-1.5">
                Shelf / Rack <span className="text-slate-400 normal-case font-bold">(optional)</span>
              </label>
              <div className="relative">
                ...
                <input value={stock.rackNumber} onChange={(e) => onUpdateStock({ rackNumber: e.target.value })}
                  placeholder="Shelf-A, Counter-2"
                  className="h-12 w-full rounded-xl border-2 border-slate-200 pl-10 pr-3 text-sm font-bold focus:outline-none focus:border-pink-500" />
              </div>
            </div>
          </div>
        </section>
      ...
      {hasVariants && (
        <section className="rounded-2xl border-2 border-pink-300 bg-gradient-to-br from-pink-50 to-white p-5 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1">
              <h3 className="font-extrabold text-pink-900 text-base">Shades / Sizes</h3>
              <p className="text-xs text-pink-700 font-semibold">Each variant keeps its own stock and price</p>
            </div>
            <span className="px-2.5 py-1 rounded-lg bg-pink-100 text-pink-800 text-[10px] font-extrabold">
              {variants.length} variants • {totalVariantStock} units
            </span>
          </div>

          <div className="rounded-xl bg-white border-2 border-pink-200 p-3 space-y-2 max-h-64 overflow-y-auto">
            ...
            {VARIANT_PRESETS.map((grp) => (
              <div key={grp.group} className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-extrabold uppercase text-slate-500 min-w-[100px]">{grp.group}:</span>
                {grp.items.map((it) => {
                  ...
                  return (
                    <button key={it} type="button" disabled={ex} onClick={() => addVariant(it)}
                      className={['px-2.5 py-1 rounded-lg border-2 text-[11px] font-extrabold transition disabled:cursor-not-allowed',
                        ex ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : 'border-slate-200 bg-white text-slate-700 hover:border-pink-400'].join(' ')}>
                      {ex ? '✓ ' : '+ '}{it}
                    </button>
                  );
                })}
          ...
          <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end">
            <Input label="Custom shade / size" value={vName}
              onChange={(e) => setVName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && addVariant(vName)}
              placeholder="e.g. 240 Golden Beige, 50ml" />
            <button type="button" onClick={() => addVariant(vName)} disabled={!vName.trim()}
              className="h-11 px-5 rounded-xl bg-pink-600 hover:bg-pink-700 text-white font-extrabold text-sm inline-flex items-center gap-1 disabled:opacity-50 shadow-md">
          ...
          {variants.length > 0 ? (
            <div className="space-y-2">
              {variants.map((v) => (
                <div key={v.tempId} className="rounded-xl border-2 border-slate-200 bg-white p-3 space-y-2">
                  <div className="flex items-center gap-3">
                    {v.shadeHex && (
                      <div className="h-10 w-10 rounded-lg border-2 border-slate-200 shrink-0 shadow-sm"
                        style={{ backgroundColor: v.shadeHex }} />
                    )}
                    {!v.shadeHex && (
                      <div className="h-10 w-10 rounded-lg bg-pink-100 text-pink-700 flex items-center justify-center shrink-0">
                        <Palette className="h-5 w-5" />
                      </div>
                    )}
                    ...
                    <input type="color" value={v.shadeHex || '#dfa0a0'}
                      onChange={(e) => onUpdateVariant(v.tempId, { shadeHex: e.target.value })}
                      className="h-8 w-10 rounded-lg border-2 border-slate-200 cursor-pointer shrink-0" />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div>
                      ...
                      <input type="number" step="1" value={v.stock}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-sm font-extrabold tabular-nums focus:outline-none focus:border-pink-500" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={v.sku ?? ''} onChange={(e) => onUpdateVariant(v.tempId, { sku: e.target.value })}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-xs font-mono font-bold focus:outline-none focus:border-pink-500" />
                    </div>
                    <div>
            ...
            <div className="rounded-xl border-2 border-dashed border-pink-300 bg-white p-6 text-center">
              <Palette className="h-10 w-10 text-pink-400 mx-auto mb-2" />
      ...
      {displayStock > 0 && sale > 0 && (
        <section className="rounded-2xl bg-gradient-to-br from-slate-950 to-pink-900 text-white p-5 shadow-xl">
