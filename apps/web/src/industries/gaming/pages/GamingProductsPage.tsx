...
import {
  Gamepad2, Plus, Search, X, RefreshCw, Download, Grid3x3, List,
  Package, AlertTriangle, DollarSign, Eye, Edit3, Trash2,
  CheckCircle2, Star, Timer, Rocket, PackageX,
} from 'lucide-react';
...
const PLATFORMS = [
  { v: 'PS5', l: 'PS5', e: '🎮' }, { v: 'PS4', l: 'PS4', e: '🎮' },
  { v: 'XBOX_SERIES_X', l: 'Xbox X', e: '🟩' }, { v: 'NINTENDO_SWITCH', l: 'Switch', e: '🔴' },
  { v: 'PC', l: 'PC', e: '🖥️' }, { v: 'MULTI', l: 'Multi', e: '🌐' },
];

const CATEGORIES = [
  'GAME_DISC', 'GAME_DIGITAL', 'CONSOLE_PS5', 'CONSOLE_XBOX_SERIES',
  'CONTROLLER', 'HEADSET_GAMING', 'GPU', 'CPU', 'MONITOR_GAMING', 'CHAIR_GAMING',
];

export default function GamingProductsPage() {
  ...
  const stats = useMemo(() => ({
    ...
    rentable: (profiles as any[]).filter((p) => p.isRentable).length,
  }), [products, profiles]);
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((p) => {
      ...
      return (p.name || '').toLowerCase().includes(q) || (p.sku || '').toLowerCase().includes(q) ||
        (p.barcode || '').toLowerCase().includes(q) || (pr?.publisher || '').toLowerCase().includes(q);
    });
    if (platform !== 'all') list = list.filter((p) => pmap.get(p.id)?.platform === platform);
    if (categoryType !== 'all') list = list.filter((p) => pmap.get(p.id)?.categoryType === categoryType);
    if (condition !== 'all') list = list.filter((p) => pmap.get(p.id)?.conditionType === condition);
    if (rentableOnly) list = list.filter((p) => pmap.get(p.id)?.isRentable);
    if (preOrderOnly) list = list.filter((p) => pmap.get(p.id)?.isPreOrder);
    ...
    return list;
  }, [products, search, platform, categoryType, condition, rentableOnly, preOrderOnly, stockFilter, sortKey, pmap]);
  ...
  const exportCSV = () => {
    ...
    const head = ['Name', 'SKU', 'Platform', 'Category', 'Condition', 'Publisher', 'Cost', 'Retail', 'Stock', 'Rentable', 'Rent/Day'];
    const rows = filtered.map((p) => {
      const pr = pmap.get(p.id);
      return [p.name, p.sku || '', pr?.platform || '', pr?.categoryType || '', pr?.conditionType || '',
        pr?.publisher || '', Number(p.costPrice || 0).toFixed(2), Number(p.price || 0).toFixed(2),
        Number(p.stock || 0), pr?.isRentable ? 'Yes' : 'No', pr?.rentalPricePerDay || ''];
    });
    const csv = [head, ...rows].map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8' }));
    const a = document.createElement('a');
    a.href = url; a.download = `gaming-products-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="space-y-5 pb-6">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Gamepad2 className="h-3.5 w-3.5 text-amber-300" /> Gaming Catalogue
            </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Kpi icon={Package} label="Products" value={stats.total} sub={`${stats.totalStock} units`} tone="violet" />
        <Kpi icon={DollarSign} label="Stock Value" value={formatPKR(stats.stockValue)} sub="Retail" tone="emerald" />
        <Kpi icon={Timer} label="Rentable" value={stats.rentable} sub="lendable" tone="blue" onClick={() => setRentableOnly((v) => !v)} />
        ...
        <div className="flex gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input autoFocus value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Title, SKU, barcode, publisher..."
          ...
          <button onClick={exportCSV}
            className="h-12 px-4 rounded-2xl border-2 border-slate-200 hover:border-violet-300 text-sm font-bold text-slate-700 inline-flex items-center gap-1.5">
            <Download className="h-4 w-4" /> Export
          </button>
        </div>

        <div className="flex gap-1.5 overflow-x-auto pb-1">
          <Chip active={platform === 'all'} onClick={() => setPlatform('all')} label="All" emoji="🌐" />
          {PLATFORMS.map((p) => (
            <Chip key={p.v} active={platform === p.v} onClick={() => setPlatform(platform === p.v ? 'all' : p.v)} label={p.l} emoji={p.e} />
          ))}
        </div>

        <div className="flex gap-2 flex-wrap items-center">
          <select value={categoryType} onChange={(e) => setCategoryType(e.target.value)}
            className="h-10 rounded-xl border-2 border-slate-200 bg-white px-3 text-xs font-bold focus:outline-none focus:border-violet-500">
          ...
          <button onClick={() => setRentableOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${rentableOnly ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-slate-200 text-slate-700'}`}>
            <Timer className="h-3.5 w-3.5" /> Rentable
          </button>
          <button onClick={() => setPreOrderOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${preOrderOnly ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-700'}`}>
            <Rocket className="h-3.5 w-3.5" /> Pre-order
          </button>
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
            {(['all', 'in', 'low', 'out'] as StockFilter[]).map((v) => (
              <button key={v} onClick={() => setStockFilter(v)}
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold ${stockFilter === v ? 'bg-violet-600 text-white' : 'text-slate-600'}`}>
...
Ye irreversible hai. Continue?`);
      ...
      {isLoading ? (
        ...
        <section className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          <div className="mx-auto h-20 w-20 rounded-3xl bg-gradient-to-br from-violet-100 to-fuchsia-200 flex items-center justify-center">
            <Gamepad2 className="h-9 w-9 text-violet-600" />
          </div>
          <h3 className="mt-5 text-xl font-extrabold text-slate-900">No products found</h3>
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add your first game, console or accessory</p>
          <Link to="/gaming-products/new">
            <Button className="mt-5 bg-gradient-to-r from-violet-600 to-fuchsia-700"><Plus className="h-4 w-4" /> Add Product</Button>
          </Link>
        </section>
      ) : view === 'grid' ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b-2 border-slate-200">
              <tr>
                ...
                <Th>Product</Th><Th>Platform</Th><Th>Category</Th>
                <Th className="text-right">Retail</Th><Th className="text-right">Stock</Th>
                <Th className="text-center">Rental</Th><Th className="text-right">Actions</Th>
              </tr>
            </thead>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-violet-50/40">
                    ...
                    <td className="px-3 py-2.5">
                      <Link to={`/gaming-products/${p.id}`} className="flex items-center gap-2.5 group">
                        ...
                        <div className="min-w-0">
                          <div className="font-extrabold text-slate-900 text-sm truncate group-hover:text-violet-700">{p.name}</div>
                          <div className="text-[10px] font-mono text-slate-500 truncate">{p.sku || pr?.publisher || '—'}</div>
                        </div>
                      </Link>
                    </td>
                    <td className="px-3 py-2.5 text-xs font-bold text-slate-700">{pr?.platform?.replace(/_/g, ' ') || '—'}</td>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      {pr?.isRentable ? (
                        <span className="px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 text-[10px] font-extrabold">
                          {pr.rentalPricePerDay ? `${formatPKR(pr.rentalPricePerDay)}/day` : 'Yes'}
                        </span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/gaming-products/${p.id}`} className="h-8 w-8 rounded-lg bg-violet-50 hover:bg-violet-100 text-violet-700 flex items-center justify-center">
                          <Eye className="h-3.5 w-3.5" />
                        </Link>
                        <Link to={`/gaming-products/${p.id}/edit`} className="h-8 w-8 rounded-lg bg-fuchsia-50 hover:bg-fuchsia-100 text-fuchsia-700 flex items-center justify-center">
...
function Card({ p, pr, sel, onToggle }: any) {
  ...
  return (
    <div className={`group relative rounded-2xl bg-white border-2 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 ${
      sel ? 'border-violet-500 ring-2 ring-violet-200' : isOut ? 'border-rose-200' : isLow ? 'border-amber-200' : 'border-slate-200'}`}>
      <button onClick={onToggle}
        className={`absolute top-2 left-2 z-10 h-6 w-6 rounded-lg border-2 flex items-center justify-center transition ${
          sel ? 'bg-violet-600 border-violet-600 text-white' : 'bg-white/90 border-slate-300 opacity-0 group-hover:opacity-100'}`}>
        {sel && <CheckCircle2 className="h-3.5 w-3.5" />}
      </button>
      <Link to={`/gaming-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 relative overflow-hidden">
          {p.images?.[0]?.url
            ...
            : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-violet-50 to-fuchsia-50"><Gamepad2 className="h-10 w-10 text-violet-300" /></div>}
          {pr?.isPreOrder && <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-md bg-amber-500 text-white text-[9px] font-extrabold">PRE-ORDER</div>}
          {pr?.isFeatured && !pr?.isPreOrder && (
        ...
        <div className="p-2.5">
          ...
          {pr?.platform && <div className="mt-1 text-[9px] font-extrabold uppercase text-violet-700">{pr.platform.replace(/_/g, ' ')}</div>}
          ...
          {pr?.conditionType && pr.conditionType !== 'NEW_SEALED' && (
            <div className="mt-2 inline-flex items-center px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-800 text-[9px] font-extrabold">
              {pr.conditionType.replace(/_/g, ' ')}
            </div>
          )}
        </div>
      </Link>
...
function Kpi({ icon: Icon, label, value, sub, tone, onClick }: any) {
  const tones: Record<string, string> = {
    violet: 'from-violet-500 to-fuchsia-700', emerald: 'from-emerald-500 to-emerald-700',
