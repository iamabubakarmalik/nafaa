import {
  Gamepad2, Boxes, TrendingUp, DollarSign, Timer, Rocket,
  AlertTriangle, CheckCircle2, Sparkles, Monitor,
} from 'lucide-react';
import { formatPKRFull } from '@core/lib/format';
import type { GamingWizardDraft } from '../../hooks/useGamingWizard';

interface Props {
  draft: GamingWizardDraft;
  stats: {
    variantCount: number;
    totalStock: number;
    stockValue: number;
...
export function GamingWizardSummary({ draft, stats, allValid }: Props) {
  ...
  return (
    <aside className="flex flex-col gap-3 xl:sticky xl:top-4 xl:self-start">
      <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-violet-900 to-fuchsia-700 text-white p-5 shadow-xl overflow-hidden relative">
        <div className="absolute -top-8 -right-8 h-32 w-32 rounded-full bg-fuchsia-400/20 blur-2xl" />
        <div className="relative">
          ...
          <h3 className="mt-2 font-extrabold text-xl leading-tight line-clamp-2">
            {draft.basic.name || 'Product title...'}
          </h3>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {draft.basic.platform && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <Monitor className="h-3 w-3" /> {draft.basic.platform.replace(/_/g, ' ')}
              </span>
            )}
            {draft.basic.categoryType && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-white/15 backdrop-blur px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider">
                <Gamepad2 className="h-3 w-3" /> {draft.basic.categoryType.replace(/_/g, ' ')}
              </span>
            )}
            {draft.basic.conditionType && draft.basic.conditionType !== 'NEW_SEALED' && (
      ...
      <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm overflow-hidden">
        <div className="grid grid-cols-2 divide-x divide-slate-100">
          <Cell icon={Boxes} label="Editions" value={stats.variantCount} hint={draft.hasVariants ? 'variants on' : 'single SKU'} tone="fuchsia" />
          <Cell icon={Boxes} label="Total Stock" value={stats.totalStock} hint="units" tone="emerald" />
        </div>

        {draft.rental.isRentable && (
          <div className="border-t border-slate-100 p-4 bg-gradient-to-br from-violet-50 to-white">
            <div className="flex items-center gap-1.5 mb-1">
              <Timer className="h-3 w-3 text-violet-700" />
              <div className="text-[10px] uppercase tracking-wider font-extrabold text-violet-700">Rental enabled</div>
            </div>
            <div className="text-sm font-extrabold text-violet-900">
              {draft.rental.rentalPricePerHour ? `${formatPKRFull(Number(draft.rental.rentalPricePerHour))}/hr` : ''}
              {draft.rental.rentalPricePerHour && draft.rental.rentalPricePerDay ? ' • ' : ''}
              {draft.rental.rentalPricePerDay ? `${formatPKRFull(Number(draft.rental.rentalPricePerDay))}/day` : ''}
            </div>
            {draft.rental.rentalDeposit ? (
              <div className="text-[10px] font-bold text-amber-700 mt-0.5">
                Deposit {formatPKRFull(Number(draft.rental.rentalDeposit))}
              </div>
            ) : null}
          </div>
        )}

        {draft.rental.isPreOrder && (
          <div className="border-t border-slate-100 p-4 bg-gradient-to-br from-amber-50 to-white">
            <div className="flex items-center gap-1.5 mb-1">
              <Rocket className="h-3 w-3 text-amber-700" />
              <div className="text-[10px] uppercase tracking-wider font-extrabold text-amber-700">Pre-order</div>
            </div>
            <div className="text-sm font-extrabold text-amber-900">
              {draft.rental.preOrderReleaseDate
                ? new Date(draft.rental.preOrderReleaseDate).toLocaleDateString('en-PK', { day: 'numeric', month: 'short', year: 'numeric' })
                : 'Date not set'}
            </div>
          </div>
        )}
      ...
      {(draft.basic.isFeatured || draft.basic.isBestSeller || draft.basic.isNewRelease) && (
        <div className="rounded-2xl bg-white border-2 border-slate-200 shadow-sm p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {draft.basic.isNewRelease && <Flag icon="🆕" label="New Release" tone="emerald" />}
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-slate-50 border-2 border-slate-200 p-3 space-y-1.5">
        ...
        <Chk done={hasName} label="Product title" />
        <Chk done={hasPrice} label="Retail price" />
        <Chk done={!!draft.basic.categoryType} label="Category type" />
        <Chk done={!!draft.basic.platform} label="Platform" />
        <Chk done={stats.totalStock > 0} label="Stock added" />
      </div>

      <div className="rounded-xl bg-violet-50 border-2 border-violet-200 p-2.5 text-[10px] text-violet-800 font-extrabold text-center">
