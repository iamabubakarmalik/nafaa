...
import type {
  GamingWizardBasic, GamingWizardVariant, GamingWizardStock,
} from '../../hooks/useGamingWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<GamingWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<GamingWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<GamingWizardStock>) => void;
  errors: string[];
}
...
const VARIANT_PRESETS = [
  { group: 'Edition', items: ['Standard', 'Deluxe', 'Ultimate', 'Gold', 'Collector', 'Day One'] },
  { group: 'Region', items: ['Region Free', 'PAL', 'NTSC-U', 'NTSC-J', 'Asia', 'UK'] },
  { group: 'Storage', items: ['500GB', '825GB', '1TB', '2TB'] },
  { group: 'Colour', items: ['Black', 'White', 'Blue', 'Red', 'Camo', 'RGB'] },
  { group: 'Condition', items: ['New Sealed', 'Pre-owned', 'Open Box'] },
];

export function GamingWizardStep4Stock({
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-violet-50 to-white border-2 border-violet-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-extrabold text-violet-900">Stock entry — two ways</h3>
          <p className="text-xs text-violet-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> one quantity (accessories, single-edition games).
            <strong> Editions:</strong> Standard / Deluxe / Ultimate or PAL / NTSC each with its own stock and price.
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Editions / Variants</div>
            <div className="text-xs text-slate-600 font-semibold">Standard vs Deluxe, PAL vs NTSC, 1TB vs 2TB</div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={stock.rackNumber} onChange={(e) => onUpdateStock({ rackNumber: e.target.value })}
                  ...
                  className="h-12 w-full rounded-xl border-2 border-slate-200 pl-10 pr-3 text-sm font-bold focus:outline-none focus:border-violet-500" />
              </div>
            </div>
          </div>
        </section>
      ...
      {hasVariants && (
        <section className="rounded-2xl border-2 border-fuchsia-300 bg-gradient-to-br from-fuchsia-50 to-white p-5 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1">
              <h3 className="font-extrabold text-fuchsia-900 text-base">Editions</h3>
              <p className="text-xs text-fuchsia-700 font-semibold">Each edition keeps its own stock and price</p>
            </div>
            <span className="px-2.5 py-1 rounded-lg bg-fuchsia-100 text-fuchsia-800 text-[10px] font-extrabold">
              {variants.length} editions • {totalVariantStock} units
            </span>
          </div>
          ...
          <div className="rounded-xl bg-white border-2 border-fuchsia-200 p-3 space-y-2">
            ...
            {VARIANT_PRESETS.map((grp) => (
              <div key={grp.group} className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-extrabold uppercase text-slate-500 min-w-[62px]">{grp.group}:</span>
                {grp.items.map((it) => {
          ...
          <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end">
            <Input label="Custom edition name" value={vName}
              onChange={(e) => setVName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && addVariant(vName)}
              placeholder="e.g. Ultimate Edition PAL" />
          ...
          {variants.length > 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div>
                      ...
                      <input type="number" step="1" value={v.stock}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-sm font-extrabold tabular-nums focus:outline-none focus:border-violet-500" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={v.sku ?? ''} onChange={(e) => onUpdateVariant(v.tempId, { sku: e.target.value })}
                        ...
                        className="h-10 w-full rounded-lg border-2 border-slate-200 px-2 text-xs font-mono font-bold focus:outline-none focus:border-violet-500" />
                    </div>
                    <div>
            ...
            <div className="rounded-xl border-2 border-dashed border-fuchsia-300 bg-white p-6 text-center">
              ...
              <div className="text-sm font-extrabold text-slate-700">No editions yet</div>
      ...
      {displayStock > 0 && sale > 0 && (
        <section className="rounded-2xl bg-gradient-to-br from-slate-950 to-violet-900 text-white p-5 shadow-xl">
