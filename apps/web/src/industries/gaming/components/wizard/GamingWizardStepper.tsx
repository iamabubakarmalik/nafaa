import { Check, Gamepad2, Sparkles, Timer, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/useGamingWizard';
...
const STEPS = [
  { id: 1, label: 'Basic Info', short: 'Info', desc: 'Title, platform, price', icon: Gamepad2 },
  { id: 2, label: 'Details & Specs', short: 'Specs', desc: 'Genre, hardware specs', icon: Sparkles },
  { id: 3, label: 'Rental & Pre-order', short: 'Rental', desc: 'Rent rates, release', icon: Timer },
  { id: 4, label: 'Stock & Editions', short: 'Stock', desc: 'Quantity, variants', icon: Package },
] as const;

export function GamingWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-1.5 flex-1">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-violet-600 to-fuchsia-700 text-white shadow-md shadow-violet-500/40 scale-[1.02]'
                    : isPast
                      ? 'bg-violet-50 text-violet-800 hover:bg-violet-100 border-2 border-violet-200'
              ...
              >
                <div className={[
                  ...
                  isActive ? 'bg-white/25 text-white'
                    : isPast && isComplete ? 'bg-violet-600 text-white'
                ...
                <div className="text-left min-w-0">
                  <div className={[
                    ...
                    isActive ? 'text-white/90' : isPast ? 'text-violet-700' : 'text-slate-500',
                  ].join(' ')}>
                    Step {s.id}
                  </div>
              ...
              {idx < STEPS.length - 1 && (
                <div className={['h-1 w-3 sm:w-4 rounded-full transition',
                  currentStep > s.id ? 'bg-violet-500' : 'bg-slate-200'].join(' ')} />
              )}
            </div>
          );
        })}
