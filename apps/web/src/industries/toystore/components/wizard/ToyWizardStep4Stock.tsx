...
import {
  ...
  Boxes, MapPin, Sparkles, CheckCircle2,
} from 'lucide-react';
import { Input } from '@core/ui/Input';
import { formatPKRFull } from '@core/lib/format';
import type { ToyWizardBasic, ToyWizardVariant, ToyWizardStock } from '../../hooks/useToyWizard';
...
interface Props {
  ...
  onAddVariant: (v: Omit<ToyWizardVariant, 'tempId' | 'sortOrder' | 'isActive'>) => void;
  onUpdateVariant: (tempId: string, patch: Partial<ToyWizardVariant>) => void;
  onRemoveVariant: (tempId: string) => void;
  onUpdateStock: (patch: Partial<ToyWizardStock>) => void;
  errors: string[];
}
...
const VARIANT_PRESETS = [
  { group: 'Color', items: ['Pink', 'Blue', 'Red', 'Green', 'Yellow', 'Purple', 'White', 'Black', 'Multi'] },
  { group: 'Size', items: ['Small', 'Medium', 'Large', 'XL', 'Mini', 'Giant'] },
  { group: 'Piece Count', items: ['100 pcs', '200 pcs', '500 pcs', '1000 pcs', '2000 pcs'] },
  { group: 'Edition', items: ['Standard', 'Deluxe', 'Premium', 'Limited Edition', 'Gift Set'] },
  { group: 'Character', items: ['Princess', 'Superhero', 'Animal', 'Vehicle', 'Fantasy'] },
];

export function ToyWizardStep4Stock({
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-white border-2 border-pink-200 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-pink-800 font-semibold mt-0.5 leading-relaxed">
            <strong>Simple:</strong> single count (one edition of the toy).
            <strong> Variants:</strong> different colors, sizes, or editions each with their own stock and price.
          </p>
        </div>
      </div>
      ...
      <button type="button" onClick={() => onToggleVariants(!hasVariants)}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-extrabold text-slate-900">Color / Size / Edition Variants</div>
            <div className="text-xs text-slate-600 font-semibold">Pink vs Blue, Small vs Large, 200 pcs vs 500 pcs</div>
          </div>
      ...
      {hasVariants && (
        <section className="rounded-2xl border-2 border-pink-300 bg-gradient-to-br from-pink-50 to-white p-5 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1">
              <h3 className="font-extrabold text-pink-900 text-base">Variants</h3>
              <p className="text-xs text-pink-700 font-semibold">Each variant has its own stock and optional price</p>
            </div>
          ...
          <div className="rounded-xl bg-white border-2 border-pink-200 p-3 space-y-2">
            ...
            {VARIANT_PRESETS.map((grp) => (
              <div key={grp.group} className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-extrabold uppercase text-slate-500 min-w-[70px]">{grp.group}:</span>
                {grp.items.map((it) => {
          ...
          {variants.length > 0 ? (
            ...
            <div className="rounded-xl border-2 border-dashed border-pink-300 bg-white p-6 text-center">
              <Boxes className="h-10 w-10 text-pink-400 mx-auto mb-2" />
