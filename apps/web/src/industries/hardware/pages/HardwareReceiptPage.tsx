...
import {
  Printer, ArrowLeft, MessageCircle, X, Package, Building,
  MapPin, Phone, Calendar, User, CheckCircle2, ShieldAlert, Truck, Layers,
} from 'lucide-react';
...
export default function HardwareReceiptPage() {
  const { id } = useParams();
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Hardware Store';
    ...
    sale.items.forEach((it: any, i: number) => {
      ...
      lines.push(`   ${it.quantity} ${it.product.unit} × ${formatPKR(it.price)} = ${formatPKR(it.total)}`);
      if (it.note && it.note.startsWith('Tier:')) {
        lines.push(`   🏗️ ${it.note}`);
      }
    });
    lines.push('');
    ...
    lines.push('');
    lines.push('_Delivery arrangement contact karo._');
  ...
  if (!sale) {
    return <div className="min-h-screen flex flex-col items-center justify-center"><p className="font-bold">Receipt not found</p><Link to="/sales" className="mt-4 text-amber-600 hover:underline">← Back</Link></div>;
  }

  const settings = sale.tenant?.settings;
  const shopName = settings?.shopName || sale.tenant?.name || 'Hardware Store';
  ...
  const hasProject = ((sale as any).note || '').startsWith('Project:');
  const totalUnits = sale.items.reduce((s: number, it: any) => s + Number(it.quantity || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50 py-6 px-4 print:bg-white print:py-0 print:px-0">
      <div className={`mx-auto space-y-4 ${format === 'a4' ? 'max-w-4xl' : 'max-w-md'}`}>
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
            <CheckCircle2 className="h-6 w-6" />
            <div className="flex-1">
              <div className="font-extrabold">Sale Complete! 🔨</div>
              <div className="text-xs text-white/90">Print bill for contractor</div>
            </div>
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => window.print()}
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-600 to-orange-700 hover:from-amber-700 hover:to-orange-800 px-4 py-2.5 text-sm font-bold text-white shadow-md">
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
            <div className="relative bg-gradient-to-br from-slate-950 via-amber-900 to-orange-700 text-white px-8 py-7 print:bg-white print:text-slate-900 print:border-b-4 print:border-double print:border-slate-700 overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <Package className="h-3 w-3" /> Hardware Sale Invoice
                    </div>
            ...
            {sale.customer && (
              <div className="px-8 py-4 border-b-2 border-slate-100 bg-slate-50/50 grid sm:grid-cols-2 gap-3 print:bg-white">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-[10px] uppercase font-extrabold text-slate-500">Customer / Contractor</div>
                ...
                {hasProject && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-[10px] uppercase font-extrabold text-blue-700">Linked Project</div>
                      <div className="font-extrabold text-blue-900">{(sale as any).note?.replace('Project:', '').trim()}</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b-2 border-slate-300 bg-gradient-to-r from-amber-50 to-slate-50 print:bg-white">
                    ...
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase text-right w-28">Rate</th>
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase text-right w-32">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {sale.items.map((it: any, idx: number) => (
                    <tr key={it.id} className="border-b border-slate-100 align-top hover:bg-slate-50/50">
                      ...
                      <td className="py-3 px-2">
                        ...
                        {it.note && it.note.startsWith('Tier:') && (
                          <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-100 border border-amber-300 text-[10px] font-extrabold text-amber-900 print:bg-white print:border-slate-400">
                            <Layers className="h-2.5 w-2.5" />
                            {it.note} — Bulk rate applied
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-2 text-center">
                        <div className="text-lg font-extrabold text-slate-900 tabular-nums">{it.quantity}</div>
                        <div className="text-[10px] font-bold text-slate-500 uppercase">{it.product.unit}</div>
                      </td>
            ...
            <div className="px-8 py-6 border-t-2 border-dashed border-slate-300 bg-gradient-to-br from-slate-50 to-white print:bg-white">
              <div className="ml-auto max-w-md space-y-1.5">
                ...
                <div className="pt-3 border-t border-slate-200 space-y-1">
                  ...
                  {sale.creditAmount > 0 && (
                    <div className="flex justify-between text-sm rounded-lg bg-amber-50 border-2 border-amber-300 px-3 py-2 mt-2 print:bg-white"><span className="text-amber-800 font-bold">Udhaar (Baqi)</span><span className="font-extrabold text-amber-700 text-base tabular-nums">{formatPKR(sale.creditAmount)}</span></div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="px-8 py-5 border-t-2 border-slate-100 bg-amber-50/50 print:bg-white">
              <div className="text-[10px] uppercase font-extrabold text-amber-700 mb-2">Delivery & Terms</div>
              <ul className="text-[11px] text-slate-700 space-y-1 list-disc pl-4">
                <li>Truck delivery available — arrangement bhi ho sakti hai (extra charge)</li>
                <li>Damaged goods only on delivery day return honge (with original bill)</li>
                <li>Cement / plaster material return nahi hoti — quality guarantee hain</li>
                <li>Keep this invoice safe for warranty & claims</li>
              </ul>
            </div>

            <div className="px-8 py-5 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-amber-50 to-orange-50 print:bg-white">
            ...
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              {sale.customer && (
                <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                  ...
                  {hasProject && <div className="mt-1 font-bold text-center">📋 {(sale as any).note?.replace('Project:', '').trim()}</div>}
                </div>
              )}
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                ...
                {sale.items.map((item: any, idx: number) => (
                  <div key={item.id} className="mb-1">
                    ...
                    {item.note && item.note.startsWith('Tier:') && (
                      <div className="pl-2 text-[9px] italic">{item.note}</div>
                    )}
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1 text-center text-[9px]">
                {sale.items.length} items • {totalUnits.toFixed(0)} units
              </div>
