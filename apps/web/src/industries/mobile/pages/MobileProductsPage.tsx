// src/industries/mobile/pages/MobileProductsPage.tsx
import { useMemo, useState, useEffect, useRef } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import {
  Smartphone, Plus, Search, X, RefreshCw, Download, Grid3x3, List,
  Package, AlertTriangle, DollarSign, Eye, Edit3, Trash2,
  Barcode, ShoppingCart, CheckCircle2, XCircle, Star,
  PackageX, GraduationCap, Keyboard, Printer, Cable, Hash,
  ShieldCheck, AlertOctagon, Clock, Shield,
} from 'lucide-react';
...
/* ═════════════════════════════════════════════════════════════
   NAFAA MOBILE PRODUCTS — FULL BEST v1
   ─────────────────────────────────────────────────────────────
   📱🎧 Phone/Accessory type badges — koi Pakistani grocery
      Quick Setup nahi, ye purely mobile-industry ka apna page
   🌙 Dark mode complete
   🎓 Teacher — mobile-specific guide (wizard, IMEI, PTA)
   📊 PTA breakdown strip — non-PTA / pending phones ek nazar me
   🔢 Stock column type-aware: Phone = IMEI count, Accessory = qty
   ⌨️  / search focus • Esc modals close
   🖨️ Print / CSV export
   ═════════════════════════════════════════════════════════════ */
...
type TypeFilter = 'all' | 'PHONE' | 'ACCESSORY';
type SortKey = 'name' | 'stock-low' | 'stock-high' | 'price-low' | 'price-high' | 'newest';

const VIEW_KEY = 'mobile-products-view';
const PAGE_SIZE = 48;
...
export default function MobileProductsPage() {
  ...
  const { data: imeiStats } = useQuery({ queryKey: ['imei-stats'], queryFn: imeiApi.stats });

  /* ─── Stats ─── */
  const stats = useMemo(() => {
    const active = products.filter((p) => p.isActive);
    const phones = products.filter((p) => (p.productType ?? 'PHONE') === 'PHONE');
  ...
  const ptaAlerts = useMemo(() => {
    const nonPta = imeiStats?.byPta?.find((p) => p.ptaStatus === 'NON_PTA')?.count ?? 0;
    const pending = imeiStats?.byPta?.find((p) => p.ptaStatus === 'PENDING')?.count ?? 0;
    return { nonPta, pending };
  }, [imeiStats]);
  ...
  const filtered = useMemo(() => {
    ...
    if (typeFilter !== 'all') {
      list = list.filter((p) => (p.productType ?? 'PHONE') === typeFilter);
    }
    if (stockFilter !== 'all') {
      list = list.filter((p) => {
        const s = Number(p.stock || 0);
    ...
    return list;
  }, [products, search, categoryId, brandId, typeFilter, stockFilter, statusFilter, sortKey]);

  const visible = filtered.slice(0, visibleCount);
  useEffect(() => { setVisibleCount(PAGE_SIZE); }, [search, categoryId, brandId, typeFilter, stockFilter, statusFilter, sortKey]);

  const hasFilters = !!search || categoryId !== 'all' || brandId !== 'all' || typeFilter !== 'all' || stockFilter !== 'all' || statusFilter !== 'active';
  const clearFilters = () => {
    setSearch(''); setCategoryId('all'); setBrandId('all'); setTypeFilter('all');
  ...
  const forceDeleteAll = async (ids: string[]) => {
    ...
    if (ok) toast.success(`${ok} products force-deleted — sales, IMEIs, stock, sab kuch`);
  ...
  const bulkDelete = useMutation({
    ...
    onSuccess: ({ ids, ok, fail }) => {
      ...
      if (fail) {
        toast.error(`${fail} products ki sales/IMEI history hai`, {
          duration: 15000,
          action: {
            label: '⚠️ Force Delete All',
            onClick: () => {
              if (!confirm(`⚠️ DANGER: ${fail} products AUR unki saari sales / IMEIs / stock history PERMANENTLY delete ho jayegi.\n\nYe undo nahi ho sakta. Continue?`)) return;
  ...
  const exportCSV = (rows: any[]) => {
    ...
    const summary = [
      [`Mobile Products Report — ${tenantName || 'Nafaa'}`],
      [`Shop: ${shopName || 'All'}  •  Generated: ${new Date().toLocaleString('en-PK')}`],
      [`Total: ${rows.length}  •  Phones: ${stats.phoneCount}  •  Accessories: ${stats.accessoryCount}  •  Stock Value: ${stats.stockValue.toFixed(2)}`],
      [''],
    ];
    const head = ['Name', 'Type', 'SKU', 'Barcode', 'Category', 'Brand', 'Cost', 'Sale', 'Wholesale', 'Stock', 'Stock Value', 'Active'];
    const body = rows.map((p) => [
      p.name, p.productType ?? 'PHONE', p.sku || '', p.barcode || '', p.category?.name || '', p.brand?.name || '',
    ...
    a.href = url;
    a.download = `mobile-products-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-4 sm:space-y-5 pb-24 print:space-y-3">
                ...
                <>
                  <div className="rounded-2xl bg-rose-50 dark:bg-rose-900/20 border-2 border-rose-200 dark:border-rose-800 p-4">
                    ...
                    <ul className="text-xs font-semibold text-rose-800 dark:text-rose-300 space-y-1">
                      <li>• Products + images + variants</li>
                      <li>• IMEIs (agar phone hai)</li>
        ...
        <div className="flex items-center justify-between border-b-4 border-blue-600 pb-3 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-600 font-semibold mt-1">
              {shopName ? `Shop: ${shopName}  •  ` : ''}Mobile Products • {filtered.length} items
            </p>
          </div>
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1.5 text-xs sm:text-sm text-white/90 font-semibold">
              <strong className="text-blue-200">{stats.phoneCount}</strong> phones
              <span className="opacity-50 mx-1.5">•</span>
              <strong className="text-emerald-300">{stats.accessoryCount}</strong> accessories
              {!hideCost && (
                <>
                  <span className="opacity-50 mx-1.5">•</span>
          ...
          <div className="flex gap-2 flex-wrap items-center shrink-0">
            ...
            <Link
              ...
              className="h-11 px-3 rounded-xl bg-white/15 hover:bg-white/25 border border-white/25 text-xs font-extrabold inline-flex items-center gap-1.5 backdrop-blur-md transition"
            >
              <Hash className="h-4 w-4" /> <span className="hidden sm:inline">All IMEIs</span>
            </Link>
            <Link
              to="/mobile-products/new"
      ...
      {showTeacher && <MobileProductsTeacher onClose={() => setShowTeacher(false)} onAddPhone={() => { setShowTeacher(false); navigate('/mobile-products/new'); }} />}

      {/* ═══ KPIs ═══ */}
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 print:hidden">
        <Kpi icon={Package} label="Total Products" value={stats.total} sub={`${stats.phoneCount} phone • ${stats.accessoryCount} accessory`} tone="blue" />
      ...
      {/* ═══ PTA ALERT STRIP ═══ */}
      {(ptaAlerts.nonPta > 0 || ptaAlerts.pending > 0) && (
        <section className="rounded-2xl sm:rounded-3xl bg-gradient-to-br from-rose-50 to-orange-50 dark:from-rose-500/10 dark:to-orange-500/10 border-2 border-rose-300 dark:border-rose-500/40 p-4 print:hidden">
          <div className="flex items-start gap-3 flex-wrap">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-rose-500 to-orange-600 text-white flex items-center justify-center shadow-md shadow-rose-500/40 shrink-0">
              <ShieldCheck className="h-5 w-5" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-extrabold text-rose-900 dark:text-rose-200 text-sm">
                🛡️ PTA Compliance Alert
              </h3>
              <p className="text-xs font-semibold text-rose-800 dark:text-rose-300 mt-0.5">
                {ptaAlerts.nonPta > 0 && <><strong>{ptaAlerts.nonPta}</strong> non-PTA IMEIs — sell nahi ho payenge. </>}
                {ptaAlerts.pending > 0 && <><strong>{ptaAlerts.pending}</strong> PTA-pending — verify karein.</>}
              </p>
            </div>
            <Link
              to="/imei-inventory"
              className="px-3 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-extrabold shrink-0 transition shadow-md"
            >
              IMEIs dekho →
            </Link>
          </div>
      ...
      <section className="rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900/80 dark:backdrop-blur-sm border-2 border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-black/20 p-4 space-y-3 print:hidden">
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="inline-flex rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
            <button
              ...
              title="Card view"
              className={`px-4 h-12 text-xs font-extrabold transition ${view === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
            ...
            <button
              ...
              title="List view"
              className={`px-4 h-12 text-xs font-extrabold border-l-2 border-slate-200 dark:border-slate-700 transition ${view === 'table' ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
          ...
          <button
            ...
            className="h-12 px-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-500/50 bg-white dark:bg-slate-800 text-sm font-extrabold text-slate-700 dark:text-slate-200 inline-flex items-center gap-1.5 transition"
          >
        ...
        <div className="flex gap-2 flex-wrap items-center">
          {/* Type filter — Phone vs Accessory */}
          <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl p-1">
            {([
              { v: 'all' as TypeFilter, l: 'Sab', icon: null },
              { v: 'PHONE' as TypeFilter, l: 'Phones', icon: Smartphone },
              { v: 'ACCESSORY' as TypeFilter, l: 'Accessories', icon: Cable },
            ]).map((o) => (
              <button
                key={o.v}
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold inline-flex items-center gap-1 transition ${
                  typeFilter === o.v ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
          ...
          <select
            ...
            className="h-10 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 transition"
          >
            ...
            {([
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold transition ${
                  stockFilter === o.v ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                {o.l}
                {o.c != null && (
      ...
      {filtered.length === 0 ? (
        <section className="rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900/80 border-2 border-dashed border-slate-300 dark:border-slate-700 p-12 sm:p-16 text-center">
          <div className="mx-auto h-20 w-20 rounded-3xl bg-gradient-to-br from-blue-500 to-indigo-700 flex items-center justify-center shadow-lg shadow-blue-500/40">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 font-semibold max-w-md mx-auto">
            {hasFilters ? 'Filter change kar ke dekho' : 'Wizard se poora product ek page mein banao — phone ho ya accessory, IMEI ya stock qty sab ek saath.'}
          </p>
            ...
            {hasFilters ? (
              ...
              <>
                ...
                <Link to="/mobile-products/new">
                  <Button className="bg-gradient-to-r from-blue-600 to-indigo-700 font-extrabold"><Plus className="h-4 w-4" /> Naya Product</Button>
                </Link>
              </>
            )}
          </div>
            ...
            <table className="w-full text-sm print:text-[10px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {visible.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className={['hover:bg-blue-50/40 dark:hover:bg-blue-500/5 transition', selected.has(p.id) ? 'bg-blue-50/60 dark:bg-blue-500/10' : ''].join(' ')}>
                      <td className="px-3 py-2.5 print:hidden">
                        <input type="checkbox" checked={selected.has(p.id)} onChange={() => toggleOne(p.id)} className="h-4 w-4 rounded accent-blue-600" />
                      </td>
                      <td className="px-3 py-2.5">
                        <Link to={`/mobile-products/${p.id}`} className="flex items-center gap-2.5 group">
                          <Thumb p={p} size="h-10 w-10" />
                          <div className="min-w-0">
                            <div className="font-extrabold text-slate-900 dark:text-white text-sm truncate group-hover:text-blue-700 dark:group-hover:text-blue-400 flex items-center gap-1">
                              {p.name}
                      ...
                      <td className="px-3 py-2.5 text-right">
                        <button onClick={() => setStockModalProduct(p)} className="inline-flex flex-col items-end group">
                          <span className={[
                            ...
                            {stock} <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{isPhone ? 'IMEI' : p.unit}</span>
                          </span>
                          <span className="text-[9px] font-extrabold text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition print:hidden">+ Stock</span>
                        </button>
                      </td>
                      {!hideCost && (
                      ...
                      <td className="px-3 py-2.5 print:hidden">
                        <div className="flex items-center justify-end gap-1">
                          <IconBtn to={`/mobile-products/${p.id}`} title="Dekho" tone="blue"><Eye className="h-3.5 w-3.5" /></IconBtn>
                          <IconBtn to={`/mobile-products/${p.id}/edit`} title="Edit" tone="violet"><Edit3 className="h-3.5 w-3.5" /></IconBtn>
                          {isPhone && (
                            <IconBtn to={`/imei-inventory/${p.id}`} title="IMEIs" tone="indigo"><Hash className="h-3.5 w-3.5" /></IconBtn>
                          )}
                          <button
                            onClick={() => setStockModalProduct(p)}
                            title="Stock"
      ...
      {visibleCount < filtered.length && (
        <div className="flex justify-center print:hidden">
          <button
            ...
            className="px-6 py-3 rounded-2xl bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500/50 text-sm font-extrabold text-slate-700 dark:text-slate-200 shadow-sm transition"
          >
      ...
      <style>{`
        @media print {
          ...
          thead th { background: #2563eb !important; color: white !important; padding: 5px 4px !important; font-size: 8px !important; font-weight: 800 !important; border: 1px solid #1d4ed8 !important; }
...
/* ═════════════════════════════════════════════════════════════
   TEACHER — mobile-specific guide
   ═════════════════════════════════════════════════════════════ */
function MobileProductsTeacher({ onClose, onAddPhone }: { onClose: () => void; onAddPhone: () => void }) {
  return (
    <div
      ...
      >
        <div className="px-5 py-3 border-b-2 border-blue-200 dark:border-blue-500/30 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-500/15 dark:to-indigo-500/15 flex items-center justify-between sticky top-0 z-10">
          <h3 className="font-extrabold text-blue-900 dark:text-blue-200 flex items-center gap-2">
            <GraduationCap className="h-5 w-5" /> Mobile Products — Complete Guide
          </h3>
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 leading-relaxed">
            Ye page aapka <strong>poora catalog</strong> hai — phones (IMEI-tracked) aur accessories
            (simple stock) dono ek jagah. Har card pe type badge dikhta hai: 📱 Phone ya 🎧 Accessory.
          </p>
          ...
          <div className="rounded-2xl border-2 border-blue-200 dark:border-blue-500/30 bg-blue-50/60 dark:bg-blue-500/5 p-4 space-y-2.5">
            ...
            <div className="space-y-2 text-xs font-semibold text-slate-700 dark:text-slate-200">
              <GuideRow emoji="🧙" title="Naya Product Wizard" desc="3 step — basic info, colors/storage, IMEIs ya stock" />
              <GuideRow emoji="📷" title="Camera scan" desc="Wizard ke andar box ka IMEI barcode scan karo" />
              <GuideRow emoji="📋" title="Bulk paste" desc="Ek saath 10+ IMEIs paste karo — auto duplicate-check" />
            </div>
          </div>
          ...
          <div className="rounded-xl bg-rose-50 dark:bg-rose-500/10 border-2 border-rose-200 dark:border-rose-500/40 p-3 text-xs font-semibold text-rose-900 dark:text-rose-200">
            🛡️ <strong>PTA zaroori hai:</strong> Non-PTA ya Pending IMEIs POS me sell nahi ho sakte.
            Upar wali red strip pe click kar ke seedha IMEI inventory kholo aur fix karo.
          </div>
          ...
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 p-3 space-y-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200">
            ...
            <TipRow><strong>Type filter</strong> — Phones / Accessories alag dekho</TipRow>
            <TipRow><strong>Card checkbox</strong> — select → bulk active/band/delete</TipRow>
            <TipRow><strong>🗑️ Delete</strong> — sales/IMEI history ho to soft-delete, warna 2-step permanent</TipRow>
          </div>
          ...
          <Button
            ...
            onClick={onAddPhone}
          >
            <Plus className="h-4 w-4" /> Samajh Gaya — Naya Product Banao!
          </Button>
        </div>
      </div>
    </div>
...
function ProductCard({ p, selected, onToggle, onStock }: any) {
  ...
  return (
    <div className={[
      'group relative rounded-2xl bg-white dark:bg-slate-900/80 dark:backdrop-blur-sm border-2 overflow-hidden transition-all hover:shadow-lg dark:hover:shadow-blue-500/10 hover:-translate-y-0.5',
      selected ? 'border-blue-500 ring-2 ring-blue-200 dark:ring-blue-500/30' : isOut ? 'border-rose-200 dark:border-rose-500/40' : isLow ? 'border-amber-200 dark:border-amber-500/40' : 'border-slate-200 dark:border-slate-800',
      ...
      <button
        ...
        className={[
          ...
          selected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white/90 dark:bg-slate-900/90 border-slate-300 dark:border-slate-600 opacity-0 group-hover:opacity-100',
        ].join(' ')}
      >
      ...
      <div className="absolute top-2 right-2 z-10">
        <TypePill isPhone={isPhone} compact />
      </div>

      <Link to={`/mobile-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 dark:bg-slate-800 overflow-hidden relative">
          {p.images?.[0]?.url ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              {isPhone ? <Smartphone className="h-10 w-10 text-slate-300 dark:text-slate-600" /> : <Cable className="h-10 w-10 text-slate-300 dark:text-slate-600" />}
            </div>
          )}
          {p.isFeatured && (
            <div className="absolute bottom-2 right-2 h-6 w-6 rounded-full bg-amber-500 flex items-center justify-center shadow">
          ...
          {(isOut || isLow) && (
            <div className={[
              ...
              {isOut ? 'STOCK KHATAM' : `SIRF ${stock} ${isPhone ? 'IMEI' : p.unit} BACHA`}
            </div>
          )}
        </div>
        ...
        <div className="p-2.5">
          ...
          <div className="mt-1.5 flex items-end justify-between gap-1">
            <div>
              ...
              <div className="text-[9px] font-bold text-slate-500 dark:text-slate-400">per {isPhone ? 'unit' : p.unit}</div>
            </div>
            <div className="text-right">
              <div className={[
              ...
              <div className="text-[9px] font-bold text-slate-500 dark:text-slate-400">{isPhone ? 'IMEIs' : 'stock'}</div>
            </div>
          </div>
        </div>
      </Link>
      ...
      <div className="px-2.5 pb-2.5 flex items-center gap-1">
        ...
        {isPhone && (
          <Link
            to={`/imei-inventory/${p.id}`}
            className="h-8 w-8 rounded-lg bg-indigo-50 dark:bg-indigo-500/15 hover:bg-indigo-100 dark:hover:bg-indigo-500/25 text-indigo-700 dark:text-indigo-300 flex items-center justify-center transition"
            title="IMEIs"
          >
...
function TypePill({ isPhone, compact }: { isPhone: boolean; compact?: boolean }) {
  return (
    <span className={[
      'inline-flex items-center gap-1 rounded-full font-extrabold',
      compact ? 'px-1.5 py-0.5 text-[9px] shadow-sm backdrop-blur' : 'px-2 py-0.5 text-[10px]',
      isPhone
        ? 'bg-blue-100 dark:bg-blue-500/80 text-blue-700 dark:text-white'
        : 'bg-emerald-100 dark:bg-emerald-500/80 text-emerald-700 dark:text-white',
    ].join(' ')}>
      {isPhone ? <Smartphone className="h-2.5 w-2.5" /> : <Cable className="h-2.5 w-2.5" />}
  ...
  return (
    <div className={`${size} rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden shrink-0 border border-slate-200 dark:border-slate-700`}>
      {p.images?.[0]?.url ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          {isPhone ? <Smartphone className="h-4 w-4 text-slate-400 dark:text-slate-500" /> : <Cable className="h-4 w-4 text-slate-400 dark:text-slate-500" />}
        </div>
      )}
    </div>
  );
...
function IconBtn({ to, title, tone, children }: any) {
  const tones: Record<string, string> = {
    blue: 'bg-blue-50 dark:bg-blue-500/15 hover:bg-blue-100 dark:hover:bg-blue-500/25 text-blue-700 dark:text-blue-300',
    ...
    indigo: 'bg-indigo-50 dark:bg-indigo-500/15 hover:bg-indigo-100 dark:hover:bg-indigo-500/25 text-indigo-700 dark:text-indigo-300',
  };
  return (
  ...
  return (
    <Comp
      ...
      className={[
        ...
        active
          ? 'border-blue-500 dark:border-blue-500/60 ring-2 ring-blue-200 dark:ring-blue-500/20'
