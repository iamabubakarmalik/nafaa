...
export default function ElectronicsProductsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const low = products.filter((p) => Number(p.stock || 0) > 0 && Number(p.stock || 0) <= Number(p.lowStockAlert ?? 5));
  ...
  const exportCSV = () => {
    ...
    const head = ['Name', 'Model', 'SKU', 'Barcode', 'Brand', 'Category', 'Condition', 'Cost', 'Retail', 'Stock', 'Warranty'];
    const body = filtered.map((p) => {
      ...
      return [
        ...
        brand?.name || '', prof?.categoryType || '', prof?.conditionType || '',
    ...
    a.href = url; a.download = `electronics-products-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-5 pb-6">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Cpu className="h-3.5 w-3.5 text-amber-300" /> Electronics Store
            </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi icon={Package} label="Total Products" value={stats.total} sub={`${stats.totalStock} pcs`} tone="blue" />
      ...
      <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="inline-flex rounded-2xl border-2 border-slate-200 bg-white overflow-hidden">
            <button onClick={() => setView('grid')}
              className={`px-4 h-12 text-xs font-extrabold transition ${view === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
              <Grid3x3 className="h-4 w-4" />
            </button>
            <button onClick={() => setView('table')}
              className={`px-4 h-12 text-xs font-extrabold border-l-2 border-slate-200 transition ${view === 'table' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
              <List className="h-4 w-4" />
            </button>
          </div>
          <button onClick={exportCSV}
            className="h-12 px-4 rounded-2xl border-2 border-slate-200 hover:border-blue-300 bg-white text-sm font-bold text-slate-700 inline-flex items-center gap-1.5">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <select value={brandId} onChange={(e) => setBrandId(e.target.value)}
            className="h-10 rounded-xl border-2 border-slate-200 bg-white px-3 text-xs font-bold focus:outline-none focus:border-blue-500">
            ...
            {[
              ...
              <button key={o.v} onClick={() => setStockFilter(o.v)}
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold transition ${
                  stockFilter === o.v ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
                }`}>{o.l}</button>
            ))}
          </div>
      ...
      {isLoading ? (
        ...
        <section className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          <div className="mx-auto h-20 w-20 rounded-3xl bg-gradient-to-br from-blue-100 to-cyan-200 flex items-center justify-center">
            <Cpu className="h-9 w-9 text-blue-600" />
          </div>
          <h3 className="mt-5 text-xl font-extrabold text-slate-900">No products found</h3>
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add karo pehla product — 1 minute ka kaam</p>
          <Link to="/electronics-products/new">
            <Button className="mt-5 bg-gradient-to-r from-blue-600 to-cyan-700">
                ...
                {filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-blue-50/40 transition">
                      ...
                      <td className="px-3 py-2.5">
                        <Link to={`/electronics-products/${p.id}`} className="flex items-center gap-2.5 group">
                          <div className="h-10 w-10 rounded-lg bg-slate-100 overflow-hidden shrink-0">
                            {p.images?.[0]?.url ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center"><Cpu className="h-4 w-4 text-slate-400" /></div>
                            )}
                          </div>
                          <div className="min-w-0">
                            <div className="font-extrabold text-slate-900 text-sm truncate group-hover:text-blue-700">{p.name}</div>
                      ...
                      <td className="px-3 py-2.5 text-xs font-bold text-slate-600">{profile?.categoryType?.replace(/_/g, ' ') || '—'}</td>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center justify-end gap-1">
                          <Link to={`/electronics-products/${p.id}`} className="h-8 w-8 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 flex items-center justify-center">
                            <Eye className="h-3.5 w-3.5" />
                          </Link>
                          <Link to={`/electronics-products/${p.id}/edit`} className="h-8 w-8 rounded-lg bg-violet-50 hover:bg-violet-100 text-violet-700 flex items-center justify-center">
...
function ProductCard({ p, profile, selected, onToggle }: any) {
  ...
  return (
    <div className={[
      ...
      selected ? 'border-blue-500 ring-2 ring-blue-200' : isOut ? 'border-rose-200' : isLow ? 'border-amber-200' : 'border-slate-200',
    ].join(' ')}>
      <button onClick={onToggle}
        className={[
          'absolute top-2 left-2 z-10 h-6 w-6 rounded-lg border-2 flex items-center justify-center transition',
          selected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white/90 border-slate-300 opacity-0 group-hover:opacity-100',
        ].join(' ')}>
      ...
      <Link to={`/electronics-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 overflow-hidden relative">
          ...
          {profile?.isBestSeller && (
            <div className="absolute top-2 right-2 mt-8 h-6 px-2 rounded bg-orange-600 flex items-center justify-center shadow text-[8px] font-extrabold text-white uppercase">
              Bestseller
            </div>
          )}
          {(isOut || isLow) && (
        ...
        <div className="p-2.5">
          ...
          {profile?.modelNumber && (
            <div className="mt-1 text-[9px] font-mono text-slate-500 truncate">{profile.modelNumber}</div>
          )}
          ...
          {profile?.warrantyMonths ? (
            <div className="mt-2 inline-flex items-center gap-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded-md bg-blue-100 text-blue-700">
              <Award className="h-2.5 w-2.5" /> {profile.warrantyMonths}m warranty
            </div>
          ) : null}
        </div>
      </Link>
...
function Kpi({ icon: Icon, label, value, sub, tone, onClick }: any) {
  const tones: Record<string, string> = {
    blue: 'from-blue-500 to-cyan-700 shadow-blue-500/30',
  ...
  return (
    <Comp onClick={onClick}
      className={[
        ...
        onClick ? 'hover:border-blue-300 hover:shadow-md transition' : '',
      ].join(' ')}>
