...
import {
  ...
  RefreshCw, Globe, Phone, Mail, ExternalLink, CheckCircle2,
  Sparkles, Grid3x3, List, Save, Package, TrendingUp,
} from 'lucide-react';
import { toast } from 'sonner';
import { electronicsBrandsApi, type ElectronicsBrand } from '../api/brands.api';
...
export default function ElectronicsBrandsPage() {
  ...
  const [editing, setEditing] = useState<ElectronicsBrand | null>(null);

  const { data: brands = [], isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['electronics-brands', authorizedOnly, featuredOnly],
    queryFn: () => electronicsBrandsApi.list({
  ...
  const removeMutation = useMutation({
    mutationFn: (id: string) => electronicsBrandsApi.remove(id),
    onSuccess: () => {
  ...
  const toggleFeatured = useMutation({
    mutationFn: (id: string) => electronicsBrandsApi.toggleFeatured(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['electronics-brands'] }),
  });
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Award className="h-3.5 w-3.5 text-amber-300" /> Electronics Brands
            </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi icon={Award} label="Total Brands" value={stats.total} sub="All brands" tone="violet" />
        <Kpi icon={CheckCircle2} label="Authorized" value={stats.authorized} sub="Dealer status"
          tone="emerald" onClick={() => setAuthorizedOnly(!authorizedOnly)} />
        <Kpi icon={Star} label="Featured" value={stats.featured} sub="Highlighted"
          tone="amber" onClick={() => setFeaturedOnly(!featuredOnly)} />
        <Kpi icon={TrendingUp} label="Total Revenue" value={formatPKR(stats.totalRevenue)} sub="All-time" tone="blue" />
      </section>
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          ...
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add first brand — Apple, Samsung, Xiaomi</p>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((b) => (
                <tr key={b.id} className="hover:bg-violet-50/40">
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-3">
                      {b.logoUrl ? (
                        ...
                        <div className="h-10 w-10 rounded-lg bg-violet-100 text-violet-700 flex items-center justify-center font-extrabold">
                          {b.name.charAt(0)}
                        </div>
                      )}
                      <div>
                  ...
                  <td className="px-3 py-2.5 text-right font-extrabold tabular-nums">{b.productCount || 0}</td>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
            ...
            {b.dealerCode && (
              <span className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 text-[9px] font-mono">
                {b.dealerCode}
              </span>
            )}
          </div>
        ...
        <button onClick={onToggleFeatured}
          className={['h-8 w-8 rounded-lg flex items-center justify-center shrink-0',
            b.isFeatured ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-400'].join(' ')}>
...
function BrandFormModal({ editing, onClose, onSaved }: any) {
  ...
  const save = useMutation({
    mutationFn: () => editing
      ? electronicsBrandsApi.update(editing.id, form)
      : electronicsBrandsApi.create(form),
    onSuccess: () => {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.countryOfOrigin} onChange={(e) => setForm({ ...form, countryOfOrigin: e.target.value })}
                placeholder="USA, South Korea, China..."
                className="h-11 w-full rounded-xl border-2 border-slate-200 px-3 text-sm font-bold focus:outline-none focus:border-violet-500" />
            </div>
            <div>
              <label className="block text-xs font-extrabold uppercase text-slate-600 mb-1.5">Website URL</label>
          ...
          <div>
            ...
            <textarea rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Brand ke baare mein short info..."
          ...
          <div className="rounded-2xl bg-emerald-50 border-2 border-emerald-200 p-4 space-y-3">
            ...
            {form.authorizedDealer && (
              <div>
                <label className="block text-xs font-extrabold uppercase text-emerald-700 mb-1.5">Dealer Code</label>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-3 gap-2">
              <input value={form.supportContact} onChange={(e) => setForm({ ...form, supportContact: e.target.value })}
