...
import {
  ArrowLeft, Edit3, ShoppingBag, Boxes, Package, PackageX,
  DollarSign, TrendingUp, Star, ChevronRight, Cpu, Wifi,
...
export default function ElectronicsProductDetailPage() {
  const { id } = useParams();
  ...
  const { data: profile } = useQuery({
    queryKey: ['electronics-profile', id],
  ...
  const { data: serials = [] } = useQuery({
    ...
    queryFn: () => serialTrackingApi.list({ productId: id! }),
    enabled: !!id,
  });
  ...
  const stats = useMemo(() => {
    ...
    const serialWarrantyActive = serials.filter((s: any) => s.warrantyStatus === 'ACTIVE').length;
    ...
    return {
      ...
      serialInStock, serialSold, serialWarrantyActive,
    };
  }, [product, soldLines, variants, serials]);
  ...
  const TABS: { id: Tab; label: string; count?: number; icon: any }[] = [
    ...
    { id: 'specs', label: 'Tech Specs', icon: Sparkles },
    { id: 'warranty', label: 'Warranty & Box', icon: Shield },
    { id: 'variants', label: 'Variants', count: stats.variantCount, icon: Boxes },
    { id: 'serials', label: 'Serials/IMEI', count: stats.serialCount, icon: Barcode },
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      {/* HERO */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-blue-900 to-cyan-700 text-white shadow-2xl">
        ...
        <div className="relative grid lg:grid-cols-[280px_1fr] gap-6 p-6">
          <div className="space-y-2">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-white/10 backdrop-blur border-2 border-white/20">
              {gallery[imgIndex]?.url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-white/50"><Cpu className="h-16 w-16" /></div>
              )}
              {profile?.isFeatured && (
          ...
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Cpu className="h-3.5 w-3.5 text-amber-300" /> Electronics
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
              ...
              <HeroStat icon={Barcode} label="Serials" value={String(stats.serialCount)} sub={`${stats.serialInStock} in stock`} tone="amber" />
            </div>
          </div>
        </div>
      </section>
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={['px-4 py-2.5 rounded-xl text-sm font-extrabold inline-flex items-center gap-2 transition',
                  active ? 'bg-gradient-to-br from-blue-600 to-cyan-700 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'].join(' ')}>
              ...
              <div className="h-[240px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#3b82f6" strokeWidth={2.5} fill="url(#detailGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
      ...
      {/* SPECS */}
      {tab === 'specs' && (
        <Panel icon={Sparkles} title="Tech Specifications" desc="All technical details" tone="blue"
          empty={!profile} emptyText="No specs added yet"
          emptyAction={<Link to={`/electronics-products/${id}/edit`}><Button className="bg-blue-600"><Edit3 className="h-4 w-4" /> Add Specs</Button></Link>}>
          {profile && (
            <div className="p-5 space-y-5">
              {(profile.batteryCapacity || profile.batteryLifeHours || profile.powerRating) && (
                <SpecSection icon={Battery} title="Battery & Power" tone="emerald">
                  {profile.batteryCapacity && <SpecRow label="Capacity" value={profile.batteryCapacity} />}
                  {profile.batteryLifeHours && <SpecRow label="Battery Life" value={`${profile.batteryLifeHours} hours`} />}
                  {profile.chargingTimeMinutes && <SpecRow label="Charging Time" value={`${profile.chargingTimeMinutes} minutes`} />}
                  {profile.powerRating && <SpecRow label="Power Rating" value={profile.powerRating} />}
                </SpecSection>
              )}

              {profile.connectivity?.length > 0 && (
                <SpecSection icon={Wifi} title="Connectivity" tone="blue">
                  <div className="flex flex-wrap gap-1.5">
                    {profile.connectivity.map((c) => (
                      <span key={c} className="px-2.5 py-1 rounded-lg bg-blue-100 text-blue-800 text-xs font-extrabold">{c}</span>
                    ))}
                  </div>
                </SpecSection>
              )}

              {(profile.screenSize || profile.resolution || profile.refreshRate) && (
                <SpecSection icon={Monitor} title="Display" tone="violet">
                  {profile.screenSize && <SpecRow label="Screen Size" value={profile.screenSize} />}
                  {profile.resolution && <SpecRow label="Resolution" value={profile.resolution} />}
                  {profile.refreshRate && <SpecRow label="Refresh Rate" value={profile.refreshRate} />}
                </SpecSection>
              )}

              {(profile.compatibleOS?.length > 0 || profile.compatibleWith?.length > 0) && (
                <SpecSection icon={Cpu} title="Compatibility" tone="amber">
                  {profile.compatibleOS?.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase font-extrabold text-slate-500 mb-1.5">Operating Systems</div>
                      <div className="flex flex-wrap gap-1.5">
                        {profile.compatibleOS.map((os) => (
                          <span key={os} className="px-2.5 py-1 rounded-lg bg-amber-100 text-amber-800 text-xs font-extrabold">{os}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {(profile.waterResistance || profile.operatingRange) && (
                <SpecSection icon={Droplets} title="Durability" tone="sky">
                  {profile.waterResistance && <SpecRow label="Water Rating" value={profile.waterResistance} />}
                  {profile.operatingRange && <SpecRow label="Operating Range" value={profile.operatingRange} />}
                </SpecSection>
              )}

              {(profile.weightGrams || profile.lengthMm || profile.widthMm || profile.heightMm) && (
                <SpecSection icon={Ruler} title="Dimensions & Weight" tone="slate">
                  {profile.weightGrams && <SpecRow label="Weight" value={`${profile.weightGrams} g`} />}
                  {(profile.lengthMm || profile.widthMm || profile.heightMm) && (
                    <SpecRow label="Dimensions" value={`${profile.lengthMm || '?'} × ${profile.widthMm || '?'} × ${profile.heightMm || '?'} mm`} />
                  )}
                </SpecSection>
              )}
            </div>
      ...
      {/* WARRANTY */}
      {tab === 'warranty' && (
        <Panel icon={Shield} title="Warranty & Box Contents" desc="What comes with this product" tone="blue"
          empty={!profile} emptyText="No warranty info yet">
          {profile && (
            <div className="p-5 space-y-5">
              <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 border-2 border-blue-200 p-5">
                <div className="flex items-center gap-3 mb-3">
                  <Shield className="h-6 w-6 text-blue-700" />
                  <h3 className="font-extrabold text-blue-900 text-lg">Warranty Details</h3>
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <InfoBox label="Warranty Period" value={profile.warrantyMonths ? `${profile.warrantyMonths} months` : 'Not specified'} tone="blue" />
                  <InfoBox label="Type" value={profile.warrantyType || 'Not set'} tone="violet" />
                  <InfoBox label="Coverage"
                    value={profile.hasInternationalWarranty ? '🌍 International' : '🇵🇰 Local'}
                    tone={profile.hasInternationalWarranty ? 'emerald' : 'amber'} />
                  <InfoBox label="IMEI Tracked" value={profile.hasImei ? '✓ Yes' : '✗ No'} tone={profile.hasImei ? 'emerald' : 'slate'} />
                </div>
              </div>
              ...
              {profile.boxContents?.length > 0 && (
                <div className="rounded-2xl bg-white border-2 border-slate-200 p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profile.boxContents.map((item, i) => (
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2">
                    {profile.hasManual && (
                      <div className="inline-flex items-center gap-2 text-xs font-extrabold text-slate-700">
                        <FileText className="h-4 w-4 text-slate-500" /> User Manual included
                      </div>
                    )}
                    {profile.hasWarrantyCard && (
                      <div className="inline-flex items-center gap-2 text-xs font-extrabold text-slate-700">
                        <Shield className="h-4 w-4 text-slate-500" /> Warranty Card included
                      </div>
                    )}
                  </div>
                </div>
      ...
      {tab === 'variants' && (
        <Panel icon={Boxes} title="Variants" desc={`${stats.variantCount} variants • total stock ${stats.variantStock}`} tone="violet"
          ...
          emptyAction={<Link to={`/electronics-products/${id}/edit`}><Button variant="secondary"><Plus className="h-4 w-4" /> Add Variants</Button></Link>}>
      ...
      {/* SERIALS */}
      {tab === 'serials' && (
        <Panel icon={Barcode} title="Serial / IMEI Tracking" desc={`${stats.serialCount} total • ${stats.serialInStock} in stock • ${stats.serialSold} sold`} tone="amber"
          empty={serials.length === 0}
          emptyText="No serials added"
          emptyAction={<Link to={`/electronics-products/${id}/edit`}><Button variant="secondary"><Plus className="h-4 w-4" /> Add Serials</Button></Link>}>
          <div className="p-4">
            <div className="mb-4 grid grid-cols-4 gap-2">
              <SerialStat label="In Stock" value={stats.serialInStock} tone="emerald" />
              <SerialStat label="Sold" value={stats.serialSold} tone="blue" />
              <SerialStat label="Warranty Active" value={stats.serialWarrantyActive} tone="violet" />
              <SerialStat label="Total" value={stats.serialCount} tone="amber" />
            </div>
                  ...
                  {serials.map((s: any) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50/60">
                        ...
                        <td className="px-3 py-2.5 font-mono text-xs text-slate-700">{s.imei || '—'}</td>
                        <td className="px-3 py-2.5 font-mono text-xs text-slate-700">{s.macAddress || '—'}</td>
        ...
        <Panel icon={Receipt} title="Sales History" desc={`${salesForProduct.length} recent • ${stats.totalSold} sold`} tone="emerald"
          ...
          <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
            {salesForProduct.map((s: any) => {
              ...
              return (
                <Link key={s.id} to={`/sales/${s.id}/receipt`} className="block px-5 py-3 hover:bg-blue-50/40 transition">
...
function SerialStat({ label, value, tone }: any) {
  const tones: Record<string, string> = {
...
function QuickLink({ to, icon: Icon, title, desc, tone }: any) {
  ...
  return (
    <Link to={to} className="rounded-2xl bg-white border-2 border-slate-200 hover:border-blue-400 hover:shadow-md p-4 flex items-center gap-3 transition group">
