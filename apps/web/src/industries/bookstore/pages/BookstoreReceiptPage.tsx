...
import {
  Printer, ArrowLeft, MessageCircle, X, BookOpen, Palette, Pencil,
  MapPin, Phone, Calendar, User, CheckCircle2, ShieldAlert, Tag,
  Package, Award, Star, Hash, Barcode,
} from 'lucide-react';
...
export default function BookstoreReceiptPage() {
  const { id } = useParams();
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Our Bookstore';
    ...
    lines.push('');
    lines.push('_Visit again for more books & stationery!_ ❤️');
  ...
  const shopName = settings?.shopName || sale.tenant?.name || 'Bookstore';
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <BookOpen className="h-3 w-3" /> Bookstore Invoice
                    </div>
