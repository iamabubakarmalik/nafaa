...
import {
  Sofa, Plus, Search, X, RefreshCw, Download, Grid3x3, List,
  Package, AlertTriangle, DollarSign, Eye, Edit3, Trash2,
  CheckCircle2, Star, Leaf, Hammer, PackageX,
} from 'lucide-react';
...
const CATEGORIES = [
  'SOFA_SET', 'SOFA_L_SHAPE', 'BED_KING', 'BED_DOUBLE', 'WARDROBE',
  'DINING_TABLE', 'DINING_SET', 'OFFICE_DESK', 'BOOKSHELF', 'TV_CONSOLE',
];
...
export default function FurnitureProductsPage() {
  ...
  const { data: profiles = [] } = useQuery({
    queryKey: ['furniture-profiles-all'],
    queryFn: () => furnitureProductsApi.list(),
  });
  ...
  const stats = useMemo(() => ({
    ...
    lowCount: products.filter((p) => Number(p.stock || 0) > 0 && Number(p.stock || 0) <= Number(p.lowStockAlert ?? 2)).length,
    outCount: products.filter((p) => Number(p.stock || 0) <= 0).length,
    customizable: (profiles as any[]).filter((p) => p.isCustomizable).length,
  }), [products, profiles]);
  ...
  const filtered = useMemo(() => {
    ...
    if (material !== 'all') list = list.filter((p) => pmap.get(p.id)?.primaryMaterial === material);
    if (customizableOnly) list = list.filter((p) => pmap.get(p.id)?.isCustomizable);
    if (ecoOnly) list = list.filter((p) => pmap.get(p.id)?.isEcoFriendly);
    if (stockFilter !== 'all') list = list.filter((p) => {
      const s = Number(p.stock || 0), a = Number(p.lowStockAlert ?? 2);
    ...
    return list;
  }, [products, search, categoryType, material, customizableOnly, ecoOnly, stockFilter, sortKey, pmap]);
  ...
  const exportCSV = () => {
    ...
    const head = ['Name', 'SKU', 'Category', 'Material', 'Brand', 'Cost', 'Retail', 'Stock', 'Warranty'];
    const rows = filtered.map((p) => {
      const pr = pmap.get(p.id);
      return [p.name, p.sku || '', pr?.categoryType || '', pr?.primaryMaterial || '',
        pr?.brand || '', Number(p.costPrice || 0).toFixed(2), Number(p.price || 0).toFixed(2),
        Number(p.stock || 0), pr?.warrantyMonths ? `${pr.warrantyMonths}m` : ''];
    });
    const csv = [head, ...rows].map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8' }));
    const a = document.createElement('a');
    a.href = url; a.download = `furniture-products-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="space-y-5 pb-6">
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20">
              <Sofa className="h-3.5 w-3.5 text-amber-300" /> Furniture Catalogue
            </div>
            <h1 className="mt-3 text-3xl sm:text-4xl font-extrabold">Products</h1>
            <p className="mt-2 text-sm text-white/80">
              {stats.total} products • {stats.totalStock} pieces • Value{' '}
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <PrivacyToggle />
            <Link to="/furniture-products/new"
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Kpi icon={Package} label="Products" value={stats.total} sub={`${stats.totalStock} pcs`} tone="amber" />
        <Kpi icon={DollarSign} label="Stock Value" value={formatPKR(stats.stockValue)} sub="Retail" tone="emerald" />
        <Kpi icon={Hammer} label="Customizable" value={stats.customizable} sub="Custom option" tone="violet" onClick={() => setCustomizableOnly((v) => !v)} />
        <Kpi icon={AlertTriangle} label="Low Stock" value={stats.lowCount} sub="Reorder" tone="orange" onClick={() => setStockFilter('low')} />
      ...
      <section className="rounded-3xl bg-white border-2 border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="inline-flex rounded-2xl border-2 border-slate-200 bg-white overflow-hidden">
            <button onClick={() => setView('grid')} className={`px-4 h-12 ${view === 'grid' ? 'bg-amber-700 text-white' : 'text-slate-600'}`}>
              <Grid3x3 className="h-4 w-4" />
            </button>
            <button onClick={() => setView('table')} className={`px-4 h-12 border-l-2 border-slate-200 ${view === 'table' ? 'bg-amber-700 text-white' : 'text-slate-600'}`}>
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <select value={material} onChange={(e) => setMaterial(e.target.value)}
            ...
            <option value="FABRIC_VELVET">Velvet</option>
          </select>
          <button onClick={() => setEcoOnly((v) => !v)}
            className={`h-10 px-3 rounded-xl border-2 text-xs font-extrabold inline-flex items-center gap-1.5 ${ecoOnly ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-200 text-slate-700'}`}>
          ...
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
            {(['all', 'in', 'low', 'out'] as StockFilter[]).map((v) => (
              <button key={v} onClick={() => setStockFilter(v)}
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold ${stockFilter === v ? 'bg-amber-700 text-white' : 'text-slate-600'}`}>
      ...
      {isLoading ? (
        ...
        <section className="rounded-3xl bg-white border-2 border-dashed border-slate-300 p-16 text-center">
          <div className="mx-auto h-20 w-20 rounded-3xl bg-gradient-to-br from-amber-100 to-orange-200 flex items-center justify-center">
            <Sofa className="h-9 w-9 text-amber-700" />
          </div>
          <h3 className="mt-5 text-xl font-extrabold text-slate-900">No products found</h3>
          <p className="text-sm text-slate-500 mt-2 font-semibold">Add your first sofa, bed or wardrobe</p>
          <Link to="/furniture-products/new">
            <Button className="mt-5 bg-gradient-to-r from-amber-700 to-orange-800"><Plus className="h-4 w-4" /> Add Product</Button>
          </Link>
        </section>
      ) : view === 'grid' ? (
        <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {filtered.map((p) => <ProductCard key={p.id} p={p} pr={pmap.get(p.id)} sel={selected.has(p.id)} onToggle={() => toggle(p.id)} />)}
        </section>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((p) => {
                ...
                const isOut = stock <= 0;
                const isLow = !isOut && stock <= Number(p.lowStockAlert ?? 2);
                return (
                  <tr key={p.id} className="hover:bg-amber-50/40">
                    ...
                    <td className="px-3 py-2.5">
                      <Link to={`/furniture-products/${p.id}`} className="flex items-center gap-2.5 group">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[10px] font-mono text-slate-500 truncate">{p.sku || pr?.brand || '—'}</div>
                        </div>
                      </Link>
                    </td>
                    <td className="px-3 py-2.5 text-xs font-bold text-slate-700">{pr?.categoryType?.replace(/_/g, ' ') || '—'}</td>
                    <td className="px-3 py-2.5 text-xs font-bold text-slate-600">{pr?.primaryMaterial?.replace(/_/g, ' ') || '—'}</td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/furniture-products/${p.id}`} className="h-8 w-8 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 flex items-center justify-center">
                          <Eye className="h-3.5 w-3.5" />
                        </Link>
                        <Link to={`/furniture-products/${p.id}/edit`} className="h-8 w-8 rounded-lg bg-orange-50 hover:bg-orange-100 text-orange-700 flex items-center justify-center">
...
function ProductCard({ p, pr, sel, onToggle }: any) {
  const stock = Number(p.stock || 0);
  const isOut = stock <= 0;
  const isLow = !isOut && stock <= Number(p.lowStockAlert ?? 2);
  return (
    <div className={`group relative rounded-2xl bg-white border-2 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 ${
      sel ? 'border-amber-500 ring-2 ring-amber-200' : isOut ? 'border-rose-200' : isLow ? 'border-amber-200' : 'border-slate-200'}`}>
      ...
      <Link to={`/furniture-products/${p.id}`} className="block">
        <div className="aspect-square bg-slate-100 relative overflow-hidden">
          {p.images?.[0]?.url
            ...
            : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50"><Sofa className="h-10 w-10 text-amber-300" /></div>}
          {pr?.isEcoFriendly && (
            <div className="absolute top-2 left-2 px-1.5 py-0.5 rounded-md bg-green-600 text-white text-[9px] font-extrabold inline-flex items-center gap-0.5">
        ...
        <div className="p-2.5">
          ...
          {pr?.categoryType && <div className="mt-1 text-[9px] font-extrabold uppercase text-amber-700">{pr.categoryType.replace(/_/g, ' ')}</div>}
          ...
          {pr?.warrantyMonths ? (
            <div className="mt-2 inline-flex items-center gap-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded-md bg-blue-100 text-blue-700">
              🛡️ {pr.warrantyMonths}m warranty
            </div>
          ) : null}
        </div>
      </Link>
...
function Kpi({ icon: Icon, label, value, sub, tone, onClick }: any) {
  const tones: Record<string, string> = {
    amber: 'from-amber-600 to-orange-800', emerald: 'from-emerald-500 to-emerald-700',
    violet: 'from-violet-500 to-purple-700', orange: 'from-orange-500 to-red-600', rose: 'from-rose-500 to-rose-700',
  };
