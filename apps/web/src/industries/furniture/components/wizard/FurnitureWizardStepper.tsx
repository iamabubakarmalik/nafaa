import { Check, Sofa, Ruler, Palette, Package } from 'lucide-react';
import type { WizardStep } from '../../hooks/useFurnitureWizard';
...
const STEPS = [
  { id: 1, label: 'Basic Info', short: 'Info', desc: 'Name, category, price', icon: Sofa },
  { id: 2, label: 'Dimensions', short: 'Size', desc: 'L×W×H, capacity', icon: Ruler },
  { id: 3, label: 'Materials & Delivery', short: 'Details', desc: 'Wood, fabric, assembly', icon: Palette },
  { id: 4, label: 'Stock & Variants', short: 'Stock', desc: 'Quantity, editions', icon: Package },
] as const;

export function FurnitureWizardStepper({ currentStep, stepValidation, onStepClick }: Props) {
  return (
            ...
            <div key={s.id} className="flex items-center gap-1.5 flex-1">
              <button
                ...
                className={[
                  ...
                  isActive
                    ? 'bg-gradient-to-br from-amber-700 to-orange-800 text-white shadow-md shadow-amber-500/40 scale-[1.02]'
                    : isPast
              ...
              >
                <div className={[
                  ...
                  isActive ? 'bg-white/25 text-white'
                    : isPast && isComplete ? 'bg-amber-700 text-white'
              ...
              {idx < STEPS.length - 1 && (
                <div className={['h-1 w-3 sm:w-4 rounded-full transition',
                  currentStep > s.id ? 'bg-amber-600' : 'bg-slate-200'].join(' ')} />
              )}
            </div>
          );
        })}
