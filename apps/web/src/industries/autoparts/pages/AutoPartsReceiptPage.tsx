...
import {
  Printer, ArrowLeft, MessageCircle, X, Wrench, ShieldCheck, ShieldAlert,
  Award, MapPin, Phone, Calendar, User, CheckCircle2, Hash, Package, Car,
} from 'lucide-react';
...
export default function AutoPartsReceiptPage() {
  const { id } = useParams();
  ...
  const handleWhatsApp = () => {
    ...
    const shopName = sale.tenant?.settings?.shopName || sale.tenant?.name || 'Auto Parts Shop';
    ...
    lines.push('');
    lines.push('_Warranty applies as per part specifications. Keep this invoice._');

    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(lines.join('\n'))}`, '_blank');
  };

  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center"><div className="h-10 w-10 rounded-full border-4 border-slate-200 border-t-slate-700 animate-spin" /></div>;
  }
  if (!sale) {
    return <div className="min-h-screen flex flex-col items-center justify-center"><p className="font-bold">Receipt not found</p><Link to="/sales" className="mt-4 text-slate-700 hover:underline">← Back</Link></div>;
  }

  const settings = sale.tenant?.settings;
  const shopName = settings?.shopName || sale.tenant?.name || 'Auto Parts Shop';
  ...
  // Parse vehicle info from sale note if present
  const vehicleNote = (sale as any).note && (sale as any).note.startsWith('Vehicle:') ? (sale as any).note.replace('Vehicle:', '').trim() : null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-6 px-4 print:bg-white print:py-0 print:px-0">
      <div className={`mx-auto space-y-4 ${format === 'a4' ? 'max-w-4xl' : 'max-w-md'}`}>
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-slate-700 to-slate-900 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="inline-flex rounded-xl border-2 border-slate-200 bg-white shadow-sm overflow-hidden">
              {(['a4', 'thermal80', 'thermal58'] as Format[]).map((f, i) => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`px-3 py-2.5 text-xs font-bold transition ${format === f ? 'bg-slate-800 text-white' : 'text-slate-700 hover:bg-slate-50'} ${i > 0 ? 'border-l-2 border-slate-200' : ''}`}>
            ...
            <button onClick={() => window.print()}
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-slate-700 to-slate-900 hover:from-slate-800 hover:to-black px-4 py-2.5 text-sm font-bold text-white shadow-md">
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
            <div className="relative bg-gradient-to-br from-slate-950 via-slate-800 to-slate-700 text-white px-8 py-7 print:bg-white print:text-slate-900 print:border-b-4 print:border-double print:border-slate-700 overflow-hidden">
              <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full bg-slate-400/20 blur-2xl print:hidden" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <Wrench className="h-3 w-3" /> Auto Parts Invoice
                    </div>
            ...
            <div className="px-8 py-6">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b-2 border-slate-300 bg-gradient-to-r from-slate-50 to-white print:bg-white">
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase w-8">#</th>
                    <th className="py-3 px-2 font-extrabold text-[10px] uppercase">Part / Description</th>
                    ...
                    <tr key={it.id} className="border-b border-slate-100 align-top">
                      ...
                      <td className="py-3 px-2">
                        ...
                        {(it.product.sku || it.product.barcode) && (
                          <div className="text-[10px] text-slate-500 font-mono">{it.product.sku || it.product.barcode}</div>
                        )}
                        {it.note && (
                          <div className="mt-1 inline-flex items-start gap-1 px-2 py-1 rounded-md bg-slate-100 border border-slate-300 text-[10px] font-mono font-bold text-slate-800 print:bg-white print:border-slate-400">
                            <Hash className="h-2.5 w-2.5 mt-0.5 shrink-0" />
                            <span>{it.note}</span>
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-2 text-center font-extrabold text-slate-900 tabular-nums">{it.quantity} {it.product.unit}</td>
            ...
            {/* Vehicle info (if noted on sale) */}
            {vehicleNote && (
              <div className="px-8 pb-4">
                <div className="rounded-xl bg-fuchsia-50 border-2 border-fuchsia-200 p-3 flex items-center gap-2 print:bg-white print:border-slate-400">
                  <Car className="h-4 w-4 text-fuchsia-700" />
                  <div>
                    <div className="text-[10px] uppercase font-extrabold text-fuchsia-700">Vehicle Fitted</div>
                    <div className="text-sm font-mono font-bold text-fuchsia-900">{vehicleNote}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="px-8 py-6 border-t-2 border-dashed border-slate-300 bg-gradient-to-br from-slate-50 to-white print:bg-white">
              <div className="ml-auto max-w-md space-y-1.5">
                ...
                <div className="flex items-center justify-between text-lg pt-3 border-t-2 border-slate-300">
                  ...
                  <span className="font-extrabold text-slate-800 text-3xl tabular-nums">{formatPKR(sale.total)}</span>
                </div>
            ...
            {/* Warranty terms */}
            <div className="px-8 py-5 border-t-2 border-slate-100 bg-slate-50/50 print:bg-white">
              <div className="text-[10px] uppercase font-extrabold text-slate-700 mb-2 flex items-center gap-1">
                <ShieldCheck className="h-3 w-3" /> Parts Warranty Terms
              </div>
              <ul className="text-[11px] text-slate-700 space-y-1 list-disc pl-4">
                <li>Warranty as per individual part specifications (see part number for details)</li>
                <li>Physical damage, improper installation, and accidents void warranty</li>
                <li>Original invoice required for any warranty claim</li>
                <li>Consumable parts (oils, filters, bulbs) are not covered under warranty</li>
                <li>Return/exchange within 3 days with original packaging (unopened, unfitted)</li>
              </ul>
            </div>

            <div className="px-8 py-5 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-slate-50 to-slate-100 print:bg-white">
        ...
        {(format === 'thermal58' || format === 'thermal80') && (
          <div className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`}
            ...
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1">
                <div className="font-bold text-center mb-1">PARTS</div>
                {sale.items.map((item: any, idx: number) => (
                  <div key={item.id} className="mb-1">
                    <div className="font-bold">{idx + 1}. {item.product.name}</div>
                    {item.note && <div className="pl-2 text-[9px] font-mono">{item.note}</div>}
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-1 text-[9px] italic">
                ...
                <div>• As per part specifications</div>
                <div>• Keep invoice for claims</div>
                <div>• No warranty on consumables</div>
              </div>
