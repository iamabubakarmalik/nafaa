...
import {
  ...
  Calendar, User, CheckCircle2, ShieldAlert, Tag, Package, Barcode,
} from 'lucide-react';
...
export default function RetailReceiptPage() {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center"><div className="h-10 w-10 rounded-full border-4 border-sky-200 border-t-sky-600 animate-spin" /></div>;
  }
  if (!sale) {
    return <div className="min-h-screen flex flex-col items-center justify-center"><p className="font-bold">Receipt not found</p><Link to="/sales" className="mt-4 text-sky-600 hover:underline">← Back</Link></div>;
  }
  ...
  const totalQty = sale.items.reduce((s: number, it: any) => s + Number(it.quantity || 0), 0);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-sky-50 py-6 px-4 print:bg-white print:py-0 print:px-0">
      <div className={`mx-auto space-y-4 ${format === 'a4' ? 'max-w-4xl' : 'max-w-md'}`}>
        {isAutoOpened && (
          <div className="rounded-2xl bg-gradient-to-r from-sky-500 to-cyan-600 text-white px-5 py-3 flex items-center gap-3 shadow-lg print:hidden">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="inline-flex rounded-xl border-2 border-slate-200 bg-white shadow-sm overflow-hidden">
              {(['a4', 'thermal80', 'thermal58'] as Format[]).map((f, i) => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`px-3 py-2.5 text-xs font-bold transition ${format === f ? 'bg-sky-600 text-white' : 'text-slate-700 hover:bg-slate-50'} ${i > 0 ? 'border-l-2 border-slate-200' : ''}`}>
            ...
            <button onClick={() => window.print()}
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-sky-600 to-cyan-700 hover:from-sky-700 hover:to-cyan-800 px-4 py-2.5 text-sm font-bold text-white shadow-md">
        ...
        {format === 'a4' && (
          <div className="receipt-a4 bg-white shadow-2xl rounded-3xl border overflow-hidden print:shadow-none print:border-none print:rounded-none">
            <div className="relative bg-gradient-to-br from-slate-950 via-sky-900 to-cyan-700 text-white px-8 py-7 print:bg-white print:text-slate-900 print:border-b-4 print:border-double print:border-slate-700 overflow-hidden">
              <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full bg-sky-400/20 blur-2xl print:hidden" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold print:hidden">
                      <ShoppingCart className="h-3 w-3" /> Sales Receipt
                    </div>
            ...
            {sale.customer && (
              <div className="px-8 py-4 border-b-2 border-slate-100 bg-slate-50/50 flex items-start justify-between print:bg-white">
                ...
                <div className="text-right">
                  ...
                  <div className="font-extrabold text-lg">{sale.items.length} lines • {totalQty.toFixed(0)} qty</div>
                </div>
              </div>
            )}
            ...
            <div className="px-8 py-6">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b-2 border-slate-300 bg-gradient-to-r from-sky-50 to-slate-50 print:bg-white">
                    ...
                    <tr key={it.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="py-3 px-2">
                        ...
                        {(it.product.sku || it.product.barcode) && (
                          <div className="text-[10px] text-slate-500 font-mono inline-flex items-center gap-1">
                            <Barcode className="h-2.5 w-2.5" />
                            {it.product.sku || it.product.barcode}
                          </div>
                        )}
                        {it.note && (
                          <div className="text-[10px] italic text-amber-700 mt-0.5">📝 {it.note}</div>
                        )}
                      </td>
            ...
            <div className="px-8 py-6 border-t-2 border-dashed border-slate-300 bg-gradient-to-br from-slate-50 to-white print:bg-white">
              <div className="ml-auto max-w-md space-y-1.5">
                ...
                {sale.discount > 0 && (
                  <div className="flex justify-between text-sm"><span className="text-amber-700 inline-flex items-center gap-1"><Tag className="h-3 w-3" />Discount</span><span className="font-bold text-amber-700 tabular-nums">-{formatPKR(sale.discount)}</span></div>
                )}
                <div className="flex items-center justify-between text-lg pt-3 border-t-2 border-slate-300">
                  ...
                  <span className="font-extrabold text-sky-700 text-3xl tabular-nums">{formatPKR(sale.total)}</span>
                </div>
            ...
            <div className="px-8 py-5 text-center border-t-2 border-double border-slate-300 bg-gradient-to-br from-sky-50 to-cyan-50 print:bg-white">
          ...
          <div className={`receipt-thermal bg-white shadow-2xl print:shadow-none mx-auto ${format === 'thermal58' ? 'w-[58mm]' : 'w-[80mm]'}`}
            ...
            <div className={`${format === 'thermal58' ? 'p-2 text-[10px]' : 'p-3 text-[11px]'} leading-tight`}>
              ...
              <div className="border-t border-dashed border-slate-400 pt-1 mb-2 text-center text-[9px]">
                {sale.items.length} items • {totalQty.toFixed(0)} total qty
              </div>
