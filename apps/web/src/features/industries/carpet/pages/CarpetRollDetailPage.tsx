...
import {
  ...
  Sliders, Trash2, Package, History, TrendingUp,
  Sparkles, BarChart3, Clock, Award, Calendar, Hash,
  ChevronRight, Eye, EyeOff, ArrowUpRight, ArrowDownRight, Box,
  FileText, Zap, TrendingDown,
} from 'lucide-react';
...
export default function CarpetRollDetailPage() {
  ...
  const analytics = useMemo(() => {
    ...
    return { fullWidth, originalReal, remainingReal, soldLength, soldSqft, remainingSqft, originalSqft, estimatedRevenue, estimatedCost, estimatedProfit, profitMargin, percentRemaining, percentSold, lockedCapital, potentialRevenue, potentialProfit };
  }, [roll]);
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <p className="mt-3 text-slate-700 font-extrabold">Loading roll details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <button
          ...
          className={`h-10 px-3 rounded-xl border-2 text-sm font-extrabold inline-flex items-center gap-1.5 transition ${
        ...
        <div className="relative flex items-center justify-between gap-4 flex-wrap">
          <div className="min-w-0 flex-1">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-extrabold border border-white/20 mb-3">
              <Layers className="h-3.5 w-3.5 text-amber-300" /> Carpet Roll Details
            </div>
            <h1 className="text-4xl sm:text-5xl font-extrabold font-mono leading-tight">{roll.rollNumber}</h1>
            <div className="mt-3 flex items-center gap-2 flex-wrap text-base">
              ...
              {roll.designCode && (
                <>
                  <span className="text-white/50">•</span>
                  <span className="font-mono text-white/80 bg-white/10 backdrop-blur rounded-md px-2 py-0.5 text-sm">{roll.designCode}</span>
                </>
              )}
            </div>
            {(roll.shop || roll.rackNumber) && (
              <div className="mt-2 inline-flex items-center gap-1.5 text-sm text-white/80 font-bold">
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            <span className={`px-3 py-1.5 rounded-full border text-sm font-extrabold ${statusColors[roll.status]} bg-white shadow-md`}>
              {roll.status}
            </span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <AnalyticsCard label="Remaining Stock" value={`${analytics.remainingSqft.toFixed(1)}`} unit="sqft" subValue={`${Number(roll.remainingLengthFt)}ft${Number((roll as any).remainingLengthInch || 0) > 0 ? ` ${(roll as any).remainingLengthInch}in` : ''}`} tone="emerald" icon={Ruler} percent={analytics.percentRemaining} />
        <AnalyticsCard label="Sold So Far" value={`${analytics.soldSqft.toFixed(1)}`} unit="sqft" subValue={`${analytics.soldLength.toFixed(1)}ft sold`} tone="blue" icon={TrendingDown} percent={analytics.percentSold} trend="up" />
        {showPrice && (
          <AnalyticsCard label="Revenue Earned" value={formatPKRFull(analytics.estimatedRevenue)} subValue={`Margin: ${analytics.profitMargin.toFixed(1)}%`} tone="violet" icon={DollarSign} trend="up" />
        )}
        {showCost && (
          <AnalyticsCard label="Estimated Profit" value={formatPKRFull(analytics.estimatedProfit)} subValue={`from ${analytics.soldSqft.toFixed(1)} sqft`} tone="amber" icon={TrendingUp} trend="up" />
        )}
      </div>
        ...
        <div className="rounded-3xl bg-white border-2 border-slate-200 p-5 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2 mt-4">
            <div className="rounded-xl bg-slate-50 border-2 border-slate-200 p-3 text-center">
              ...
              <div className="text-base font-extrabold text-slate-900 tabular-nums mt-1">
            ...
            <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-3 text-center">
              <div className="text-[10px] uppercase font-extrabold text-blue-700">Sold</div>
              <div className="text-base font-extrabold text-blue-700 tabular-nums mt-1">{analytics.soldLength.toFixed(1)}ft</div>
            ...
            <div className="rounded-xl bg-emerald-50 border-2 border-emerald-200 p-3 text-center">
              ...
              <div className="text-base font-extrabold text-emerald-700 tabular-nums mt-1">
          ...
          <div className="rounded-3xl bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 border-2 border-amber-300 p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 font-semibold">Investment in remaining stock</p>
              </div>
            </div>
            <div className="space-y-3">
              <div>
                <div className="text-[10px] uppercase font-extrabold text-amber-700">Cost Value</div>
                <div className="text-3xl font-extrabold text-amber-900 tabular-nums">{formatPKRFull(analytics.lockedCapital)}</div>
              </div>
              <div className="grid grid-cols-2 gap-2 pt-3 border-t-2 border-amber-200">
                <div>
                  ...
                  <div className="text-base font-extrabold text-amber-900 tabular-nums">{formatPKRFull(analytics.potentialRevenue)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase font-extrabold text-emerald-700">Potential Profit</div>
                  <div className="text-base font-extrabold text-emerald-700 tabular-nums">{formatPKRFull(analytics.potentialProfit)}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            {([
              ...
              return (
                <button
                  ...
                  className={`px-4 py-3 rounded-t-xl text-sm font-extrabold inline-flex items-center gap-1.5 whitespace-nowrap transition border-b-2 ${
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl bg-rose-50/30 border-2 border-rose-200 p-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {roll.status === 'ACTIVE' && (
                    <button
                      onClick={() => { if (confirm('Mark roll as finished?')) markFinishedMutation.mutate(); }}
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-extrabold text-slate-700 border-2 border-slate-300"
                    >
                      <CheckCircle2 className="h-4 w-4" /> Mark Finished
                    </button>
                  )}
                  <button
                    onClick={() => { if (confirm(`Delete roll ${roll.rollNumber}? This cannot be undone.`)) removeMutation.mutate(); }}
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-rose-600 hover:bg-rose-700 text-sm font-extrabold text-white shadow-md"
                  >
                    <Trash2 className="h-4 w-4" /> Delete Roll Permanently
                  </button>
                </div>
              </div>
            </div>
            ...
            <div>
              {!roll.movements || roll.movements.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="font-extrabold text-slate-700 text-base">No movements yet</p>
                  <p className="text-sm text-slate-500 mt-1">All roll activity will appear here</p>
                </div>
              ) : (
            ...
            <div>
              {!roll.cutPieces || roll.cutPieces.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="font-extrabold text-slate-700 text-base">No cut pieces from this roll</p>
                  <p className="text-sm text-slate-500 mt-1">Leftover pieces will appear here after cutting</p>
                </div>
              ) : (
                <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {roll.cutPieces.map((piece: any) => (
                    <div key={piece.id} className="rounded-xl border-2 border-violet-200 bg-gradient-to-br from-violet-50 to-purple-50 p-3 hover:border-violet-400 hover:shadow-md transition">
                      ...
                      {showPrice && (
                        <div className="text-base font-extrabold text-emerald-700 mt-2">{formatPKRFull(piece.salePrice)}</div>
                      )}
          ...
          {tab === 'edit' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  <h3 className="font-extrabold text-slate-900 text-xl">Edit Roll Details</h3>
                  <p className="text-sm text-slate-500 font-semibold">Update any field — audit log mein record hoga</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setTab('overview')} className="px-4 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-extrabold text-slate-700">Cancel</button>
              ...
              <EditSection icon={DollarSign} title="Pricing (per sqft)" tone="blue">
                ...
                {editForm.costPerSqft > 0 && editForm.salePricePerSqft > 0 && (
                  <div className="mt-2 rounded-lg bg-emerald-100 border border-emerald-300 p-2 text-sm">
                    ...
                    <span className="text-emerald-700 ml-2">(Rs {(editForm.salePricePerSqft - editForm.costPerSqft).toFixed(2)} per sqft)</span>
                  </div>
                )}
              </EditSection>
...
function AnalyticsCard({ label, value, unit, subValue, tone, icon: Icon, percent, trend }: any) {
  ...
  return (
    <div className={`rounded-2xl bg-gradient-to-br border-2 p-4 shadow-sm ${tones[tone]}`}>
      ...
      {subValue && <div className="text-xs font-bold opacity-70 mt-1.5">{subValue}</div>}
      {percent !== undefined && (
...
function DetailRow({ label, value, mono, icon: Icon, tone }: any) {
  ...
  return (
    <div className={`rounded-xl border-2 p-3 ${tone ? toneClass[tone] : 'bg-white border-slate-200'}`}>
      ...
      <div className={`text-base font-extrabold text-slate-900 mt-1 ${mono ? 'font-mono' : ''}`}>{value}</div>
    </div>
  );
}
    ...
    <div className="relative pl-10">
      ...
      <div className="rounded-xl border-2 border-slate-200 bg-white p-3 hover:border-slate-300 transition">
        <div className="flex items-center justify-between gap-2 mb-1">
          ...
          <div className={`text-base font-extrabold tabular-nums ${isNegative ? 'text-rose-600' : 'text-emerald-600'}`}>
        ...
        {movement.note && <div className="text-xs text-slate-500 mt-1 italic">"{movement.note}"</div>}
        <div className="text-xs text-slate-400 mt-1 font-bold inline-flex items-center gap-1">
...
function CutFromRollModal({ rollId, rollWidthFt, remainingLengthFt, salePricePerSqft, onClose, onSuccess }: any) {
  ...
  const cutMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(`Cut ${res.cutLengthFt}ft (${res.cutSqft.toFixed(2)} sqft). ` + (res.leftoverPiece ? `Leftover cut piece created.` : ''));
      onSuccess();
    },
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[95vh] flex flex-col">
        <div className="bg-gradient-to-br from-emerald-800 to-emerald-700 text-white p-5 flex items-center justify-between shrink-0">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-extrabold">Customer Sale</h2>
            <p className="text-sm text-white/85 mt-1 font-semibold">Roll se length cut karein — stock auto reduce hoga</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-3">
            <div className="font-extrabold text-blue-900">Roll Info</div>
            <div className="text-blue-700 mt-1 text-sm font-semibold">
          ...
          {realLength > 0 && customerWidthFt > 0 && (
            <div className="rounded-2xl bg-emerald-50 border-2 border-emerald-200 p-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-xs uppercase font-extrabold text-emerald-700">Cut Area</div>
                  <div className="text-3xl font-extrabold text-emerald-900 tabular-nums">{cutSqft.toFixed(2)} <span className="text-base">sqft</span></div>
                  <div className="text-xs text-emerald-700 font-bold">{customerWidthFt}ft × {lengthFt}ft{lengthInch > 0 ? ` ${lengthInch}in` : ''}</div>
                </div>
                <div>
                  <div className="text-xs uppercase font-extrabold text-emerald-700">Sale Price</div>
                  <div className="text-3xl font-extrabold text-emerald-900 tabular-nums">{formatPKRFull(cutPrice)}</div>
                  <div className="text-xs text-emerald-700 font-bold">@ {formatPKRFull(salePricePerSqft)}/sqft</div>
                </div>
              </div>
              ...
              {widthDiff > 0.1 && (
                <div className="mt-3 pt-3 border-t-2 border-emerald-200">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      <div className="text-xs uppercase font-extrabold text-amber-700">Leftover Width</div>
                      <div className="text-base font-extrabold text-amber-900">{widthDiff.toFixed(2)}ft × {lengthFt}ft = {leftoverSqft.toFixed(2)} sqft</div>
                    </div>
        ...
        <div className="bg-slate-50 border-t-2 border-slate-200 p-4 flex items-center justify-between shrink-0">
...
function AdjustRollModal({ rollId, currentRemaining, onClose, onSuccess }: any) {
  ...
  const adjustMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Roll adjusted'); onSuccess(); },
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-br from-amber-700 to-orange-700 text-white p-5 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-extrabold">Adjust Roll</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <Input label="Length Delta (ft) *" type="number" step="1" value={lengthDeltaFt} onChange={(e) => setLengthDeltaFt(Number(e.target.value))} hint="+ to add, - to reduce" />
