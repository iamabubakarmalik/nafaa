...
import { useCarpetWizard, type WizardStep } from '../hooks/useCarpetWizard';
import { CarpetWizardStepper } from '../components/wizard/CarpetWizardStepper';
import { CarpetWizardStep1Basic } from '../components/wizard/CarpetWizardStep1Basic';
import { CarpetWizardStep2Variants } from '../components/wizard/CarpetWizardStep2Variants';
import { CarpetWizardStep3Rolls } from '../components/wizard/CarpetWizardStep3Rolls';
import { CarpetWizardSummary } from '../components/wizard/CarpetWizardSummary';
import { saveCarpetWizard, updateCarpetWizard, loadCarpetProductForEdit, healOrphanCarpetRolls, type CarpetWizardSaveResult } from '../api/carpet-wizard.api';
import { forceRefreshProducts } from '@/lib/offline/offlineProducts';

export default function CarpetProductWizardPage() {
  ...
  const {
    ...
    addRoll, duplicateRoll, updateRoll, removeRoll,
    addPieceLine, updatePieceLine, duplicatePieceLine, removePieceLine,
    upsertFtStock,
    reset,
    setDraftDirect,
  ...
  React.useEffect(() => {
    if (isEditMode && editQuery.data && !hydrated) {
      setDraftDirect({
        step: 1,
        basic: editQuery.data.basic as any,
        hasVariants: editQuery.data.hasVariants,
        variants: editQuery.data.variants as any,
        rolls: editQuery.data.rolls as any,
        pieceLines: editQuery.data.pieceLines as any,
        ftStock: [],
      });
      setHydrated(true);
    }
  }, [isEditMode, editQuery.data, hydrated, setDraftDirect]);

  const [savedResult, setSavedResult] = useState<CarpetWizardSaveResult | null>(null);
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <div className="mt-3 text-sm font-extrabold text-slate-700">Loading product...</div>
          <div className="text-xs text-slate-500 font-semibold mt-1">Variants, rolls, pieces</div>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto py-12 space-y-6">
        <div className="rounded-3xl bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-300 p-8 text-center shadow-xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            ...
            <SuccessStat label="Sqft" value={savedResult.totalSqft.toFixed(0)} />
          </div>
        </div>
    ...
    <div className="space-y-5 pb-24">
      ...
      <div className="grid xl:grid-cols-[1fr_360px] gap-5 items-start">
        <div className="min-w-0">
          ...
          {draft.step === 3 && (
            <CarpetWizardStep3Rolls
              basic={draft.basic}
              hasVariants={draft.hasVariants}
              variants={draft.variants}
              rolls={draft.rolls}
              pieceLines={draft.pieceLines}
              ftStock={draft.ftStock}
              onAddRoll={addRoll}
              onDuplicateRoll={duplicateRoll}
              onUpdateRoll={updateRoll}
              onRemoveRoll={removeRoll}
              onAddPieceLine={addPieceLine}
              onUpdatePieceLine={updatePieceLine}
              onDuplicatePieceLine={duplicatePieceLine}
              onRemovePieceLine={removePieceLine}
              onUpsertFtStock={upsertFtStock}
