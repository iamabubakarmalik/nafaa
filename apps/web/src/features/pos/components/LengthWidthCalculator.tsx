      ...
      <div className="w-full sm:max-w-2xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 text-white shrink-0">
          ...
          <div className="relative px-6 py-5 flex items-center justify-between gap-3">
            ...
            <button onClick={onClose} className="h-10 w-10 rounded-xl bg-white/15 hover:bg-white/25 border border-white/20 flex items-center justify-center transition shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 bg-slate-50/30 min-h-0">
            ...
            return (
              <div
                ...
                className={`rounded-2xl border-2 p-4 transition ${
                  hasValue ? 'border-emerald-300 bg-emerald-50/60' : 'border-slate-200 bg-white hover:border-emerald-200'
                }`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="inline-flex items-center gap-2">
                    ...
                    {hasValue && <CheckCircle2 className="h-4 w-4 text-emerald-600" />}
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => duplicatePiece(piece.id)} className="h-8 w-8 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 flex items-center justify-center transition" title="Duplicate">
                      <Copy className="h-4 w-4" />
                    </button>
                    <button onClick={() => removePiece(piece.id)} className="h-8 w-8 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center transition" title="Remove">
                ...
                <div className={`grid gap-2 ${isAreaBased ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2'}`}>
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Length</label>
                    <div className="relative">
                      <input
                        ...
                        placeholder="0"
                        className="h-12 w-full rounded-xl border-2 border-slate-200 bg-white px-3 pr-10 text-xl font-extrabold text-slate-900 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition"
                      />
                  ...
                  {isAreaBased && (
                    <div>
                      <label className="block text-[10px] font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Width</label>
                      <div className="relative">
                        <input
                          type="number" inputMode="decimal" step="0.01"
                          value={piece.width}
                  ...
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-600 mb-1 uppercase tracking-wider">Pieces</label>
                    <input
                      type="number" inputMode="numeric" min="1" step="1"
                      value={piece.quantity}
                      onChange={(e) => updatePiece(piece.id, 'quantity', e.target.value)}
                      placeholder="1"
                      className="h-12 w-full rounded-xl border-2 border-slate-200 bg-white px-3 text-xl font-extrabold text-slate-900 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition"
                    />
                  </div>

                  <div>
                    <label className="block text-[10px] font-extrabold text-emerald-700 mb-1 uppercase tracking-wider">Sub Total</label>
                    <div className="h-12 rounded-xl bg-emerald-50 border-2 border-emerald-300 px-3 flex items-center justify-end shadow-sm">
                      <div className="text-xl font-extrabold text-emerald-700 tabular-nums">
                        {calc.total.toFixed(2)}
                      </div>
                      <div className="text-[10px] font-extrabold text-emerald-600 ml-1">{unit}</div>
                    </div>
                  </div>
                </div>
          ...
          <button
            onClick={addPiece}
            className="w-full h-12 rounded-xl border-2 border-dashed border-emerald-300 hover:border-emerald-500 text-emerald-700 hover:bg-emerald-50 font-extrabold text-base transition inline-flex items-center justify-center gap-2 group"
          >
