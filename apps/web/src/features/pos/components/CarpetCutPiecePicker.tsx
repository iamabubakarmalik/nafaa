    ...
    let result = q
      ? pieces.filter(
          (p) =>
            ...
            ((p as any).notes || '').toLowerCase().includes(q) ||
            ((p as any).sourceRoll?.rollNumber || '').toLowerCase().includes(q),
        )
      : [...pieces];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="w-full sm:max-w-5xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-white/80 font-semibold mt-1 flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-violet-300 font-extrabold">{totalSqft.toFixed(0)} sqft total</span>
                </p>
              </div>
            </div>
        ...
        {/* SEARCH + SORT */}
        <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/50 space-y-2 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="h-12 w-full rounded-xl border-2 border-slate-200 bg-white pl-11 pr-11 text-base font-semibold focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200 transition"
            />
            {search && (
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-slate-50/30 min-h-0">
          {isLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="h-52 rounded-2xl bg-slate-100 animate-pulse" />)}
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <div className="mt-4 font-extrabold text-slate-900 text-lg">No cut pieces</div>
              <p className="text-sm text-slate-500 mt-1 max-w-sm text-center font-semibold">
                {search ? 'Try different search' : 'Cut pieces auto-create hote hain jab carpet roll cut karte hain'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered.map((piece) => {
                ...
                return (
                  <button
                    ...
                    className="group rounded-2xl border-2 border-slate-200 bg-white p-3 text-left hover:border-violet-500 hover:shadow-xl hover:-translate-y-1 transition-all"
                  >
                    ...
                    {(piece as any).variant && (
                      <div className="text-xs font-extrabold text-violet-700 flex items-center gap-1 mb-2 bg-violet-50 rounded-md px-1.5 py-0.5">
                        {(piece as any).variant.colorHex && (
                          <span className="h-2 w-2 rounded-full border border-white" style={{ backgroundColor: (piece as any).variant.colorHex }} />
                        )}
                        <span className="truncate">{(piece as any).variant.name}</span>
                      </div>
                    )}

                    <div className="rounded-xl bg-slate-50 border-2 border-slate-200 p-2 mb-2">
                      <div className="text-[10px] uppercase font-extrabold text-slate-500 flex items-center gap-0.5">
                        <Ruler className="h-2.5 w-2.5" />
                        Dimensions
                      </div>
                      <div className="font-extrabold text-slate-900 text-base tabular-nums mt-0.5">
                        {widthFt}ft{widthInch > 0 ? ` ${widthInch}in` : ''} × {lengthFt}ft{lengthInch > 0 ? ` ${lengthInch}in` : ''}
                      </div>
                      <div className="text-sm font-extrabold text-violet-700 tabular-nums">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {(piece as any).sourceRoll && (
                        <div className="text-right shrink-0">
                          ...
                          <div className="text-[10px] font-mono font-extrabold text-slate-700 truncate max-w-[80px]">
                    ...
                    {(piece as any).condition && (piece as any).condition !== 'Good' && (
                      <div className="mt-2 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-700 text-[10px] font-extrabold uppercase tracking-wider">
                        {(piece as any).condition}
                      </div>
                    )}
                  </button>
                );
