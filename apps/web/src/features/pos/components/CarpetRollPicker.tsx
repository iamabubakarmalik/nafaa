...
export function CarpetRollPicker({ product, variant, preSelectedRoll, onConfirm, onClose }: Props) {
    ...
    let result = q
      ? rolls.filter(
          (r) =>
            ...
            ((r as any).rackNumber || '').toLowerCase().includes(q),
        )
      : [...rolls];
  ...
  const calc = useMemo(() => {
    ...
    const availableReal =
      ...
      Number((selectedRoll as any).remainingLengthInch || 0) / 12;
    const remainingAfterCut = availableReal - cutLength;

    const defaultRate = Number((selectedRoll as any).salePricePerSqft);
    const wholesaleRate = (selectedRoll as any).wholesalePricePerSqft
      ? Number((selectedRoll as any).wholesalePricePerSqft)
      : null;
    ...
    const widthError =
      customerWidth > rollFullWidth + 0.01
        ? `Width ${rollFullWidth.toFixed(2)}ft se zyada nahi ho sakti`
        : customerWidth < 0.1
        ? `Width too small`
        : null;
    const lengthError =
      cutLength > availableReal + 0.01
        ? `Length available (${availableReal.toFixed(2)}ft) se zyada nahi`
        : null;
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="w-full sm:max-w-6xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[95dvh] sm:max-h-[92vh] flex flex-col min-h-0">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-900 to-emerald-700 text-white shrink-0">
          ...
          <div className="relative px-5 py-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              {showMobileConfig && (
                <button onClick={() => setShowMobileConfig(false)} className="lg:hidden h-10 w-10 rounded-xl bg-white/15 backdrop-blur flex items-center justify-center shrink-0">
            ...
            <button onClick={onClose} className="h-10 w-10 rounded-xl bg-white/15 hover:bg-white/25 backdrop-blur border border-white/20 flex items-center justify-center transition shrink-0">
              ...
              <div className="px-4 py-3 border-b border-slate-100 bg-white space-y-2 shrink-0">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search roll # or design..."
                  ...
                  />
                  {search && (
                    <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 h-7 w-7 rounded-md hover:bg-slate-100 flex items-center justify-center">
                  ...
                  <div className="space-y-2">
                    {filtered.map((roll) => {
                      ...
                      const percentRemaining =
                        (roll as any).originalLengthFt > 0
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                  ...
                  <div className="rounded-2xl bg-white border-2 border-emerald-200 p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[11px] font-extrabold text-slate-700 mb-1 uppercase">Feet</label>
                        <input
                          ref={widthInputRef}
                          type="number" inputMode="decimal" step="1" min="0"
                          value={customerWidthFt}
                          onChange={(e) => setCustomerWidthFt(e.target.value)}
                          className={`h-14 w-full rounded-xl border-2 px-3 text-2xl font-extrabold focus:outline-none focus:ring-2 transition ${
                            calc?.widthError ? 'border-rose-300 bg-rose-50 focus:border-rose-500 focus:ring-rose-200' : 'border-emerald-200 bg-white focus:border-emerald-500 focus:ring-emerald-200'
                          }`}
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-extrabold text-slate-700 mb-1 uppercase">Inches (0-11)</label>
                        <input
                          type="number" inputMode="numeric" step="1" min="0" max="11"
                          value={customerWidthInch}
                          onChange={(e) => setCustomerWidthInch(e.target.value)}
                          placeholder="0"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. 10"
                          className={`h-14 w-full rounded-xl border-2 px-3 text-2xl font-extrabold focus:outline-none focus:ring-2 transition ${
                            calc?.lengthError ? 'border-rose-300 bg-rose-50 focus:border-rose-500 focus:ring-rose-200' : 'border-emerald-200 bg-white focus:border-emerald-500 focus:ring-emerald-200'
                          }`}
                        />
                      </div>
                      <div>
                  ...
                  <div className="rounded-2xl bg-amber-50 border-2 border-amber-300 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      {calc?.rateSource !== 'default' && (
                        <button onClick={resetRate} className="inline-flex items-center gap-0.5 text-xs font-extrabold text-amber-700 hover:text-amber-900 hover:underline">
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                      {calc?.wholesaleRate && (
                        <button
                          onClick={() => { setUseWholesale(true); setCustomRate(''); }}
                          className={`h-9 rounded-lg border-2 text-xs font-extrabold transition ${
                            useWholesale ? 'border-violet-500 bg-violet-100 text-violet-900 shadow-sm' : 'border-violet-200 bg-white text-violet-700 hover:border-violet-400'
                          }`}
                        >
                          W/S
                        </button>
                      ...
                      {[5, 10, 15].map((p) => (
                        <button key={p} onClick={() => applyDiscount(p)} className="h-9 rounded-lg border-2 border-amber-200 bg-white text-amber-700 hover:border-amber-400 text-xs font-extrabold inline-flex items-center justify-center gap-0.5 transition">
                      ...
                      {calc.widthDiff > 0.05 && (
                        <div className="pt-2 border-t border-slate-300">
                          <label className="flex items-start gap-2 cursor-pointer">
                            <input type="checkbox" checked={createLeftover} onChange={(e) => setCreateLeftover(e.target.checked)} className="h-5 w-5 rounded mt-0.5" />
        ...
        <div className="border-t-2 border-slate-200 bg-emerald-50 px-5 py-3 flex items-center justify-between gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2.5 rounded-xl text-sm font-extrabold text-slate-600 hover:bg-slate-100 transition">Cancel</button>

          {selectedRoll && !showMobileConfig && (
            <button onClick={() => setShowMobileConfig(true)} className="lg:hidden flex-1 px-4 py-3 rounded-xl bg-emerald-600 text-white text-base font-extrabold inline-flex items-center justify-center gap-1.5">
