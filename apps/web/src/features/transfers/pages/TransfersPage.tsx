...
import {
  ...
  CheckCircle2, XCircle, Calendar, Download, Eye, Trash2, AlertCircle,
} from 'lucide-react';
...
type TransferItem = { productId: string; productName: string; unit: string; quantity: number };
...
export default function TransfersPage() {
  ...
  const addProduct = (p: any) => {
    ...
    setItems([...items, { productId: p.id, productName: p.name, unit: p.unit, quantity: 1 }]);
    setProductSearch('');
  };
          ...
          <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="px-5 py-4 border-b border-slate-200 bg-gradient-to-br from-cyan-50 to-blue-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">Move inventory between shops</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-11 w-full rounded-xl border-2 border-slate-200 pl-10 pr-3 text-sm focus:outline-none focus:border-cyan-500"
                  />
                </div>
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-slate-500">
                            {p.sku ? `${p.sku} • ` : ''}{p.stock} {p.unit} • {formatPKR(p.price)}
                          </div>
                        </div>
              ...
              {items.length === 0 ? (
                <div className="rounded-xl border-2 border-dashed border-slate-200 p-8 text-center">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Search aur products select karein</p>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  {items.map((it) => (
                    <div key={it.productId} className="rounded-xl border border-slate-200 bg-white p-3 flex items-center gap-3">
                      <div className="h-10 w-10 rounded-lg bg-cyan-100 text-cyan-700 flex items-center justify-center shrink-0">
                        <Package className="h-5 w-5" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="font-bold text-slate-900 text-sm truncate">{it.productName}</div>
                      ...
                      />
                      <button onClick={() => removeItem(it.productId)} className="h-9 w-9 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center">
            ...
            <div className="px-5 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between gap-3">
              <div className="text-xs text-slate-500 font-semibold">
                {items.length > 0 && `${items.length} item${items.length !== 1 ? 's' : ''} • ${items.reduce((s, i) => s + i.quantity, 0).toFixed(2)} total qty`}
              </div>
              <div className="flex gap-2">
                ...
                <Button onClick={handleCreate} loading={createMutation.isPending} disabled={items.length === 0 || !fromShopId || !toShopId} className="bg-cyan-600 hover:bg-cyan-700">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="grid sm:grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl bg-slate-50 p-3">
                  ...
                  <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border" style={{}}>
                    {(() => {
                      const cfg = statusConfig[viewTransfer.status];
                      const Icon = cfg.icon;
                      return (
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${cfg.tone}`}>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {viewTransfer.items.map((it) => (
                    <div key={it.id} className="rounded-xl border border-slate-200 p-3 flex items-center gap-3">
                      <Package className="h-5 w-5 text-cyan-600 shrink-0" />
                      ...
                      <div className="font-extrabold text-cyan-700">
                        {it.quantity} <span className="text-xs text-slate-500 font-normal">{it.product.unit}</span>
                      </div>
                    </div>
                  ))}
                </div>
