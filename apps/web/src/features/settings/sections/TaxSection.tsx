  ...
  return (
    <div className="space-y-5">
      <SectionCard title="Tax Settings" desc="GST, sales tax configuration" icon={Percent} color="amber">
        <Toggle checked={s.enableTax} onChange={(v) => set('enableTax', v)} label="Enable Tax" desc="Sales par tax automatically calculate ho" />
        {s.enableTax && (
          <>
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              ...
              <Field label="Tax Calculation">
                <ChoiceGroup value={String(s.taxInclusive)} onChange={(v) => set('taxInclusive', v === 'true')} options={[
      ...
      <SectionCard title="Pricing Rules" desc="Default markup aur rounding" icon={DollarSign} color="amber">
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <Field label="Round Prices To" hint="Nearest amount pe round">
            <ChoiceGroup value={String(s.roundPriceTo)} onChange={(v) => set('roundPriceTo', Number(v))} options={[
