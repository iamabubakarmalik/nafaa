...
import { Store, ChevronDown, Check, MapPin, Phone, Plus } from 'lucide-react';
...
export default function ShopSelector() {
  ...
  const { currentShopId, setCurrentShop } = useAuthStore();
  ...
  if (isLoading) {
    return (
      <div className="h-10 px-3 rounded-xl bg-slate-100 animate-pulse w-44" />
    );
  }
              ...
              return (
                ...
                >
                  ...
                  >
                    <Store
                      className={`h-5 w-5 ${
                        shop.isMain || active ? 'text-white' : 'text-slate-500'
                      }`}
                    />
                  </div>
