import { Truck, Package, Clock, MapPinned, UtensilsCrossed } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <ToggleCard
          ...
          color="violet"
          label="Dine-in"
          desc="Restaurant only"
          checked={s.offersDineIn ?? false}
          onChange={(v: boolean) => set('offersDineIn', v)}
        />
      </div>
      ...
      {s.offersPickup && (
        <div className="rounded-2xl bg-blue-50 border-2 border-blue-200 p-5 space-y-4">
          <h4 className="text-sm font-black text-blue-900 flex items-center gap-2">
            <Package className="h-4 w-4" />
            Pickup Configuration
          </h4>
          <Field label="Estimated Pickup Ready Time (Minutes)" color="blue">
            <input
              type="number"
              min={5}
              value={s.estimatedPickupMinutes ?? 15}
              onChange={(e) => set('estimatedPickupMinutes', Number(e.target.value))}
              className="w-full h-11 px-3 rounded-xl border-2 border-blue-300 bg-white text-sm font-bold outline-none focus:border-blue-500"
            />
          </Field>
        </div>
      )}
...
function ToggleCard({ icon: Icon, color, label, desc, checked, onChange }: any) {
  const colors: any = {
    ...
    violet:  { bg: 'bg-violet-500',  light: 'bg-violet-50',  border: 'border-violet-500'  },
  };
  ...
  return (
    <label className={`flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition ${
      ...
      <div className={`h-6 w-11 rounded-full transition ${checked ? c.bg : 'bg-slate-200'} relative shrink-0`}>
...
function Field({ label, hint, children, color = 'emerald' }: any) {
  const textColor = color === 'blue' ? 'text-blue-900' : 'text-emerald-900';
  const hintColor = color === 'blue' ? 'text-blue-700' : 'text-emerald-700';
  return (
    <div>
      <label className={`text-xs font-black ${textColor} mb-1.5 block`}>{label}</label>
      {children}
      {hint && <p className={`text-xs ${hintColor} mt-1 font-medium`}>{hint}</p>}
    </div>
  );
}
